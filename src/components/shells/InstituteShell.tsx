...
import { LayoutDashboard, Users, BookOpen, Settings, LogOut, ChevronLeft, BarChart3, Bell, DollarSign } from 'lucide-react';

const NAV = [
  { href: '/institute/dashboard', icon: LayoutDashboard, label: 'لوحة التحكم' },
  { href: '/institute/students',  icon: Users,            label: 'الطلاب' },
  { href: '/institute/courses',   icon: BookOpen,         label: 'الكورسات' },
  { href: '/institute/finance',   icon: DollarSign,       label: 'المالية' },
  { href: '/institute/reports',   icon: BarChart3,        label: 'التقارير' },
  { href: '/institute/settings',  icon: Settings,         label: 'الإعدادات' },
];
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0F172A' }}>
      <aside style={{ background: '#1E293B', borderLeft: '1px solid #334155', display: 'flex', flexDirection: 'column', width: collapsed ? '72px' : '240px', transition: 'width 0.25s', flexShrink: 0, height: '100vh', position: 'sticky', top: 0, overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem', borderBottom: '1px solid #334155', minHeight: '64px' }}>
          {!collapsed && (
            <Link href="/institute/dashboard" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span style={{ width: '36px', height: '36px', borderRadius: '10px', background: `linear-gradient(135deg,${ACCENT},#06B6D4)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '700', color: '#fff' }}>م</span>
        ...
        <nav style={{ flex: 1, padding: '1rem 0.5rem', display: 'flex', flexDirection: 'column', gap: '2px', overflowY: 'auto' }}>
          {NAV.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link key={href} href={href} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.65rem 0.75rem', borderRadius: '8px', textDecoration: 'none', background: active ? `rgba(34,211,238,0.12)` : 'transparent', whiteSpace: 'nowrap' }}>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <header style={{ background: '#1E293B', borderBottom: '1px solid #334155', padding: '0 1.5rem', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: `linear-gradient(135deg,${ACCENT},#06B6D4)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '700', color: '#fff' }}>م</div>
          </div>
        </header>
