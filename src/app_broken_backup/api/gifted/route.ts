...
export async function GET(request: Request) {
  try {
    ...
    const result = await pool.query('SELECT * FROM gifted ORDER BY created_at DESC');
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE gifted SET student_name=$1, talent_type=$2, description=$3, program=$4, teacher_name=$5, status=$6 WHERE id::text = $7::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM gifted WHERE id::text = $1::text', [id]);
