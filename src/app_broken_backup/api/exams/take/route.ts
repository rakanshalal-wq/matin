...
export async function POST(request: Request) {
  try {
    ...
    // التحقق من الاختبار
    const examResult = await pool.query('SELECT * FROM exams WHERE id::text = $1::text', [exam_id]);
    ...
    const existingSession = await pool.query(
      'SELECT * FROM exam_sessions WHERE exam_id = $1 AND student_id::text = $2::text AND status != $3',
    ...
    await pool.query(
      ...
      [sessionId, exam_id, user.id, ipAddress, String(user.school_id)]
    );
...
export async function PUT(request: Request) {
  try {
    ...
    if (action === 'submit') {
      // تسليم الاختبار
      const session = await pool.query('SELECT * FROM exam_sessions WHERE id::text = $1::text AND student_id::text = $2::text', [session_id, String(user.id)]);
      ...
      const answers = await pool.query(
        `SELECT sa.*, eq.correct_answer, eq.points FROM student_answers sa
         JOIN exam_questions eq ON eq.id::text = sa.question_id::text WHERE sa.session_id = $1`,
        [session_id]
      );
      ...
      for (const ans of answers.rows) {
        ...
        await pool.query('UPDATE student_answers SET is_correct = $1, score = $2 WHERE id::text = $3::text',
          [isCorrect, isCorrect ? (ans.points || 1) : 0, ans.id]);
      }

      await pool.query(
        'UPDATE exam_sessions SET status = $1, finished_at = NOW(), score = $2 WHERE id::text = $3::text',
      ...
      await pool.query(
        ...
        [String(user.id), examData.exam_id, totalScore, String(user.school_id)]
      );
    ...
    if (existing.rows.length > 0) {
      await pool.query('UPDATE student_answers SET answer = $1, answered_at = NOW() WHERE id::text = $2::text',
        [answer, existing.rows[0].id]);
    } else {
      await pool.query(
        `INSERT INTO student_answers (session_id, question_id, answer, answered_at, school_id)
         VALUES ($1, $2, $3, NOW(), $4)`,
        [session_id, question_id, answer, String(user.school_id)]
      );
    }
