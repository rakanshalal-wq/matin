  ...
  try {
    ...
    if (exam_id) {
      const exam = await pool.query(`
        ...
        FROM exams e
        LEFT JOIN schools s ON s.id::text = e.school_id::text
      ...
      if (['super_admin', 'owner', 'admin', 'teacher'].includes(user.role)) {
        const qResult = await pool.query(
          `SELECT eq.*, qb.question_text as bank_text, qb.options as bank_options, qb.correct_answer as bank_answer
           ...
           LEFT JOIN question_bank qb ON qb.id::text = eq.question_bank_id::text
      ...
      if (['super_admin', 'owner', 'admin', 'teacher'].includes(user.role)) {
        const rResult = await pool.query(
          `SELECT es.*, u.name as student_name
           ...
           LEFT JOIN users u ON u.id::text = es.student_id::integer
  ...
  try {
    ...
    if (questionsArray.length > 0) {
      for (let i = 0; i < questionsArray.length; i++) {
        ...
        try {
          const qData = await pool.query('SELECT * FROM question_bank WHERE id::text = $1::text', [q.question_id]);
          ...
          if (q.question_id) {
            await pool.query('UPDATE question_bank SET usage_count = COALESCE(usage_count, 0) + 1, times_used = COALESCE(times_used, 0) + 1 WHERE id::text = $1::text', [q.question_id]);
          }
        } catch (qErr) {
...
export async function PUT(request: Request) {
  try {
    ...
    // جلب الاختبار
    const examResult = await pool.query('SELECT * FROM exams WHERE id::text = $1::text', [id]);
    ...
    if (action) {
      switch (action) {
        case 'publish':
          ...
          await pool.query(`UPDATE exams SET status = 'PUBLISHED'::"ExamStatus", updated_at = NOW() WHERE id::text = $1::text`, [id]);
        ...
        case 'activate':
          ...
          await pool.query(`UPDATE exams SET status = 'ACTIVE'::"ExamStatus", updated_at = NOW() WHERE id::text = $1::text`, [id]);
          return NextResponse.json({ success: true, message: 'تم تفعيل الاختبار' });

        case 'close':
          // إغلاق الاختبار
          await pool.query(`UPDATE exams SET status = 'COMPLETED'::"ExamStatus", updated_at = NOW() WHERE id::text = $1::text`, [id]);
        ...
        case 'grade_answer':
          ...
          const answer = await pool.query('SELECT * FROM student_answers WHERE id::text = $1::text', [answer_id]);
          ...
          await pool.query(
            `UPDATE student_answers SET grade = $1, is_final = true, graded_by_ai = false WHERE id::text = $2::text`,
...
export async function DELETE(request: Request) {
  try {
    ...
    await pool.query('DELETE FROM exams WHERE id::text = $1::text', [id]);
...
export async function PATCH(request: Request) {
  try {
    ...
    if (action === 'print') {
      // جلب بيانات الاختبار
      const exam = await pool.query('SELECT * FROM exams WHERE id::text = $1::text', [exam_id]);
      if (exam.rows.length === 0) return NextResponse.json({ error: 'الاختبار غير موجود' }, { status: 404 });
      const e = exam.rows[0];
