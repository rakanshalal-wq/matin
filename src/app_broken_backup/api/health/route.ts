...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE health_records SET student_name=$1, condition=$2, blood_type=$3, allergies=$4, medications=$5, doctor_name=$6, notes=$7, status=$8 WHERE id::text = $9::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM health_records WHERE id::text = $1::text', [id]);
