...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE recordings SET title=$1, subject=$2, teacher_name=$3, class_name=$4, duration=$5, file_url=$6, file_size=$7, views_count=$8, date=$9, status=$10 WHERE id::text = $11::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM recordings WHERE id::text = $1::text', [id]);
