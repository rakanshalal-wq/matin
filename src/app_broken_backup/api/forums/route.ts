...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE forums SET title=$1, category=$2, author=$3, description=$4, members_count=$5, posts_count=$6, status=$7 WHERE id::text = $8::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM forums WHERE id::text = $1::text', [id]);
