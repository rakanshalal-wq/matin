...
export async function GET(request: Request) {
  try {
    ...
    let query = `
      ...
      FROM schedules sc
      LEFT JOIN classes c ON c.id::text = sc.class_id::text
      LEFT JOIN courses co ON co.id::text = sc.course_id::text
      LEFT JOIN subjects sub ON sub.id::text = co.subject_id::text
      LEFT JOIN teachers t ON t.id::text = sc.teacher_id::text
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM schedules WHERE id::text = $1::text', [id]);
