...
export async function POST(request: Request) {
  try {
    ...
    // تحقق من حدود الباقة
    if (user.role === 'owner') {
      const schoolCount = await pool.query('SELECT COUNT(*) FROM schools WHERE owner_id::text = $1::text', [String(user.id)]);
    ...
    if (user.role === 'owner' && !user.school_id) {
      await pool.query('UPDATE users SET school_id = $1 WHERE id::text = $2::text', [id, String(user.id)]);
    }
...
export async function PUT(request: Request) {
  try {
    ...
    // تحقق من الملكية
    if (user.role === 'owner') {
      const check = await pool.query('SELECT id FROM schools WHERE id::text = $1::text AND owner_id::text = $2::text', [id, String(user.id)]);
...
export async function DELETE(request: Request) {
  try {
    ...
    await pool.query('DELETE FROM schools WHERE id::text = $1::text', [id]);
