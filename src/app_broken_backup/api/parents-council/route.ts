...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE parents_council SET title=$1, type=$2, date=$3, location=$4, attendees_count=$5, agenda=$6, notes=$7, status=$8 WHERE id::text = $9::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM parents_council WHERE id::text = $1::text', [id]);
