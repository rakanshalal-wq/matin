...
export async function GET(request: Request) {
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM store_settings WHERE school_id::text = $1::text',
      [user.school_id]
    );
...
export async function POST(request: Request) {
  try {
    ...
    const existing = await pool.query('SELECT id FROM store_settings WHERE school_id::text = $1::text', [user.school_id]);
    ...
    if (existing.rows.length > 0) {
      await pool.query(
        `UPDATE store_settings SET store_model = COALESCE($1, store_model), store_name = COALESCE($2, store_name),
         ...
         WHERE school_id::text = $6::text`,
        [store_model, store_name, currency, tax_rate, shipping_enabled, String(user.school_id)]
      );
    } else {
      await pool.query(
        ...
        [String(user.school_id), store_model || 'products', store_name || 'متجر المدرسة', currency || 'SAR', tax_rate || 15, shipping_enabled || false]
      );
    }
