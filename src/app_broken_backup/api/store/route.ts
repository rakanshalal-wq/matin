  ...
  try {
    ...
    if (action === 'get_cart') {
      ...
      const result = await pool.query(`
        ...
        FROM cart_items ci
        LEFT JOIN store_products sp ON sp.id::text = ci.product_id::text
    ...
    if (action === 'remove_from_cart') {
      ...
      const userId = user.id || user.user_id;
      if (cart_item_id) {
        await pool.query('DELETE FROM cart_items WHERE id::text = $1::text AND user_id::text = $2::text', [cart_item_id, userId]);
      } else if (product_id) {
        await pool.query('DELETE FROM cart_items WHERE product_id = $1 AND user_id::text = $2::text', [product_id, userId]);
      }
    ...
    if (action === 'update_quantity') {
      ...
      if (quantity <= 0) {
        await pool.query('DELETE FROM cart_items WHERE id::text = $1::text', [cart_item_id]);
      } else {
        await pool.query('UPDATE cart_items SET quantity = $1, updated_at = NOW() WHERE id::text = $2::text', [quantity, cart_item_id]);
      }
      return NextResponse.json({ success: true });
    }
    ...
    if (action === 'checkout') {
      ...
      for (const item of cartItems.rows) {
        await pool.query('UPDATE store_products SET stock = GREATEST(stock - $1, 0) WHERE id::text = $2::text', [item.quantity, item.product_id]);
      }

      // تفريغ السلة
      await pool.query('DELETE FROM cart_items WHERE user_id::text = $1::text', [userId]);

      // ===== بنية بوابة الدفع الجاهزة =====
      // عند التعاقد مع شركة مدفوعات، فقط أضف الكود هنا:
      // 
      // const paymentSettings = await pool.query('SELECT * FROM payment_settings WHERE school_id::text = $1::text', [ids.school_id]);
    ...
    if (action === 'my_orders') {
      const userId = user.id || user.user_id;
      const result = await pool.query(
        `SELECT * FROM store_orders WHERE user_id::text = $1::text ORDER BY created_at DESC`,
        [userId]
      );
      return NextResponse.json(result.rows);
    }
...
export async function PUT(request: Request) {
  try {
    ...
    if (type === 'order') {
      ...
      const result = await pool.query('UPDATE store_orders SET status=$1, payment_status=$2 WHERE id::text = $3::text RETURNING *', [status, payment_status, id]);
      return NextResponse.json(result.rows[0]);
    }

    const { name, description, price, sale_price, image, category, stock, is_active } = body;
    const result = await pool.query(
      'UPDATE store_products SET name=$1, description=$2, price=$3, sale_price=$4, image=$5, category=$6, stock=$7, is_active=$8 WHERE id::text = $9::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    const type = searchParams.get('type');
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    if (type === 'order') await pool.query('DELETE FROM store_orders WHERE id::text = $1::text', [id]);
    else await pool.query('DELETE FROM store_products WHERE id::text = $1::text', [id]);
    return NextResponse.json({ success: true });
  } catch (error) { console.error('Error:', error); return NextResponse.json({ error: 'فشل' }, { status: 500 }); }
}
