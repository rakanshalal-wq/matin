...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE library_books SET title=$1, author=$2, isbn=$3, category=$4, quantity=$5, status=$6 WHERE id::text = $7::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM library_books WHERE id::text = $1::text', [id]);
