  ...
  try {
    if (type === 'dashboard') {
      ...
      const [statsRes, activityRes, settingsRes] = await Promise.all([
        ...
        pool.query(`
          ...
          LEFT JOIN users u ON ml.moderator_id::text = u.id::text
    ...
    if (type === 'comments') {
      ...
      const res = await pool.query(`
        ...
        LEFT JOIN posts p ON c.post_id::text = p.id::text
        ${whereClause}
    ...
    if (type === 'reports') {
      const res = await pool.query(`
        SELECT cr.*,
          ...
          END as content_text,
          CASE
            WHEN cr.content_type='post' THEN (SELECT u.name FROM posts p LEFT JOIN users u ON p.user_id::text = u.id::text WHERE p.id=cr.content_id)
            WHEN cr.content_type='comment' THEN (SELECT u.name FROM comments c LEFT JOIN users u ON c.user_id::text = u.id::text WHERE c.id=cr.content_id)
          END as content_author
        FROM content_reports cr
        LEFT JOIN users reporter ON cr.reporter_id::text = reporter.id::text
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'hide_post') {
      ...
      await pool.query('UPDATE posts SET is_hidden=true, hidden_reason=$1 WHERE id::text = $2::text', [reason || 'مخالفة قواعد المجتمع', post_id]);
    ...
    if (action === 'show_post') {
      const { post_id } = body;
      await pool.query('UPDATE posts SET is_hidden=false, hidden_reason=NULL WHERE id::text = $1::text', [post_id]);
    ...
    if (action === 'delete_post') {
      ...
      await pool.query('DELETE FROM posts WHERE id::text = $1::text', [post_id]);
    ...
    if (action === 'pin_post') {
      const { post_id } = body;
      await pool.query('UPDATE posts SET is_pinned=true WHERE id::text = $1::text', [post_id]);
    ...
    if (action === 'unpin_post') {
      const { post_id } = body;
      await pool.query('UPDATE posts SET is_pinned=false WHERE id::text = $1::text', [post_id]);
    ...
    if (action === 'hide_comment') {
      ...
      await pool.query('UPDATE comments SET is_hidden=true, hidden_reason=$1 WHERE id::text = $2::text', [reason || 'مخالفة', comment_id]);
    ...
    if (action === 'show_comment') {
      ...
      await pool.query('UPDATE comments SET is_hidden=false, hidden_reason=NULL WHERE id::text = $1::text', [comment_id]);
    ...
    if (action === 'delete_comment') {
      ...
      await pool.query('DELETE FROM comments WHERE id::text = $1::text', [comment_id]);
    ...
    if (action === 'ban_user') {
      ...
      await pool.query('UPDATE users SET is_community_banned=true, community_ban_reason=$1, community_ban_until=NOW() WHERE id::text = $2::text', [reason, user_id]);
    ...
    if (action === 'unban_user') {
      const { user_id } = body;
      await pool.query('UPDATE users SET is_community_banned=false, community_ban_reason=NULL, community_ban_until=NULL WHERE id::text = $1::text', [user_id]);
    ...
    if (action === 'warn_user') {
      const { user_id, reason } = body;
      await pool.query('UPDATE users SET warnings_count = COALESCE(warnings_count,0) + 1 WHERE id::text = $1::text', [user_id]);
    ...
    if (action === 'resolve_report') {
      ...
      await pool.query(
        'UPDATE content_reports SET status=$1, resolved_by=$2, resolved_at=NOW() WHERE id::text = $3::text',
      ...
      for (const post of unscanned.rows) {
        try {
          ...
          if (action_taken === 'hidden') {
            await pool.query("UPDATE posts SET is_hidden=true, hidden_reason='إخفاء تلقائي بواسطة AI' WHERE id::text = $1::text", [post.id]);
            ...
            await pool.query('DELETE FROM posts WHERE id::text = $1::text', [post.id]);
            deleted++;
          }
