...
export async function POST(request: Request) {
  ...
  if (action === 'start_session') {
    ...
    const activeSession = await pool.query(
      `SELECT id FROM exam_proctoring_sessions 
       WHERE student_id::text = $1::text AND exam_id = $2 AND status = 'active'`,
      [String(user.id), exam_id]
    );
  ...
  if (action === 'report_violation') {
    ...
    const sessionResult = await pool.query(
      `SELECT * FROM exam_proctoring_sessions WHERE id::text = $1::text AND student_id::text = $2::text`,
    ...
    if (updatedRisk.risk_score >= 100) {
      await pool.query(
        `UPDATE exam_proctoring_sessions SET status = 'terminated', ended_at = NOW() WHERE id::text = $1::text`,
        [session_id]
      );
      return NextResponse.json({ 
        success: true, 
      ...
      if (info) {
        await pool.query(
          `INSERT INTO exam_violations (session_id, student_id, exam_id, school_id, violation_type, layer, severity, description, evidence_url, ai_confidence)
           ...
           FROM exam_proctoring_sessions WHERE id::text = $1::text`,
  ...
  if (action === 'end_session') {
    ...
    await pool.query(
      `UPDATE exam_proctoring_sessions SET status = 'completed', ended_at = NOW() WHERE id::text = $1::text AND student_id::text = $2::text`,
      [session_id, String(user.id)]
    );
    ...
    const summary = await pool.query(
      `SELECT s.risk_score, s.violations_count, s.is_flagged,
       ...
       LEFT JOIN exam_violations v ON v.session_id::text = s.id::text
