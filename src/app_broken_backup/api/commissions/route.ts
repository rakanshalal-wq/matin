...
export async function GET(request: Request) {
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM commissions WHERE 1=1 ${filter.sql} ORDER BY created_at DESC`,
      filter.params
    );
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE commissions SET name=$1, role=$2, amount=$3, percentage=$4, type=$5, month=$6, year=$7, status=$8 WHERE id::text = $9::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM commissions WHERE id::text = $1::text', [id]);
