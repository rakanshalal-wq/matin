...
export async function GET(request: Request) {
  try {
    ...
    if (lecture_id) {
      const lecture = await pool.query('SELECT * FROM lectures WHERE id::text = $1::text', [lecture_id]);
    ...
    const result = await pool.query(
      `SELECT l.*, 
       ...
       FROM lectures l
       LEFT JOIN users u ON u.id::text = l.teacher_id::text
       LEFT JOIN classes c ON c.id::text = l.class_id::text
...
export async function PUT(request: Request) {
  try {
    ...
    const old = await pool.query('SELECT * FROM lectures WHERE id::text = $1::text', [id]);
    ...
    if (action) {
      switch (action) {
        case 'confirm':
          ...
          await pool.query(
            `UPDATE lectures SET status = 'confirmed_online', confirmed = true, 
             confirmation_status = 'confirmed', updated_at = NOW() WHERE id::text = $1::text`,
            [id]
          );
        ...
        case 'start_live':
          // بدء البث المباشر
          await pool.query(
            `UPDATE lectures SET status = 'live', updated_at = NOW() WHERE id::text = $1::text`,
            [id]
          );
        ...
        case 'complete':
          // إنهاء المحاضرة
          await pool.query(
            `UPDATE lectures SET status = 'completed', updated_at = NOW() WHERE id::text = $1::text`,
            [id]
          );
        ...
        case 'cancel':
          // إلغاء المحاضرة
          await pool.query(
            `UPDATE lectures SET status = 'cancelled', updated_at = NOW() WHERE id::text = $1::text`,
            [id]
          );
        ...
        case 'upload_recording':
          ...
          await pool.query(
            `UPDATE lectures SET recording_url = $1, status = 'completed', updated_at = NOW() WHERE id::text = $2::text`,
            [recording_url, id]
          );
        ...
        case 'force_online':
          ...
          await pool.query(
            `UPDATE lectures SET type = 'online', status = 'confirmed_online', 
             ...
             updated_at = NOW() WHERE id::text = $1::text`,
            [id]
          );
...
export async function DELETE(request: Request) {
  try {
    ...
    const lecture = await pool.query('SELECT status FROM lectures WHERE id::text = $1::text', [id]);
    if (lecture.rows[0]?.status === 'completed') {
      return NextResponse.json({ error: 'لا يمكن حذف محاضرة مكتملة' }, { status: 400 });
    }
    
    await pool.query('DELETE FROM lectures WHERE id::text = $1::text', [id]);
