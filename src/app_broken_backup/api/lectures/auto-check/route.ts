  ...
  try {
    ...
    const lectures3h = await pool.query(
      `SELECT l.*, u.phone, u.email, u.name as teacher_name_real
       FROM lectures l
       LEFT JOIN users u ON u.id::text = l.teacher_id::integer
    ...
    for (const l of lectures3h.rows) {
      ...
      await pool.query('UPDATE lectures SET notification_2h_sent = true WHERE id::text = $1::text', [l.id]);
    }
    ...
    for (const l of lectures2h.rows) {
      ...
      await pool.query('UPDATE lectures SET notification_1h_sent = true WHERE id::text = $1::text', [l.id]);
    }
    ...
    const lectures30m = await pool.query(
      `SELECT l.*, u.phone, u.email, u.name as teacher_name_real,
       ...
       FROM lectures l
       LEFT JOIN users u ON u.id::text = l.teacher_id::integer
       LEFT JOIN users admin ON admin.school_id::text = l.school_id::text AND admin.role = 'admin' AND admin.status = 'active'
       WHERE l.date BETWEEN $1 AND $2
       AND l.type = 'online'
    ...
    if (now.getHours() === 8 && now.getMinutes() < 15) {
      ...
      const teachers3days = await pool.query(
        `SELECT u.id, u.name, u.owner_id, u.school_id,
                admin.phone as admin_phone, admin.email as admin_email, admin.name as admin_name
         FROM users u
         LEFT JOIN users admin ON admin.school_id::text = u.school_id::text 
