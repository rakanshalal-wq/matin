    ...
    if (user.role === "super_admin") {
      const result = await pool.query(
        `SELECT s.*, 
         ...
         LEFT JOIN plans p ON p.id::text = s.plan_id::text
...
export async function PUT(request: Request) {
  try {
    ...
    if (user.role !== "super_admin") {
      const check = await pool.query("SELECT id FROM subscriptions WHERE id::text = $1::text AND owner_id = $2::text", [id, String(user.id)]);
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: "المعرف مطلوب" }, { status: 400 });
    await pool.query("DELETE FROM subscriptions WHERE id::text = $1::text", [id]);
