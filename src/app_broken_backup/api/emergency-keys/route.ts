...
export async function GET(request: Request) {
  try {
    ...
    const result = await pool.query(`
      ...
      LEFT JOIN exams e ON e.id::text = ek.exam_id::text
...
export async function PUT(request: Request) {
  try {
    ...
    const req = await pool.query('SELECT requester_id FROM emergency_keys WHERE id::text = $1::text', [id]);
    ...
    await pool.query(
      'UPDATE emergency_keys SET status = $1, approver_id = $2, approved_at = NOW() WHERE id::text = $3::text',
