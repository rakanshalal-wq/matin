  ...
  try {
    ...
    if (user.role !== 'super_admin') {
      ...
      const result = await pool.query(
        'SELECT * FROM settings WHERE school_id::text = $1::text OR owner_id = $2 ORDER BY updated_at DESC LIMIT 200',
        [user.school_id || null, String(user.id)]
      );
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO settings (id, key, value, category, updated_at, school_id, owner_id)
       ...
       RETURNING *`,
      [settingId, key, value || '', category || 'general', user.school_id || null, String(user.id)]
    );
    return NextResponse.json(result.rows[0], { status: 201 });
  } catch (error) {
    console.error('Error:', error);
  ...
  try {
    ...
    if (user.role === 'super_admin') {
      ...
      await pool.query(
        ...
        [value || '', key, user.school_id || null, String(user.id)]
      );
    }
...
export async function DELETE(request: Request) {
  try {
    ...
    if (user.role === 'super_admin' && key) {
      ...
      await pool.query('DELETE FROM settings WHERE id::text = $1::text', [id]);
    } else {
