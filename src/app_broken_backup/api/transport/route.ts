  ...
  try {
    ...
    if (type === 'riders' && bus_id) {
      const result = await pool.query(`
        ...
        FROM bus_riders br
        LEFT JOIN students s ON s.id::text = br.student_id::text
    ...
    if (type === 'trips') {
      ...
      const result = await pool.query(`
        ...
        FROM bus_trips bt
        JOIN buses b ON b.id::text = bt.bus_id::text
    ...
    if (type === 'events') {
      ...
      const result = await pool.query(`
        ...
        FROM bus_events be
        LEFT JOIN students s ON s.id::text = be.student_id::text
        LEFT JOIN users u ON u.id::text = s.user_id
    ...
    // موقع GPS للباص
    if (type === 'gps' && bus_id) {
      const result = await pool.query(`SELECT gps_lat, gps_lng, last_gps_update FROM buses WHERE id::text = $1::text`, [bus_id]);
      return NextResponse.json(result.rows[0] || {});
    }

    // default: إرجاع كل الباصات
    const defaultResult = await pool.query(`SELECT b.*, (SELECT COUNT(*) FROM bus_riders WHERE bus_id = b.id AND status = 'active') as rider_count FROM buses b WHERE 1=1 ${filter.sql} ORDER BY b.created_at DESC`, filter.params);
    return NextResponse.json(defaultResult.rows);
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'end_trip') {
      const { trip_id } = body;
      await pool.query(`UPDATE bus_trips SET status='completed', ended_at=NOW() WHERE id::text = $1::text`, [trip_id]);
    ...
    if (action === 'student_event') {
      ...
      const studentInfo = await pool.query(`
        ...
        LEFT JOIN users u ON u.id::text = s.user_id WHERE s.id::text = $1::text
      `, [student_id]);
      ...
      if (studentInfo.rows[0]?.parent_id) {
        ...
        await pool.query(`UPDATE bus_events SET notified_parent = true WHERE id::text = $1::text`, [id]);
      }
    ...
    if (action === 'update_gps') {
      ...
      await pool.query(`UPDATE buses SET gps_lat=$1, gps_lng=$2, last_gps_update=NOW() WHERE id::text = $3::text`, [lat, lng, bus_id]);
    ...
    if (action === 'maintenance') {
      ...
      await pool.query(`UPDATE buses SET last_maintenance=NOW(), next_maintenance=$1, status='maintenance' WHERE id::text = $2::text`, [next_date||null, bus_id]);
...
export async function DELETE(request: Request) {
  try {
    ...
    if (type === 'rider') await pool.query('DELETE FROM bus_riders WHERE id::text = $1::text', [id]);
    else { await pool.query('DELETE FROM bus_riders WHERE bus_id=$1', [id]); await pool.query('DELETE FROM buses WHERE id::text = $1::text', [id]); }
