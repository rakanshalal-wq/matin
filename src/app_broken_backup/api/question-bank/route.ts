    ...
    if (action) {
      switch (action) {
        case 'approve':
          ...
          await pool.query(`UPDATE question_bank SET status = 'approved', updated_at = NOW() WHERE id::text = $1::text`, [id]);
          return NextResponse.json({ success: true, message: 'تم الموافقة على السؤال' });

        case 'lock':
          await pool.query(`UPDATE question_bank SET status = 'locked', updated_at = NOW() WHERE id::text = $1::text`, [id]);
        ...
        case 'reject':
          ...
          await pool.query(`UPDATE question_bank SET status = 'rejected', updated_at = NOW() WHERE id::text = $1::text`, [id]);
...
export async function DELETE(request: Request) {
  try {
    ...
    if (user.role === 'super_admin') {
      await pool.query('DELETE FROM question_bank WHERE id::text = $1::text', [id]);
    } else {
      await pool.query('DELETE FROM question_bank WHERE id::text = $1::text AND school_id::text = $2::text', [id, String(user.school_id)]);
    }
