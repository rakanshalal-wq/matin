  ...
  try {
    ...
    if (user.role === 'super_admin') {
      query = `SELECT t.*, u.name, u.email, u.phone, u.national_id, sc.name as school_name
        ...
        LEFT JOIN schools sc ON sc.id::text = t.school_id::text WHERE 1=1`;
    } else if (user.role === 'owner') {
      query = `SELECT t.*, u.name, u.email, u.phone, u.national_id, sc.name as school_name
        FROM teachers t LEFT JOIN users u ON u.id::text = t.user_id
        LEFT JOIN schools sc ON sc.id::text = t.school_id::text
        WHERE t.school_id IN (SELECT id FROM schools WHERE owner_id::text = $1::text)`;
      params = [String(user.id)];
    } else {
      query = `SELECT t.*, u.name, u.email, u.phone, u.national_id, sc.name as school_name
        FROM teachers t LEFT JOIN users u ON u.id::text = t.user_id
        LEFT JOIN schools sc ON sc.id::text = t.school_id::text WHERE t.school_id::text = $1::text`;
...
export async function POST(request: Request) {
  try {
    ...
    let schoolSlug = '';
    if (user.role === 'owner') {
      const s = await pool.query('SELECT id, name, slug FROM schools WHERE owner_id::text = $1::text LIMIT 1', [String(user.id)]);
      ...
      const s = await pool.query('SELECT name, slug FROM schools WHERE id::text = $1::text', [schoolId]);
      schoolName = s.rows[0]?.name || ''; schoolSlug = s.rows[0]?.slug || '';
    }

    const cnt = await pool.query('SELECT COUNT(*) FROM teachers WHERE school_id::text = $1::text', [schoolId]);
...
export async function PUT(request: Request) {
  try {
    ...
    await pool.query(
      `UPDATE teachers SET employee_id=COALESCE($1,employee_id), specialization=$2, department=$3, salary=$4, hire_date=$5, updated_at=NOW() WHERE id::text = $6::text`,
      [employee_id||null, specialization||null, department||null, salary||null, hire_date||null, id]
    );

    const teacher = await pool.query('SELECT user_id FROM teachers WHERE id::text = $1::text', [id]);
...
export async function DELETE(request: Request) {
  try {
    ...
    await pool.query('DELETE FROM teachers WHERE id::text = $1::text', [id]);
