...
export async function POST(request: Request) {
  ...
  if (action === 'encrypt_exam') {
    ...
    const examResult = await pool.query(
      `SELECT * FROM exams WHERE id::text = $1::text ${filter.sql}`,
    ...
    const questionsResult = await pool.query(
      `SELECT q.* FROM questions q
       JOIN exam_questions eq ON eq.question_id::text = q.id::text
       WHERE eq.exam_id = $1`,
      [exam_id]
    );
    ...
    for (const q of questionsResult.rows) {
      if (!q.is_encrypted) {
        ...
        await pool.query(
          `UPDATE questions SET content_encrypted = $1, answer_encrypted = $2, is_encrypted = true WHERE id::text = $3::text`,
    ...
    // تحديث الاختبار
    await pool.query(
      `UPDATE exams SET is_encrypted = true, encryption_key_hash = $1 WHERE id::text = $2::text`,
      [keyHash, exam_id]
    );
  ...
  if (action === 'decrypt_exam_for_student') {
    ...
    const examResult = await pool.query(
      `SELECT * FROM exams WHERE id::text = $1::text AND school_id::text = $2::text AND status = 'ACTIVE'`,
      [exam_id, String(user.school_id)]
    );
  ...
  if (action === 'setup_emergency_keys') {
    ...
    await pool.query(
      `UPDATE emergency_keys SET is_active = false WHERE school_id::text = $1::text AND purpose = $2`,
      [school_id, purpose]
    );
  ...
  if (action === 'use_emergency_keys') {
    ...
    const keyResult = await pool.query(
      `SELECT * FROM emergency_keys 
       WHERE school_id::text = $1::text AND purpose = $2 AND is_active = true
    ...
    if (keyResult.rows.length === 0) {
      ...
      await pool.query(
        ...
        [String(user.id), reason, request.headers.get('x-forwarded-for') || 'unknown']
      );
    ...
    await pool.query(
      `UPDATE emergency_keys SET last_used = NOW() WHERE id::text = $1::text`,
      [emergencyKey.id]
    );
  ...
  if (action === 'log_exam_print') {
    ...
    const examResult = await pool.query(
      `SELECT is_encrypted, print_allowed, max_print_copies, print_watermark FROM exams WHERE id::text = $1::text`,
      [exam_id]
    );
  ...
  if (action === 'print_logs') {
    ...
    const logs = await pool.query(
      `SELECT epl.*, u.name as printed_by_name, e.title_ar as exam_title
       ...
       JOIN users u ON u.id::text = epl.printed_by::text
       JOIN exams e ON e.id::text = epl.exam_id::text
  ...
  if (action === 'emergency_logs') {
    ...
    const logs = await pool.query(
      `SELECT ekl.*, u.name as used_by_name, ek.purpose
       ...
       LEFT JOIN users u ON u.id::text = ekl.used_by::text
       LEFT JOIN emergency_keys ek ON ek.id::text = ekl.emergency_key_id::text
