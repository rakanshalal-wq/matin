...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE scholarships SET student_name=$1, type=$2, amount=$3, percentage=$4, reason=$5, start_date=$6, end_date=$7, status=$8 WHERE id::text = $9::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM scholarships WHERE id::text = $1::text', [id]);
