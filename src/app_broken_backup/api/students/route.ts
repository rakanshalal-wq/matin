  ...
  try {
    ...
    if (user.role === 'super_admin') {
      query = `SELECT s.*, u.name, u.email, u.phone, u.national_id, sc.name as school_name
        ...
        LEFT JOIN schools sc ON sc.id::text = s.school_id::text WHERE 1=1`;
    } else if (user.role === 'owner') {
      query = `SELECT s.*, u.name, u.email, u.phone, u.national_id, sc.name as school_name
        FROM students s LEFT JOIN users u ON u.id::text = s.user_id
        LEFT JOIN schools sc ON sc.id::text = s.school_id::text
        WHERE s.school_id IN (SELECT id FROM schools WHERE owner_id::text = $1::text)`;
      params = [String(user.id)];
    } else {
      query = `SELECT s.*, u.name, u.email, u.phone, u.national_id, sc.name as school_name
        FROM students s LEFT JOIN users u ON u.id::text = s.user_id
        LEFT JOIN schools sc ON sc.id::text = s.school_id::text WHERE s.school_id::text = $1::text`;
...
export async function POST(request: Request) {
  try {
    ...
    const cnt = await pool.query('SELECT COUNT(*) FROM students WHERE school_id::text = $1::text', [schoolId]);
...
export async function PUT(request: Request) {
  try {
    ...
    await pool.query(
      `UPDATE students SET student_id=COALESCE($1,student_id), gender=$2, date_of_birth=$3, class_id=$4, updated_at=NOW() WHERE id::text = $5::text`,
      [student_id||null, gender||null, date_of_birth||null, class_id||null, id]
    );

    const student = await pool.query('SELECT user_id FROM students WHERE id::text = $1::text', [id]);
...
export async function DELETE(request: Request) {
  try {
    ...
    await pool.query('DELETE FROM students WHERE id::text = $1::text', [id]);
