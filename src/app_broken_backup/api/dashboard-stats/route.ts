  ...
  try {
    ...
    if (user.role === 'owner') {
      ...
      const [my_schools, my_students, my_teachers, classes, exams] = await Promise.all([
        ...
        pool.query("SELECT COUNT(*) FROM students WHERE school_id::text = $1::text", [schoolId]),
        pool.query("SELECT COUNT(*) FROM teachers WHERE school_id::text = $1::text", [schoolId]),
        pool.query("SELECT COUNT(*) FROM classes WHERE school_id::text = $1::text", [schoolId]),
        pool.query("SELECT COUNT(*) FROM exams WHERE school_id::text = $1::text AND status IN ('SCHEDULED', 'ONGOING')", [schoolId]),
      ]);
      return NextResponse.json({
    ...
    if (user.role === 'admin') {
      ...
      const [my_students, my_teachers, classes, exams, subjects] = await Promise.all([
        ...
        pool.query("SELECT COUNT(*) FROM classes WHERE school_id::text = $1::text", [schoolId]),
        pool.query("SELECT COUNT(*) FROM exams WHERE school_id::text = $1::text AND status IN ('SCHEDULED', 'ONGOING')", [schoolId]),
        pool.query("SELECT COUNT(*) FROM subjects WHERE school_id::text = $1::text", [schoolId]),
      ]);
      return NextResponse.json({
        my_students: Number(my_students.rows[0].count),
        my_teachers: Number(my_teachers.rows[0].count),
    ...
    if (user.role === 'teacher') {
      ...
      const [courses, students, exams] = await Promise.all([
        pool.query("SELECT COUNT(*) FROM courses WHERE school_id::text = $1::text", [schoolId]),
        pool.query("SELECT COUNT(*) FROM students WHERE school_id::text = $1::text", [schoolId]),
        pool.query("SELECT COUNT(*) FROM exams WHERE school_id::text = $1::text AND status IN ('SCHEDULED', 'ONGOING')", [schoolId]),
      ]);
      return NextResponse.json({
    ...
    if (user.role === 'student') {
      ...
      const exams = await pool.query(
        "SELECT COUNT(*) FROM exams WHERE school_id::text = $1::text AND status IN ('SCHEDULED', 'ONGOING')",
        [schoolId]
      );
      return NextResponse.json({
        gpa: '—',
