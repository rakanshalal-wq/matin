...
export async function GET(request: Request) {
  try {
    ...
    const result = await pool.query(`
      ...
      LEFT JOIN users u ON u.id::text = a.author_id::text
...
export async function POST(request: Request) {
  try {
    ...
    const result = await pool.query(
      ...
      ['ann_' + Date.now(), title, content, target_audience || 'all', priority || 'normal', user.id, String(user.school_id)]
    );
...
export async function DELETE(request: Request) {
  try {
    ...
    await pool.query('DELETE FROM announcements WHERE id::text = $1::text', [id]);
