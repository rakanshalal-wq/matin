...
export async function GET(request: Request) {
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM messages WHERE 1=1 ${filter.sql} ORDER BY created_at DESC`,
      filter.params
    );
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE messages SET title=$1, content=$2, type=$3, priority=$4, status=$5, is_read=$6 WHERE id::text = $7::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM messages WHERE id::text = $1::text', [id]);
