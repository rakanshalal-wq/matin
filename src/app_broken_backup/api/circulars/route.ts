...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE circulars SET title=$1, type=$2, content=$3, sender=$4, target_audience=$5, priority=$6, publish_date=$7, status=$8 WHERE id::text = $9::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM circulars WHERE id::text = $1::text', [id]);
