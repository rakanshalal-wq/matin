...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE surveys SET title=$1, description=$2, target_audience=$3, questions_count=$4, responses_count=$5, start_date=$6, end_date=$7, status=$8 WHERE id::text = $9::text RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM surveys WHERE id::text = $1::text', [id]);
