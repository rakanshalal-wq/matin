...
export async function POST(request: Request) {
  try {
    ...
    const { student_id, student_name, amount, fee_type, description, due_date, status, payment_method } = body;

    if (!student_name || !amount) return NextResponse.json({ error: 'اسم الطالب والمبلغ مطلوبان' }, { status: 400 });
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      `UPDATE student_fees SET student_name=$1, amount=$2, fee_type=$3, description=$4, due_date=$5, status=$6, payment_method=$7 WHERE id::text = $8::text RETURNING *`,
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM student_fees WHERE id::text = $1::text', [id]);
