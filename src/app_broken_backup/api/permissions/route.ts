  ...
  try {
    ...
    if (user.role === "owner" || user.role === "admin") {
      ...
      let query = `
        ...
        FROM users u
        WHERE u.school_id::text = $1::text AND u.role NOT IN ('super_admin', 'owner')
      `;
...
export async function PUT(request: Request) {
  try {
    ...
    if (user.role === "owner") {
      const check = await pool.query("SELECT id FROM users WHERE id = $1::integer AND school_id::text = $2::text", [id, String(user.school_id)]);
