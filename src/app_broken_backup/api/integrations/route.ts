    ...
    if (user.role === 'super_admin') {
      ...
      const result = await pool.query(`
        SELECT 
          ...
          si.connected_at
        FROM integrations i
        LEFT JOIN school_integrations si ON si.provider::text = i.name::text AND si.school_id::text = $1::text
...
export async function PUT(request: Request) {
  try {
    ...
    if (action === 'toggle') {
      const current = await pool.query('SELECT is_active FROM integrations WHERE id::text = $1::text', [id]);
      ...
      const result = await pool.query(
        'UPDATE integrations SET is_active = $1, updated_at = NOW() WHERE id::text = $2::text RETURNING *',
        [newStatus, id]
      );
    ...
    if (action === 'save_config' || !action) {
      ...
      const existing = await pool.query('SELECT * FROM integrations WHERE id::text = $1::text', [id]);
      ...
      const result = await pool.query(
        `UPDATE integrations SET 
          ...
          updated_at = NOW()
        WHERE id::text = $7::text RETURNING *`,
        [
    ...
    // ── اختبار الاتصال ──
    if (action === 'test') {
      const integration = await pool.query('SELECT * FROM integrations WHERE id::text = $1::text', [id]);
...
export async function DELETE(request: Request) {
  try {
    ...
    await pool.query(
      'UPDATE integrations SET extra_config = $1, is_active = false, connected_at = NULL, updated_at = NOW() WHERE id::text = $2::text',
      ['{}', id]
    );
