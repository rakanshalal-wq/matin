  ...
  try {
    ...
    if (certificate_id) {
      const result = await pool.query(`
        ...
        FROM certificates c
        LEFT JOIN students s ON s.id::text = c.student_id::text
  ...
  try {
    ...
    if (action === 'auto_issue') {
      ...
      const grades = await pool.query(
        `SELECT * FROM grades WHERE student_id::text = $1::text ${course_id ? 'AND course_id = $2' : ''}`,
      ...
      const studentInfo = await pool.query(`
        SELECT u.name FROM students s LEFT JOIN users u ON u.id::text = s.user_id WHERE s.id::text = $1::text
      `, [student_id]);
...
export async function DELETE(request: Request) {
  try {
    ...
    await pool.query('DELETE FROM certificates WHERE id::text = $1::text', [id]);
