...
export async function GET(request: Request) {
  try {
    ...
    const result = await pool.query(`
      ...
      FROM colleges c
      LEFT JOIN schools s ON c.school_id::text = s.id::text
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(`
      UPDATE colleges SET ${setClause}, updated_at = NOW() WHERE id::text = $1::text RETURNING *
...
export async function DELETE(request: Request) {
  try {
    ...
    await pool.query('DELETE FROM colleges WHERE id::text = $1::text', [id]);
