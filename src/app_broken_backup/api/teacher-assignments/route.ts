...
export async function GET(request: Request) {
  try {
    ...
    let params: any[] = [];
    if (user.role === 'owner') {
      schoolFilter = 'AND ta.school_id IN (SELECT id FROM schools WHERE owner_id::text = $1::text)';
      params = [String(user.id)];
    } else if (user.role === 'teacher') {
      schoolFilter = 'AND t.user_id::text = $1::text';
      params = [String(user.id)];
    } else if (user.role !== 'super_admin') {
      schoolFilter = 'AND ta.school_id::text = $1::text';
    ...
    const result = await pool.query(`
      ...
      LEFT JOIN teachers t ON t.id::text = ta.teacher_id::text
      LEFT JOIN users u ON u.id::text = t.user_id
      LEFT JOIN subjects s ON s.id::text = ta.subject_id::text
...
export async function DELETE(request: Request) {
  try {
    ...
    await pool.query('DELETE FROM teacher_assignments WHERE id::text = $1::text', [id]);
