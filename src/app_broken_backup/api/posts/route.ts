...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'comment') {
      ...
      await pool.query('UPDATE posts SET comments_count = COALESCE(comments_count, 0) + 1 WHERE id::text = $1::text', [post_id]);
    ...
    if (action === 'like' || action === 'toggle_like') {
      ...
      const existing = await pool.query(
        'SELECT id FROM post_likes WHERE post_id = $1 AND user_id::text = $2::text', [post_id, userId]
      );
      
      if (existing.rows.length > 0) {
        // إلغاء الإعجاب
        await pool.query('DELETE FROM post_likes WHERE post_id = $1 AND user_id::text = $2::text', [post_id, userId]);
        await pool.query('UPDATE posts SET likes_count = GREATEST(COALESCE(likes_count, 0) - 1, 0) WHERE id::text = $1::text', [post_id]);
        ...
        await pool.query('UPDATE posts SET likes_count = COALESCE(likes_count, 0) + 1 WHERE id::text = $1::text', [post_id]);
