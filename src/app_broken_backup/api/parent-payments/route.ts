...
export async function GET(request: Request) {
  try {
    ...
    const type = searchParams.get('type') || (user.role === 'super_admin' ? 'platform_stats' : user.role === 'owner' || user.role === 'admin' ? 'school_invoices' : 'invoices');
    ...
    if (user.role === 'parent' && type === 'invoices') {
      const result = await pool.query(`
        ...
        LEFT JOIN students s ON s.id::text = ui.student_id::text
        LEFT JOIN schools sc ON sc.id::text = ui.school_id::text
        LEFT JOIN unified_invoice_items uii ON uii.invoice_id::text = ui.id::text
      ...
      `, [user.id]);
      await pool.query('UPDATE unified_invoices SET viewed_at = NOW() WHERE parent_id::text = $1::text AND viewed_at IS NULL', [user.id]);
      const summary = await pool.query(`
        ...
        FROM unified_invoices WHERE parent_id::text = $1::text
      `, [user.id]);
    ...
    if (user.role === 'parent' && type === 'history') {
      const result = await pool.query(`
        ...
        JOIN unified_invoices ui ON ui.id::text = up.invoice_id::text
        LEFT JOIN students s ON s.id::text = ui.student_id::text
    ...
    if (['owner', 'admin'].includes(user.role) && type === 'school_invoices') {
      ...
      let query = `SELECT ui.*, s.name_ar as student_name, p.name as parent_name, p.phone as parent_phone
        FROM unified_invoices ui LEFT JOIN students s ON s.id::text = ui.student_id::text LEFT JOIN parents p ON p.id::text = ui.parent_id::text
        WHERE ui.school_id::text = $1::text`;
      const params: any[] = [String(user.school_id)];
      ...
      const stats = await pool.query(`
        ...
        FROM unified_invoices WHERE school_id::text = $1::text
      `, [user.school_id]);
    ...
    if (user.role === 'super_admin' && type === 'platform_stats') {
      const result = await pool.query(`
        ...
        FROM schools sc LEFT JOIN unified_invoices ui ON ui.school_id::text = sc.id::text
    ...
    if (type === 'gateways') {
      ...
      const settings = await pool.query('SELECT * FROM school_payment_settings WHERE school_id::text = $1::text', [sid]);
...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'create_invoice') {
      ...
      const settings = await pool.query('SELECT * FROM school_payment_settings WHERE school_id::text = $1::text', [user.school_id]);
    ...
    if (action === 'confirm_cash_payment') {
      ...
      const inv = await pool.query('SELECT * FROM unified_invoices WHERE id::text = $1::text AND school_id::text = $2::text', [invoice_id, String(user.school_id)]);
      ...
      await pool.query('UPDATE unified_invoices SET status=$1, paid_at=NOW(), payment_method=\'cash\' WHERE id::text = $2::text', [newStatus, invoice_id]);
      await pool.query(
        ...
        [`PAY-${Date.now()}`, invoice_id, user.school_id, user.owner_id || user.id, paidAmount, String(user.id)]
      );
    ...
    if (action === 'initiate_payment') {
      ...
      const inv = await pool.query('SELECT ui.*, sc.name_ar as school_name FROM unified_invoices ui JOIN schools sc ON sc.id::text = ui.school_id::text WHERE ui.id::text = $1::text AND ui.parent_id=$2', [invoice_id, String(user.id)]);
      if (!inv.rows[0]) return NextResponse.json({ error: 'الفاتورة غير موجودة' }, { status: 404 });
      ...
      if (['cash', 'bank_transfer'].includes(payment_method)) {
        await pool.query('UPDATE unified_invoices SET payment_method=$1 WHERE id::text = $2::text', [payment_method, invoice_id]);
