...
export async function POST(request: Request) {
  try {
    ...
    if (user.status === 'pending') {
      return NextResponse.json({ success: false, message: 'حسابك قيد المراجعة. انتظر الموافقة' }, { status: 403 });
    }
    ...
    if (user.password.startsWith('$2b$') || user.password.startsWith('$2a$') ) {
      ...
      if (validPassword) {
        ...
        await pool.query('UPDATE users SET password = $1 WHERE id::text = $2::text', [hashed, String(user.id)]);
      }
    }
    ...
    if (otpEnabled) {
      ...
      await pool.query('DELETE FROM otp_codes WHERE user_id::text = $1::text', [user.id]);

      // إدخال الكود الجديد
      await pool.query(
        'INSERT INTO otp_codes (user_id, code, expires_at) VALUES ($1, $2, $3)',
        [String(user.id), otpCode, expiresAt]
      );
