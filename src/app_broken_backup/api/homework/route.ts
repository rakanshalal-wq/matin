  ...
  try {
    ...
    if (homework_id && searchParams.get('submissions') === 'true') {
      const result = await pool.query(`
        ...
        LEFT JOIN students s ON s.id::text = hs.student_id::text
    ...
    if (student_id) {
      const result = await pool.query(`
        ...
        FROM homework h
        LEFT JOIN homework_submissions hs ON hs.homework_id::text = h.id::text AND hs.student_id = $1
  ...
  try {
    ...
    if (action === 'grade') {
      ...
      if (result.rows[0]) {
        ...
        try {
          const hw = await pool.query('SELECT * FROM homework WHERE id::text = $1::text', [sub.homework_id]);
...
export async function DELETE(request: Request) {
  try {
    ...
    await pool.query('DELETE FROM homework WHERE id::text = $1::text', [id]);
