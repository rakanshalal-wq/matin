...
export async function POST(request: Request) {
  try {
    ...
    if (finalRole === 'teacher') {
      const cnt = await pool.query('SELECT COUNT(*) FROM teachers WHERE school_id::text = $1::text', [school.id]);
    ...
    if (finalRole === 'student') {
      const cnt = await pool.query('SELECT COUNT(*) FROM students WHERE school_id::text = $1::text', [school.id]);
