...
export default function Page() {
  ...
  const fetchItems = async () => { try { const res = await fetch('/api/contracts', { headers: getHeaders() }); const data = await res.json(); setItems(Array.isArray(data) ? data : []); } catch (e) { console.error(e); } finally { setLoading(false); } };

  const handleAdd = async () => {
    if (!formData.title) return alert('أدخل البيانات المطلوبة');
    setSaving(true);
    try {
      const res = await fetch('/api/contracts', { method: 'POST', headers: getHeaders(), body: JSON.stringify(formData) });
  ...
  const handleDelete = async (id: any) => {
    ...
    try { await fetch(`/api/contracts?id=${id}`, { method: 'DELETE', headers: getHeaders() }); fetchItems(); } catch (e) { console.error(e); }
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>📑 العقود والوثائق</h1>
          <p style={{ color: 'rgba(255,255,255,0.6)', marginTop: 8 }}>إدارة العقود والوثائق</p>
        </div>
