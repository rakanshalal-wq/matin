...
export default function Page() {
  ...
  const fetchItems = async () => { try { const res = await fetch('/api/backup', { headers: getHeaders() }); const data = await res.json(); setItems(Array.isArray(data) ? data : []); } catch (e) { console.error(e); } finally { setLoading(false); } };

  const handleAdd = async () => {
    if (!formData.title) return alert('أدخل البيانات المطلوبة');
    setSaving(true);
    try {
      const res = await fetch('/api/backup', { method: 'POST', headers: getHeaders(), body: JSON.stringify(formData) });
  ...
  const handleDelete = async (id: any) => {
    ...
    try { await fetch(`/api/backup?id=${id}`, { method: 'DELETE', headers: getHeaders() }); fetchItems(); } catch (e) { console.error(e); }
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>💾 النسخ الاحتياطي</h1>
          <p style={{ color: 'rgba(255,255,255,0.6)', marginTop: 8 }}>إدارة النسخ الاحتياطي</p>
        </div>
