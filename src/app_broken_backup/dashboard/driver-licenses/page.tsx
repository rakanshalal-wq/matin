...
export default function DriverLicensesPage() {
  ...
  const handleAdd = async () => {
    if (!formData.driver_name) return alert('أدخل اسم السائق');
    setSaving(true);
    try {
      const res = await fetch('/api/driver-licenses', { method: 'POST', headers: getHeaders(), body: JSON.stringify(formData) });
      if (res.ok) { setShowAddModal(false); setFormData({ driver_name: '', license_number: '', type: '', issue_date: '', expiry_date: '', status: 'active' }); fetchItems(); }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button onClick={() => setShowAddModal(true)} style={{ background: 'linear-gradient(135deg, #C9A227 0%, #D4B03D 100%)', color: '#0D1B2A', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer' }}>➕ إضافة رخصة</button>
      </div>

      {expiredCount > 0 && (
        <div style={{ background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <p style={{ color: '#EF4444', margin: 0, fontSize: 14 }}>🚨 تنبيه: يوجد {expiredCount} رخصة منتهية الصلاحية!</p>
        </div>
      )}
      {expiringSoonCount > 0 && (
        <div style={{ background: 'rgba(245,158,11,0.05)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <p style={{ color: '#F59E0B', margin: 0, fontSize: 14 }}>⚠️ تنبيه: يوجد {expiringSoonCount} رخصة تنتهي خلال 30 يوم!</p>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          ...
          { label: 'سارية', value: items.filter(i => i.status === 'active' && !isExpired(i.expiry_date)).length, icon: '✅', color: '#10B981' },
          { label: 'قاربت على الانتهاء', value: expiringSoonCount, icon: '⚠️', color: '#F59E0B' },
          { label: 'منتهية', value: expiredCount, icon: '🚨', color: '#EF4444' },
        ].map((stat, i) => (
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center' }}>
            ...
            <button onClick={() => setShowAddModal(true)} style={{ marginTop: 16, background: 'linear-gradient(135deg, #C9A227 0%, #D4B03D 100%)', color: '#0D1B2A', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer' }}>➕ إضافة أول رخصة</button>
          </div>
        ) : (
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#0D1B2A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 22, fontWeight: 700, margin: 0 }}>🪪 إضافة رخصة جديدة</h2>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24, justifyContent: 'flex-start' }}>
              <button onClick={handleAdd} disabled={saving} style={{ background: 'linear-gradient(135deg, #C9A227 0%, #D4B03D 100%)', color: '#0D1B2A', padding: '12px 32px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? '⏳ جاري الحفظ...' : '💾 حفظ الرخصة'}</button>
