...
export default function LiveStreamPage() {
  ...
  const platformIcons: any = { zoom: '📹', teams: '👥', meet: '🟢', youtube: '▶️', custom: '🖥️' };
    ...
    <div>
      ...
      {stats.live > 0 && (
        <div style={{ background: 'rgba(16,185,129,0.15)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 12, padding: '16px 20px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ color: '#10B981', fontWeight: 700, fontSize: 15 }}>🔴 يوجد {stats.live} بث مباشر الآن!</div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>📡 البث المباشر</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي البثوث', value: stats.total, icon: '📡', color: '#C9A227' },
          { label: 'مباشر الآن', value: stats.live, icon: '🔴', color: '#10B981' },
          { label: 'مجدول', value: stats.scheduled, icon: '📅', color: '#3B82F6' },
          { label: 'إجمالي المشاهدات', value: stats.totalViewers, icon: '👁️', color: '#8B5CF6' },
        ].map((stat, i) => (
                ...
                <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: item.status === 'live' ? 'rgba(16,185,129,0.05)' : 'transparent' }}>
                  <td style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: platformColors[item.platform]?.bg || 'rgba(59,130,246,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{platformIcons[item.platform] || '📡'}</div>
                      <div>
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    <span style={{ background: statusColors[item.status]?.bg || 'rgba(107,114,128,0.1)', color: statusColors[item.status]?.color || '#6B7280', padding: '4px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600 }}>
                      {item.status === 'live' && '🔴 '}{statusLabels[item.status] || item.status}
                    </span>
                  </td>
                  <td style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      {item.link && item.status === 'live' && (
                        <button onClick={() => window.open(item.link, '_blank')} style={{ background: 'rgba(16,185,129,0.15)', color: '#10B981', border: 'none', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>🔴 انضم</button>
                      )}
                      {item.link && item.status !== 'live' && (
                        <button onClick={() => window.open(item.link, '_blank')} style={{ background: 'rgba(59,130,246,0.1)', color: '#3B82F6', border: 'none', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>🔗 رابط</button>
                      )}
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#0D1B2A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? '✏️ تعديل بث' : '➕ جدولة بث جديد'}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                <select value={form.platform} onChange={e => setForm({ ...form, platform: e.target.value })} style={inputStyle}>
                  ...
                  <option value="custom">منصة خاصة 🖥️</option>
                </select>
              </div>
              <div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24, justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleSubmit} style={{ background: 'linear-gradient(135deg, #C9A227 0%, #D4B03D 100%)', color: '#0D1B2A', border: 'none', borderRadius: 10, padding: '12px 24px', cursor: 'pointer', fontWeight: 700 }}>{editItem ? '💾 تحديث' : '➕ جدولة'}</button>
            </div>
          </div>
        </div>
      )}
