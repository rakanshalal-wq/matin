  ...
  try {
    ...
    if (user.role === 'super_admin' && type === 'platform') {
      const result = await pool.query(`
        ...
        LEFT JOIN institution_services ist ON ist.service_key = ps.key
    ...
    if (user.role === 'super_admin' && type === 'school' && school_id) {
      const result = await pool.query(`
        ...
        FROM platform_services ps
        LEFT JOIN institution_services ist ON ist.service_key = ps.key AND ist.school_id = $1
    ...
    if (type === 'check') {
      ...
      const result = await pool.query(
        'SELECT is_enabled FROM institution_services WHERE school_id = $1 AND service_key = $2',
        [sid, key]
      );
...
export async function POST(request: Request) {
  try {
    ...
    if (user.role === 'owner' && is_enabled) {
      const school = await pool.query('SELECT plan FROM schools WHERE id = $1', [target_school]);
    ...
    const oldState = await pool.query(
      'SELECT * FROM institution_services WHERE school_id = $1 AND service_key = $2',
    ...
    for (const svc of services) {
      await pool.query(`
        ...
        ON CONFLICT (school_id, service_key) DO UPDATE SET
          ...
          disabled_at = CASE WHEN NOT EXCLUDED.is_enabled THEN NOW() ELSE NULL END,
          updated_at = NOW()
      `, [school_id, svc.key, svc.enabled, user.id]);
    }
    await pool.query(
