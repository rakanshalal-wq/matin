...
export async function GET(request: Request) {
  try {
    ...
    const result = await pool.query(
      "SELECT * FROM school_appearance WHERE school_id = $1",
      [user.school_id]
    );
...
export async function POST(request: Request) {
  try {
    ...
    await pool.query(
      ...
      [user.school_id, primary_color, secondary_color, background_color, logo_url, hero_image_url, school_name, school_slogan, template, font, show_ads, show_store_button]
    );
    ...
    await pool.query(
      `INSERT INTO school_appearance (school_id, primary_color, secondary_color, background_color, logo_url, hero_image_url, school_name, school_slogan, template, font, show_ads, show_store_button, updated_at)
       ...
       ON CONFLICT (school_id) DO UPDATE SET
         primary_color=$2, secondary_color=$3, background_color=$4, logo_url=$5, hero_image_url=$6,
         school_name=$7, school_slogan=$8, template=$9, font=$10, show_ads=$11, show_store_button=$12, updated_at=NOW()`,
      [user.school_id, primary_color, secondary_color, background_color, logo_url, hero_image_url, school_name, school_slogan, template, font, show_ads ?? true, show_store_button ?? true]
    );
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('PUT appearance error:', error);
    return NextResponse.json({ error: 'خطأ في تحديث مظهر المدرسة' }, { status: 500 });
  }
}
