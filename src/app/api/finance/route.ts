...
export async function GET(request: Request) {
  try {
    ...
    if (type === 'all_payments') {
      ...
      const allPay = await pool.query(`SELECT p.*, u.name as user_name FROM payments p LEFT JOIN users u ON u.id = p.user_id ORDER BY p.created_at DESC LIMIT 200`);
...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'confirm_payment') {
      ...
      // تحديث الفاتورة
      await pool.query(`UPDATE payments SET status = 'paid', paid_at = NOW() WHERE id = $1`, [payment_id]);
      ...
      // تحديث باقة المستخدم
      await pool.query(`UPDATE users SET package = $1 WHERE id = $2`, [pkg, p.user_id]);
