...
export async function GET(request: Request) {
  try {
    ...
    const [countResult, dataResult] = await Promise.all([
      ...
      pool.query(`SELECT * FROM audit_logs ORDER BY created_at DESC LIMIT $1 OFFSET $2`, [limit, offset])
    ]);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO audit_logs (title, description, status, created_at) VALUES ($1,$2,$3,NOW()) RETURNING *',
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      `UPDATE audit_logs SET title = COALESCE($1, title), description = COALESCE($2, description), status = COALESCE($3, status), updated_at = NOW() WHERE id = $4 RETURNING *`,
