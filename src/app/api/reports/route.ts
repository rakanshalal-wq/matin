  ...
  try {
    ...
    if (type === 'student') {
      ...
      const [info, grades, attendance, exams, certificates] = await Promise.all([
        pool.query(`SELECT s.*, u.name, u.email FROM students s LEFT JOIN users u ON u.id::text = s.user_id WHERE s.id = $1`, [student_id]),
        pool.query(`SELECT * FROM grades WHERE student_id = $1 ORDER BY created_at DESC`, [student_id]),
        pool.query(`SELECT status, COUNT(*) as count FROM attendance WHERE student_id = $1 GROUP BY status`, [student_id]),
        pool.query(`SELECT es.*, e.title_ar FROM exam_sessions es LEFT JOIN exams e ON e.id = es.exam_id WHERE es.student_id = $1 ORDER BY es.started_at DESC`, [student_id]),
        pool.query(`SELECT * FROM certificates WHERE student_id = $1`, [student_id]),
      ]);
