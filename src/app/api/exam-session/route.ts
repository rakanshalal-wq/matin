...
export async function GET(request: Request) {
  try {
    ...
    if (session_id) {
      const session = await pool.query(`SELECT * FROM exam_sessions WHERE id = $1`, [session_id]);
...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'violation') {
      ...
      await pool.query(`UPDATE exam_sessions SET ${updateField} = ${updateField} + 1 WHERE id = $1`, [session_id]);
      ...
      if (s) {
        ...
        if (totalViolations >= 5) {
          await pool.query(`UPDATE exam_sessions SET status = 'terminated', finished_at = NOW() WHERE id = $1`, [session_id]);
    ...
    if (action === 'finish') {
      ...
      await pool.query(
        `UPDATE exam_sessions SET status = 'completed', finished_at = NOW() WHERE id = $1`,
        [session_id]
      );
      ...
      await pool.query(
        `UPDATE exam_sessions SET score = $1, total_marks = $2, percentage = $3, grade = $4 WHERE id = $5`,
      ...
      try {
        const sessionInfo = await pool.query(
          `SELECT es.student_id, es.exam_id, e.school_id, e.owner_id, e.subject_id as course_id
           FROM exam_sessions es LEFT JOIN exams e ON e.id = es.exam_id
