  ...
  try {
    ...
    if (user.role === 'owner' && (role === 'teacher' || !role)) {
      const teacherCount = await pool.query(
        "SELECT COUNT(*) FROM users WHERE owner_id = $1 AND role = 'teacher'", [user.id]
      );
...
export async function PUT(request: Request) {
  try {
    ...
    // تحقق من الملكية
    if (user.role === 'owner') {
      const check = await pool.query('SELECT id FROM users WHERE id = $1 AND owner_id = $2', [id, user.id]);
      if (check.rows.length === 0) return NextResponse.json({ error: 'غير مصرح' }, { status: 403 });
    }

    const result = await pool.query(
      'UPDATE users SET name=$1, phone=$2, role=$3, status=$4 WHERE id=$5 RETURNING id, name, email, phone, role, status',
...
export async function DELETE(request: Request) {
  try {
    ...
    const target = await pool.query('SELECT role, owner_id FROM users WHERE id = $1', [id]);
