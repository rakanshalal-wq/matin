...
export async function GET(request: Request) {
  try {
    ...
    if (school_id) {
      const result = await pool.query('SELECT * FROM school_pages WHERE school_id = $1', [school_id]);
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      `UPDATE school_pages SET school_name=$1, logo=$2, cover_image=$3, description=$4, vision=$5, mission=$6, phone=$7, email=$8, address=$9, social_twitter=$10, social_instagram=$11, social_snapchat=$12, is_published=$13, updated_at=NOW() WHERE id=$14 RETURNING *`,
