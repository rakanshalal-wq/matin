...
export async function GET(request: Request) {
  try {
    ...
    const result = await pool.query('SELECT * FROM vaccinations_table ORDER BY created_at DESC LIMIT 200');
...
export async function POST(request: Request) {
  try {
    ...
    if (!student_name || !vaccine_name) return NextResponse.json({ error: 'اسم الطالب واسم التطعيم مطلوبين' }, { status: 400 });
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      'UPDATE vaccinations_table SET student_name=$1, vaccine_name=$2, dose_number=$3, date=$4, next_dose_date=$5, administered_by=$6, status=$7 WHERE id=$8 RETURNING *',
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'المعرف مطلوب' }, { status: 400 });
    await pool.query('DELETE FROM vaccinations_table WHERE id = $1', [id]);
