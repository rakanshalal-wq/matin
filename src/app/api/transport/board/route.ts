    ...
    try {
      const parentResult = await pool.query(
        `SELECT pcm.parent_id, u.name as parent_name, u.phone 
         ...
         JOIN users u ON pcm.parent_id = u.id 
         WHERE pcm.child_id = $1`,
        [student_id]
      );
      
      if (parentResult.rows.length > 0) {
        const studentResult = await pool.query('SELECT name FROM users WHERE id = $1', [student_id]);
        ...
        for (const parent of parentResult.rows) {
          await pool.query(
            ...
            [parent.parent_id, 'إشعار النقل', `${studentName} ${action === 'boarded' ? 'ركب الباص' : 'نزل من الباص'}`, user.school_id]
          );
        }
      }
    } catch (notifError) {
