...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'register_student') {
      ...
      let owner_id = null;
      if (body.school_id) {
        const school = await pool.query('SELECT owner_id FROM schools WHERE id = $1', [body.school_id]);
    ...
    if (action === 'toggle_status') {
      ...
      if (!user_id) return NextResponse.json({ error: 'معرف المستخدم مطلوب' }, { status: 400 });
      await pool.query('UPDATE users SET status = $1 WHERE id = $2', [new_status || 'suspended', user_id]);
      return NextResponse.json({ success: true });
    }
