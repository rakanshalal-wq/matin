...
export async function POST(request: Request) {
  try {
    ...
    const result = await pool.query(
      ...
      [title, description, image_url, click_url, is_platform_ad, position || "top", target_type || "all", start_date, end_date, priority || 1, user.id, user.school_id]
    );
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await pool.query(
      "UPDATE ads SET is_active = COALESCE($1, is_active), title = COALESCE($2, title), description = COALESCE($3, description) WHERE id = $4 RETURNING *",
