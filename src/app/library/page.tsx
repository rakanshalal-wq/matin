    ...
    <div style={{ background: '#06060E', minHeight: '100vh', color: '#EEEEF5', fontFamily: "'IBM Plex Sans Arabic', 'Tajawal', sans-serif", direction: 'rtl', overflowX: 'hidden' }}>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .hero-glow { position:absolute; top:-200px; left:50%; transform:translateX(-50%); width:1100px; height:700px; background:radial-gradient(ellipse, rgba(168,85,247,0.12) 0%, rgba(201,168,76,0.08) 40%, transparent 70%); pointer-events:none; z-index:0; }
        .hero-badge { display:inline-flex; align-items:center; gap:8px; background:rgba(168,85,247,0.08); border:1px solid rgba(168,85,247,0.2); color:#A855F7; padding:6px 16px; border-radius:100px; font-size:12.5px; font-weight:600; margin-bottom:40px; }
        .badge-dot { width:7px; height:7px; border-radius:50%; background:#A855F7; animation:pulse 2s infinite; }
      ...
      <section className="hero">
        ...
        <div style={{ position:'relative', zIndex:1 }}>
          <div className="hero-badge"><span className="badge-dot"></span>المكتبة الرقمية</div>
          <h1>مكتبة رقمية شاملة<span className="gold">لكل مؤسستك التعليمية</span></h1>
          <p>نظام مكتبة رقمي متكامل — استعارة إلكترونية، كتالوج رقمي، بحث متقدم، وإدارة كاملة للمصادر التعليمية.</p>
        </div>
      </section>
      <div className="divider"></div>
      <section className="section">
        <div className="section-label">المميزات الكاملة</div>
        <div className="section-title">إدارة شاملة للمكتبة</div>
        <div className="section-desc">كل ما تحتاجه لإدارة مكتبة مدرسية رقمية احترافية.</div>
        <div className="features-grid">
          {[
            { icon: '📚', title: 'كتالوج رقمي شامل', desc: 'فهرسة كاملة لجميع الكتب والمراجع مع بيانات تفصيلية — المؤلف، الناشر، الموضوع، والتصنيف.', color: 'rgba(168,85,247,0.12)' },
            { icon: '🔍', title: 'بحث متقدم', desc: 'بحث سريع بالعنوان، المؤلف، الموضوع، أو الكلمات المفتاحية مع نتائج فورية.', color: 'rgba(168,85,247,0.12)' },
            { icon: '📖', title: 'استعارة إلكترونية', desc: 'الطالب يطلب استعارة الكتاب من التطبيق — الأمين يوافق ويسجل الاستعارة رقمياً.', color: 'rgba(201,168,76,0.12)' },
            { icon: '⏰', title: 'تذكيرات الإرجاع', desc: 'إشعارات تلقائية للطلاب قبل انتهاء مدة الاستعارة لتجنب الغرامات.', color: 'rgba(201,168,76,0.12)' },
            { icon: '📱', title: 'كتب إلكترونية', desc: 'رفع الكتب الإلكترونية PDF وإتاحتها للطلاب للقراءة من أي جهاز.', color: 'rgba(201,168,76,0.12)' },
            { icon: '📊', title: 'تقارير الاستخدام', desc: 'إحصائيات الكتب الأكثر طلباً، معدلات الاستعارة، والطلاب الأكثر قراءة.', color: 'rgba(201,168,76,0.12)' },
          ].map((f, i) => (
      ...
      <section className="cta-section">
        <div className="cta-box">
          ...
          <h2 style={{ fontSize:'32px', fontWeight:900, marginBottom:'16px' }}>حوّل مكتبتك إلى مركز معرفة رقمي</h2>
          <p style={{ fontSize:'15px', color:'var(--text-2)', lineHeight:1.7 }}>متين يحول المكتبة التقليدية إلى منصة رقمية حديثة تشجع الطلاب على القراءة والبحث.</p>
