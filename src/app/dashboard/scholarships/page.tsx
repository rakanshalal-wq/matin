...
export default function ScholarshipsPage() {
  ...
  const typeIcons: any = { full_scholarship: '🎓', partial_scholarship: '📘', discount: '🏷️', sibling_discount: '👨‍👩‍👧‍👦', employee_discount: '👔', excellence: '⭐', need_based: '🤲', sports: '⚽' };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>🎓 المنح والخصومات</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي المنح', value: stats.total.toString(), icon: '🎓', color: '#C9A227', suffix: '' },
          { label: 'نشطة', value: stats.active.toString(), icon: '✅', color: '#10B981', suffix: '' },
          { label: 'منتهية', value: stats.expired.toString(), icon: '📁', color: '#6B7280', suffix: '' },
          { label: 'إجمالي المبالغ', value: formatMoney(stats.totalAmount), icon: '💰', color: '#3B82F6', suffix: ' ر.س' },
        ].map((stat, i) => (
                ...
                <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: isExpired(item.end_date) ? 'rgba(107,114,128,0.03)' : 'transparent' }}>
                  <td style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: typeColors[item.type]?.bg || 'rgba(201,162,39,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{typeIcons[item.type] || '🎓'}</div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#0D1B2A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? '✏️ تعديل منحة' : '➕ إضافة منحة/خصم'}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} style={inputStyle}>
                  <option value="full_scholarship">منحة كاملة 🎓</option>
                  <option value="partial_scholarship">منحة جزئية 📘</option>
                  <option value="discount">خصم 🏷️</option>
                  <option value="sibling_discount">خصم أخوة 👨‍👩‍👧‍👦</option>
                  <option value="employee_discount">خصم موظفين 👔</option>
                  <option value="excellence">تميز أكاديمي ⭐</option>
                  <option value="need_based">حاجة مادية 🤲</option>
                  <option value="sports">منحة رياضية ⚽</option>
                </select>
              </div>
              <div>
