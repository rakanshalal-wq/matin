...
const qualityConfig: Record<string, { color: string; bg: string; icon: string; desc: string }> = {
  'ممتاز':      { color: '#10B981', bg: 'rgba(16,185,129,0.15)',  icon: '🌟', desc: 'سؤال مميز - معدل إجابة صحيحة مثالي' },
  'جيد - سهل': { color: '#3B82F6', bg: 'rgba(59,130,246,0.15)',  icon: '✅', desc: 'سؤال جيد - مناسب للمراجعة' },
  'جيد - صعب': { color: '#8B5CF6', bg: 'rgba(139,92,246,0.15)', icon: '💪', desc: 'سؤال تحدي - مناسب للطلاب المتقدمين' },
  'سهل جداً':  { color: '#F59E0B', bg: 'rgba(245,158,11,0.15)', icon: '⚠️', desc: 'سهل جداً - لا يميز بين الطلاب' },
  'صعب جداً':  { color: '#EF4444', bg: 'rgba(239,68,68,0.15)',  icon: '🔴', desc: 'صعب جداً - قد يكون مربكاً أو غامضاً' },
...
export default function QuestionAnalyticsPage() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMsg(`✅ تم تحليل ${data.analyzed} سؤال بنجاح`);
        loadData();
      } else {
        setMsg('❌ ' + (data.error || 'فشل التحليل'));
      }
    } catch (e) {
    ...
    <div>
      ...
      <div style={{ marginBottom: 24, background: 'linear-gradient(135deg, rgba(139,92,246,0.1), rgba(59,130,246,0.1))', border: '1px solid rgba(139,92,246,0.2)', borderRadius: 16, padding: '20px 24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: 'white', margin: 0 }}>🧠 تحليل جودة الأسئلة</h1>
          ...
          {user?.role === 'super_admin' && (
            ...
            >
              {analyzing ? '⏳ جاري التحليل...' : '🔄 إعادة تحليل جميع الأسئلة'}
            </button>
          )}
        </div>
        {msg && (
          <div style={{ marginTop: 12, padding: '8px 14px', background: msg.startsWith('✅') ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', borderRadius: 8, color: msg.startsWith('✅') ? '#10B981' : '#EF4444', fontSize: 14 }}>
            {msg}
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'إجمالي الأسئلة', value: stats.total || 0, color: '#3B82F6', icon: '📦' },
          { label: 'تم تحليلها', value: stats.analyzed || 0, color: '#8B5CF6', icon: '🤖' },
          { label: 'أسئلة ممتازة', value: stats.excellent || 0, color: '#10B981', icon: '🌟' },
          { label: 'سهلة جداً', value: stats.veryEasy || 0, color: '#F59E0B', icon: '⚠️' },
          { label: 'صعبة جداً', value: stats.veryHard || 0, color: '#EF4444', icon: '🔴' },
        ].map((s, i) => (
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '16px 20px', marginBottom: 24 }}>
        <h3 style={{ color: 'white', fontSize: 14, fontWeight: 700, margin: '0 0 12px' }}>📊 مستويات جودة الأسئلة</h3>
