...
export default function Page(){
 ...
 <div><h1 style={{fontSize:28,fontWeight:800,color:'white',margin:0}}><IconRenderer name="ICON_Zap" size={18} /> إدارة الوقود</h1><p style={{color:'rgba(255,255,255,0.5)',marginTop:6,fontSize:14}}>تتبع استهلاك الوقود لأسطول النقل المدرسي</p></div>
 ...
 <div key={'إجمالي السجلات'} style={{background:CB,border:'1px solid '+BR,borderRadius:14,padding:'18px 20px'}}><div style={{fontSize:26,fontWeight:800,color:'#C9A84C'}}>{loading?'...':items.length}</div><div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:4}}>إجمالي السجلات</div></div><div key={'إجمالي اللترات'} style={{background:CB,border:'1px solid '+BR,borderRadius:14,padding:'18px 20px'}}><div style={{fontSize:26,fontWeight:800,color:'#3B82F6'}}>{loading?'...':items.reduce((a:number,r:any)=>a+(+r.liters||0),0).toFixed(0)}</div><div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:4}}>إجمالي اللترات</div></div><div key={'التكلفة الشهرية'} style={{background:CB,border:'1px solid '+BR,borderRadius:14,padding:'18px 20px'}}><div style={{fontSize:26,fontWeight:800,color:'#EF4444'}}>{loading?'...':items.reduce((a:number,r:any)=>a+(+r.cost||0),0).toLocaleString()}</div><div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:4}}>التكلفة الشهرية</div></div><div key={'متوسط الاستهلاك'} style={{background:CB,border:'1px solid '+BR,borderRadius:14,padding:'18px 20px'}}><div style={{fontSize:26,fontWeight:800,color:'#10B981'}}>{loading?'...':items.length?(items.reduce((a:number,r:any)=>a+(+r.liters||0),0)/items.length).toFixed(1):'0'}</div><div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:4}}>متوسط الاستهلاك</div></div>
 </div>
