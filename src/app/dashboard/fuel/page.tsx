...
export default function Page() {
  ...
  useEffect(() => { fetchData(); }, []);
  const fetchData = async () => { setLoading(true); try { const r = await fetch('/api/fuel', { headers: getH() }); const d = await r.json(); setItems(Array.isArray(d) ? d : (d.items || d.data || [])); } catch { setItems([]); } finally { setLoading(false); } };
  const handleSave = async () => { setSaving(true); try { const m = editItem ? 'PUT' : 'POST'; const u = editItem ? '/api/fuel?id=' + editItem.id : '/api/fuel'; const r = await fetch(u, { method: m, headers: getH(), body: JSON.stringify(form) }); if (r.ok) { setShowModal(false); setEditItem(null); setForm({}); fetchData(); } } catch { } finally { setSaving(false); } };
  const handleDelete = async (id: number) => { if (!confirm('تأكيد الحذف؟')) return; try { await fetch('/api/fuel?id=' + id, { method: 'DELETE', headers: getH() }); fetchData(); } catch { } };
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, padding: '32px 24px', direction: 'rtl', fontFamily: 'Cairo, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
        <div><h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>⛽ إدارة الوقود</h1><p style={{ color: 'rgba(255,255,255,0.5)', marginTop: 6, fontSize: 14 }}>تتبع استهلاك الوقود لأسطول النقل المدرسي</p></div>
      ...
      <div style={{ background: CB, border: '1px solid ' + BR, borderRadius: 16, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr style={{ borderBottom: '1px solid ' + BR }}>{["المركبة","الكمية (لتر)","التكلفة","التاريخ","العداد","إجراءات"].map(h => <th key={h} style={{ padding: "14px 16px", textAlign: "right", color: "rgba(255,255,255,0.5)", fontSize: 13, fontWeight: 600 }}>{h}</th>)}</tr></thead>
            <tbody>
      ...
      {showModal && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
        <div style={{ background: '#12121F', border: '1px solid ' + BR, borderRadius: 20, padding: 32, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}>
          ...
          <div style={{ display: "grid", gap: 16 }}><div><label style={lbl}>رقم المركبة *</label><input value={form.vehicle_number || ""} onChange={e => setForm({ ...form, vehicle_number: e.target.value })} style={inp} /></div><div><label style={lbl}>الكمية (لتر)</label><input type="number" value={form.quantity || ""} onChange={e => setForm({ ...form, quantity: e.target.value })} style={inp} /></div><div><label style={lbl}>التكلفة (ريال)</label><input type="number" value={form.cost || ""} onChange={e => setForm({ ...form, cost: e.target.value })} style={inp} /></div><div><label style={lbl}>التاريخ</label><input type="date" value={form.fuel_date || ""} onChange={e => setForm({ ...form, fuel_date: e.target.value })} style={inp} /></div><div><label style={lbl}>قراءة العداد (كم)</label><input type="number" value={form.odometer || ""} onChange={e => setForm({ ...form, odometer: e.target.value })} style={inp} /></div></div>
