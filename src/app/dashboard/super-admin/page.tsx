...
export default function SuperAdminDashboard() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) { setShowModal(false); setEditItem(null); setForm({ name: '', email: '', phone: '', plan: 'basic' }); fetchAll(); setMsg(method === 'PUT' ? '✅ تم تعديل المؤسسة' : '✅ تم إضافة المؤسسة'); }
  ...
  return (
    <div style={{ padding: 24, direction: 'rtl', fontFamily: 'IBM Plex Sans Arabic, sans-serif', background: '#06060E', minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ color: '#C9A227', fontSize: 26, fontWeight: 800, margin: 0 }}>🏛 لوحة السوبر أدمن</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 16, marginBottom: 28 }}>
        {[
          { label: 'المؤسسات', value: stats.schools || schools.length || 0, color: '#3B82F6', icon: '🏫' },
          { label: 'الطلاب', value: stats.students || stats.total_students || 0, color: '#10B981', icon: '🎓' },
          { label: 'المعلمون', value: stats.teachers || stats.total_teachers || 0, color: '#8B5CF6', icon: '👩‍🏫' },
          { label: 'الاشتراكات النشطة', value: stats.active_subscriptions || 0, color: '#C9A227', icon: '💎' },
        ].map((s, i) => (
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ color: '#fff', fontSize: 16, fontWeight: 700, margin: 0 }}>🏫 المؤسسات المسجلة ({schools.length})</h2>
        </div>
        {schools.length === 0 ? (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#0F0F1A', border: '1px solid rgba(201,162,39,0.2)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 480, direction: 'rtl' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ color: '#C9A227', fontSize: 18, fontWeight: 700, margin: 0 }}>{editItem ? '✏️ تعديل المؤسسة' : '+ إضافة مؤسسة جديدة'}</h2>
