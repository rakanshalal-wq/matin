...
const getHeaders = (): Record<string, string> => {
  try { const t = localStorage.getItem('matin_token'); if (t) return { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + t }; const u = JSON.parse(localStorage.getItem('matin_user') || '{}'); return { 'Content-Type': 'application/json', 'x-user-id': String(u.id || '') }; } catch { return { 'Content-Type': 'application/json' }; }
};
  ...
  return (
    <div style={{minHeight:'100vh',background:BG,padding:'32px 24px',direction:'rtl',fontFamily:'Cairo, sans-serif'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:32,flexWrap:'wrap',gap:16}}>
        <div><h1 style={{fontSize:28,fontWeight:800,color:'white',margin:0}}>📚 المكتبة المدرسية</h1><p style={{color:'rgba(255,255,255,0.5)',marginTop:6,fontSize:14}}>ادارة الكتب والاستعارات</p></div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:16,marginBottom:28}}>
        {[{l:'اجمالي الكتب',v:books.length,c:GOLD,i:'📚'},{l:'متاح',v:books.filter((r:any)=>r.status==='available').length,c:'#10B981',i:'✅'},{l:'مستعار',v:books.filter((r:any)=>r.status==='borrowed').length,c:'#F59E0B',i:'📖'},{l:'مفقود',v:books.filter((r:any)=>r.status==='lost').length,c:'#EF4444',i:'❓'}].map((s,i)=>(
      ...
      <div style={{background:CB,border:'1px solid '+BR,borderRadius:16,overflow:'hidden'}}>
        {loading?<div style={{textAlign:'center',padding:60,color:'rgba(255,255,255,0.4)'}}>جاري التحميل...</div>:filtered.length===0?
          <div style={{textAlign:'center',padding:60}}><div style={{fontSize:48,marginBottom:16}}>📚</div><p style={{color:'rgba(255,255,255,0.4)',fontSize:16}}>لا توجد كتب مسجلة</p><button onClick={()=>setShowModal(true)} style={{background:GOLD,border:'none',borderRadius:10,padding:'10px 24px',color:'#0B0B16',fontWeight:700,cursor:'pointer',marginTop:16}}>+ اضافة اول كتاب</button></div>:
