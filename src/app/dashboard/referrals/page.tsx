...
export default function Page() {
  ...
  useEffect(() => { fetchData(); }, []);
  const fetchData = async () => { setLoading(true); try { const r = await fetch('/api/referrals', { headers: getH() }); const d = await r.json(); setItems(Array.isArray(d) ? d : (d.items || d.data || [])); } catch { setItems([]); } finally { setLoading(false); } };
  const handleSave = async () => { setSaving(true); try { const m = editItem ? 'PUT' : 'POST'; const u = editItem ? '/api/referrals?id=' + editItem.id : '/api/referrals'; const r = await fetch(u, { method: m, headers: getH(), body: JSON.stringify(form) }); if (r.ok) { setShowModal(false); setEditItem(null); setForm({}); fetchData(); } } catch { } finally { setSaving(false); } };
  const handleDelete = async (id: number) => { if (!confirm('تأكيد الحذف؟')) return; try { await fetch('/api/referrals?id=' + id, { method: 'DELETE', headers: getH() }); fetchData(); } catch { } };
    ...
    <div style={{ minHeight: '100vh', background: BG, padding: '32px 24px', direction: 'rtl', fontFamily: 'Cairo, sans-serif' }}>
      ...
      <div style={{ background: CB, border: '1px solid ' + BR, borderRadius: 16, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr style={{ borderBottom: '1px solid ' + BR }}>{["الطالب","من الفصل","إلى الفصل","التاريخ","السبب","إجراءات"].map(h => <th key={h} style={{ padding: "14px 16px", textAlign: "right", color: "rgba(255,255,255,0.5)", fontSize: 13, fontWeight: 600 }}>{h}</th>)}</tr></thead>
            <tbody>
      ...
      {showModal && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
        <div style={{ background: '#12121F', border: '1px solid ' + BR, borderRadius: 20, padding: 32, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}>
          ...
          <div style={{ display: "grid", gap: 16 }}><div><label style={lbl}>اسم الطالب *</label><input value={form.student_name || ""} onChange={e => setForm({ ...form, student_name: e.target.value })} style={inp} /></div><div><label style={lbl}>من الفصل</label><input value={form.from_class || ""} onChange={e => setForm({ ...form, from_class: e.target.value })} style={inp} /></div><div><label style={lbl}>إلى الفصل</label><input value={form.to_class || ""} onChange={e => setForm({ ...form, to_class: e.target.value })} style={inp} /></div><div><label style={lbl}>السبب</label><textarea value={form.reason || ""} onChange={e => setForm({ ...form, reason: e.target.value })} style={{ ...inp, height: 70, resize: "vertical" }} /></div></div>
