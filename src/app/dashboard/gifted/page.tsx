...
export default function Page() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/gifted', { headers: getHeaders() });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = editItem ? '/api/gifted?id=' + editItem.id : '/api/gifted';
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await fetch('/api/gifted?id=' + id, { method: 'DELETE', headers: getHeaders() }); fetchItems(); } catch { }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, padding: '32px 24px', direction: 'rtl', fontFamily: 'Cairo, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}><IconRenderer name="ICON_Star" size={18} /> الطلاب الموهوبون</h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', marginTop: 6, fontSize: 14 }}>رعاية وتتبع الطلاب الموهوبين</p>
        </div>
