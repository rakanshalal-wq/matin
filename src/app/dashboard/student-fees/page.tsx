  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, padding: '32px 24px', direction: 'rtl', fontFamily: 'Cairo, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>💰 الرسوم الدراسية</h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', marginTop: 6, fontSize: 14 }}>متابعة رسوم الطلاب والمدفوعات</p>
        </div>
        <div style={{ display: 'flex', gap: 12 }}>
          <button onClick={exportCSV} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid ' + BORDER, borderRadius: 10, padding: '10px 18px', color: 'white', cursor: 'pointer', fontSize: 14 }}>📥 تصدير</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16, marginBottom: 28 }}>
        {[
          { label: 'اجمالي الرسوم', value: totalAmount.toLocaleString() + ' ر.س', color: GOLD, icon: '💵' },
          { label: 'المحصّل', value: totalPaid.toLocaleString() + ' ر.س', color: '#10B981', icon: '✅' },
          { label: 'المتبقي', value: totalPending.toLocaleString() + ' ر.س', color: '#EF4444', icon: '⏳' },
          { label: 'متاخرو الدفع', value: overdueCount, color: '#F59E0B', icon: '⚠️' },
        ].map((s, i) => (
