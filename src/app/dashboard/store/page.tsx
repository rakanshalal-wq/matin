  ...
  return (
    <div style={{ fontFamily: 'IBM Plex Sans Arabic, sans-serif', direction: 'rtl' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>🛒 إدارة المتجر</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'المنتجات', value: products.length, icon: '📦', color: '#C9A227' },
          { label: 'الطلبات', value: orders.length, icon: '🛒', color: '#3B82F6' },
          { label: 'طلبات جديدة', value: orders.filter(o => o.status === 'pending').length, icon: '⏳', color: '#F59E0B' },
          { label: 'الإيرادات', value: totalRevenue.toFixed(0) + ' ر.س', icon: '💰', color: '#22C55E' },
        ].map((stat, i) => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16 }}>
              {products.map(product => (
                <div key={product.id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, overflow: 'hidden' }}>
                  {product.image ? <div style={{ height: 140, background: `url(${product.image}) center/cover` }} /> : <div style={{ height: 140, background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 40 }}>📦</div>}
                  <div style={{ padding: 16 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => openEdit(product)} style={{ flex: 1, background: 'rgba(201,162,39,0.1)', color: '#C9A227', border: '1px solid rgba(201,162,39,0.3)', borderRadius: 8, padding: '7px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>✏️ تعديل</button>
                      <button onClick={() => handleDelete(product.id)} style={{ background: 'rgba(239,68,68,0.1)', color: '#EF4444', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: '7px 12px', fontSize: 13, cursor: 'pointer' }}>🗑️</button>
                    </div>
                  </div>
                </div>
              ))}
                  ...
                  {orders.map((order, i) => (
                    <tr key={order.id} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                      ...
                      <td style={{ padding: '14px 16px' }}>
                        <span style={{ background: order.payment_status === 'paid' ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)', color: order.payment_status === 'paid' ? '#22C55E' : '#EF4444', fontSize: 12, padding: '4px 10px', borderRadius: 20, fontWeight: 700 }}>{order.payment_status === 'paid' ? '✅ مدفوع' : '⏳ غير مدفوع'}</span>
                      </td>
                      <td style={{ padding: '14px 16px' }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: '#1B263B', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 32, width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontWeight: 800, margin: 0 }}>{editing ? '✏️ تعديل منتج' : '➕ منتج جديد'}</h2>
              <button onClick={() => setShowModal(false)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: 24, cursor: 'pointer' }}>✕</button>
            </div>
