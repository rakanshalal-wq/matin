  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي التطعيمات', value: stats.total, icon: "ICON_Syringe", color: '#C9A227' },
      ...
      <div style={{ marginBottom: 20 }}>
        <input placeholder="Search بحث بالاسم أو التطعيم..." value={search} onChange={e => setSearch(e.target.value)} style={{ ...inputStyle, maxWidth: 400 }} />
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center' }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}><IconRenderer name="ICON_Syringe" size={36} /></div>
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    {item.next_dose_date ? (
                      <span style={{ color: isOverdue(item.next_dose_date) ? '#EF4444' : isDueNow(item.next_dose_date) ? '#F59E0B' : 'rgba(255,255,255,0.6)', fontWeight: isOverdue(item.next_dose_date) || isDueNow(item.next_dose_date) ? 700 : 400, fontSize: 13 }}>
                        ...
                        {isDueNow(item.next_dose_date) && ' AlertTriangle<IconRenderer name="ICON_Star" size={18} /> قريباً'}
                      </span>
                    ) : '—'}
                  </td>
