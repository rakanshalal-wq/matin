...
export default function VaccinationsPage() {
 ...
 {[
 { label: 'إجمالي التطعيمات', value: stats.total, icon: "ICON_Syringe", color: 'var(--gold)' },
 { label: 'مكتمل', value: stats.completed, icon: "ICON_CheckCircle", color: '#10B981' },
 { label: 'بانتظار جرعة', value: stats.pending, icon: '⏳', color: '#F59E0B' },
 { label: 'متأخر', value: stats.overdue, icon: "ICON_Siren", color: '#EF4444' },
 ].map((stat, i) => (
 ...
 <div style={{width:44,height:44,borderRadius:10,background:"rgba(107,114,128,0.15)",display:"flex",alignItems:"center",justifyContent:"center"}}><Syringe size={19} color="#6B7280" /></div>
 ...
 <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(16,185,129,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}><IconRenderer name="ICON_Syringe" size={36} /></div>
 <div style={{ color: 'white', fontWeight: 600, fontSize: 14 }}>{item.student_name}</div>
 </div>
 </td>
 <td style={{ padding: '14px 16px', color: 'var(--gold)', fontWeight: 600, fontSize: 14 }}>{item.vaccine_name}</td>
 <td style={{ padding: '14px 16px' }}>
 ...
 {isOverdue(item.next_dose_date) && ' Ban متأخر'}
 {isDueNow(item.next_dose_date) && ' Alert<Triangle size={16} /> قريباً'}
 </span>
 ) : '—'}
 </td>
 ...
 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
 <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? 'Pencil<IconRenderer name="ICON_Pencil" size={18} /> تعديل تطعيم' : 'Plus تسجيل تطعيم جديد'}</h2>
 ...
 <button onClick={handleSubmit} style={{ background: 'linear-gradient(135deg, var(--gold), var(--gold2))', color: 'var(--bg)', border: 'none', borderRadius: 10, padding: '12px 24px', cursor: 'pointer', fontWeight: 700 }}>{editItem ? 'Save تحديث' : 'Plus تسجيل'}</button>
 </div>
 </div>
 </div>
 )}
