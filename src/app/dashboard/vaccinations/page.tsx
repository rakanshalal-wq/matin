  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>Syringe التطعيمات</h1>
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center' }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>Syringe</div>
                ...
                <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: item.status === 'pending' && isOverdue(item.next_dose_date) ? 'rgba(185,28,28,0.05)' : item.status === 'pending' && isDueNow(item.next_dose_date) ? 'rgba(245,158,11,0.03)' : 'transparent' }}>
                  <td style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(16,185,129,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>Syringe</div>
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    {item.next_dose_date ? (
                      <span style={{ color: isOverdue(item.next_dose_date) ? '#EF4444' : isDueNow(item.next_dose_date) ? '#F59E0B' : 'rgba(255,255,255,0.6)', fontWeight: isOverdue(item.next_dose_date) || isDueNow(item.next_dose_date) ? 700 : 400, fontSize: 13 }}>
                        ...
                        {isDueNow(item.next_dose_date) && ' AlertTriangle️ قريباً'}
                      </span>
                    ) : '—'}
                  </td>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#06060E', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? 'Pencil️ تعديل تطعيم' : 'Plus تسجيل تطعيم جديد'}</h2>
