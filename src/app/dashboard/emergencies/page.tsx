  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>🚑 الحالات الطارئة</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي الحالات', value: stats.total, icon: '🚑', color: '#C9A227' },
          { label: 'مفتوحة', value: stats.open, icon: '🔴', color: '#EF4444' },
          { label: 'تم الحل', value: stats.resolved, icon: '✅', color: '#10B981' },
          { label: 'حرجة', value: stats.critical, icon: '🚨', color: '#DC2626' },
        ].map((stat, i) => (
                ...
                <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: item.severity === 'critical' ? 'rgba(185,28,28,0.05)' : item.severity === 'high' ? 'rgba(239,68,68,0.03)' : 'transparent' }}>
                  <td style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: severityColors[item.severity]?.bg || 'rgba(245,158,11,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🚑</div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#06060E', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? '✏️ تعديل حالة' : '➕ تسجيل حالة طارئة'}</h2>
