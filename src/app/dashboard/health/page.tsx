  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}><Hospital className="w-5 h-5 inline-block" /> السجلات الصحية</h1>
        ...
        <button onClick={() => { setEditItem(null); setForm({ student_name: '', type: 'checkup', description: '', doctor_name: '', date: '', blood_type: '', allergies: '', chronic_conditions: '', status: 'active' }); setShowModal(true); }} style={{ background: 'linear-gradient(135deg, #C9A227 0%, #D4B03D 100%)', color: '#06060E', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer', fontSize: 15 }}>
          <Plus className="w-5 h-5 inline-block" /> إضافة سجل صحي
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي السجلات', value: stats.total, icon: '<Hospital className="w-5 h-5 inline-block" />', color: '#C9A227' },
          { label: 'نشط', value: stats.active, icon: '<Heart className="w-5 h-5 inline-block" />', color: '#10B981' },
          { label: 'مغلق', value: stats.closed, icon: '<Folder className="w-5 h-5 inline-block" />', color: '#6B7280' },
          { label: 'حرج', value: stats.critical, icon: '<Siren className="w-5 h-5 inline-block" />', color: '#EF4444' },
        ].map((stat, i) => (
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center' }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}><Hospital className="w-5 h-5 inline-block" /></div>
                ...
                <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  <td style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(16,185,129,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}><Hospital className="w-5 h-5 inline-block" /></div>
                      <div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#06060E', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? '<Pencil className="w-5 h-5 inline-block" />️ تعديل سجل صحي' : '<Plus className="w-5 h-5 inline-block" /> إضافة سجل صحي'}</h2>
