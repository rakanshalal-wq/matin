...
export default function LeadsPage() {
  ...
  const handleAdd = async () => {
    if (!formData.name) return alert('أدخل اسم العميل');
    setSaving(true);
    try {
      const res = await fetch('/api/leads', { method: 'POST', headers: getHeaders(), body: JSON.stringify(formData) });
      if (res.ok) { setShowAddModal(false); setFormData({ name: '', email: '', phone: '', company: '', source: '', status: 'new', notes: '' }); fetchItems(); }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>🎯 العملاء المحتملين</h1>
          <p style={{ color: 'rgba(255,255,255,0.6)', marginTop: 8 }}>متابعة العملاء المحتملين وتحويلهم لمشتركين</p>
        </div>
        <button onClick={() => setShowAddModal(true)} style={{ background: 'linear-gradient(135deg, #C9A227 0%, #D4B03D 100%)', color: '#0D1B2A', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer' }}>➕ إضافة عميل</button>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي العملاء', value: items.length, icon: '🎯', color: '#C9A227' },
          { label: 'جديد', value: items.filter(i => i.status === 'new').length, icon: '🆕', color: '#3B82F6' },
          { label: 'مهتم', value: items.filter(i => i.status === 'interested').length, icon: '✅', color: '#10B981' },
          { label: 'تم التحويل', value: items.filter(i => i.status === 'converted').length, icon: '🏆', color: '#C9A227' },
        ].map((stat, i) => (
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center' }}>
            ...
            <button onClick={() => setShowAddModal(true)} style={{ marginTop: 16, background: 'linear-gradient(135deg, #C9A227 0%, #D4B03D 100%)', color: '#0D1B2A', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer' }}>➕ إضافة أول عميل</button>
          </div>
        ) : (
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#0D1B2A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 22, fontWeight: 700, margin: 0 }}>🎯 إضافة عميل محتمل</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div><label style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginBottom: 6, display: 'block' }}>المصدر</label><select value={formData.source} onChange={e => setFormData({ ...formData, source: e.target.value })} style={{ ...inputStyle, appearance: 'auto' } as any}><option value="" style={{ background: '#0D1B2A' }}>اختر المصدر</option><option value="website" style={{ background: '#0D1B2A' }}>الموقع</option><option value="referral" style={{ background: '#0D1B2A' }}>توصية</option><option value="social" style={{ background: '#0D1B2A' }}>سوشيال ميديا</option><option value="ads" style={{ background: '#0D1B2A' }}>إعلانات</option><option value="other" style={{ background: '#0D1B2A' }}>أخرى</option></select></div>
              <div><label style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginBottom: 6, display: 'block' }}>الحالة</label><select value={formData.status} onChange={e => setFormData({ ...formData, status: e.target.value })} style={{ ...inputStyle, appearance: 'auto' } as any}><option value="new" style={{ background: '#0D1B2A' }}>جديد</option><option value="contacted" style={{ background: '#0D1B2A' }}>تم التواصل</option><option value="interested" style={{ background: '#0D1B2A' }}>مهتم</option><option value="converted" style={{ background: '#0D1B2A' }}>تم التحويل</option><option value="lost" style={{ background: '#0D1B2A' }}>خسارة</option></select></div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24, justifyContent: 'flex-start' }}>
              <button onClick={handleAdd} disabled={saving} style={{ background: 'linear-gradient(135deg, #C9A227 0%, #D4B03D 100%)', color: '#0D1B2A', padding: '12px 32px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? '⏳ جاري الحفظ...' : '💾 حفظ العميل'}</button>
