...
export default function LeadsPage() {
 ...
 case 'converted': return { text: 'تم التحويل', color: 'var(--gold)', bg: 'rgba(201,162,39,0.1)' };
 ...
 <div>
 <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}><IconRenderer name="ICON_Target" size={18} /> العملاء المحتملين</h1>
 <p style={{ color: 'rgba(255,255,255,0.6)', marginTop: 8 }}>متابعة العملاء المحتملين وتحويلهم لمشتركين</p>
 </div>
 <button onClick={() => { setEditItem(null); setFormData({ name: '', email: '', phone: '', company: '', source: '', status: 'new', notes: '' }); setErrMsg(''); setShowAddModal(true); }} style={{ background: 'linear-gradient(135deg, var(--gold), var(--gold2))', color: 'var(--bg)', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer' }}><IconRenderer name="ICON_Plus" size={18} /> إضافة عميل</button>
 </div>

 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
 {[
 { label: 'إجمالي العملاء', value: items.length, icon: "ICON_Target", color: 'var(--gold)' },
 ...
 { label: 'تم التحويل', value: items.filter(i => i.status === 'converted').length, icon: "ICON_Trophy", color: 'var(--gold)' },
 ].map((stat, i) => (
 ...
 <div style={{width:44,height:44,borderRadius:10,background:"rgba(107,114,128,0.15)",display:"flex",alignItems:"center",justifyContent:"center"}}><Target size={19} color="#6B7280" /></div>
 <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 18 }}>لا يوجد عملاء محتملين</p>
 <button onClick={() => setShowAddModal(true)} style={{ marginTop: 16, background: 'linear-gradient(135deg, var(--gold), var(--gold2))', color: 'var(--bg)', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer' }}><IconRenderer name="ICON_Plus" size={18} /> إضافة أول عميل</button>
 </div>
 ) : (
 <table style={{ width: '100%', borderCollapse: 'collapse' }}>
 <thead>
 <tr style={{ background: 'rgba(201,162,39,0.1)' }}>
 <th style={{ padding: 16, textAlign: 'right', color: 'var(--gold)', fontWeight: 700 }}>العميل</th>
 <th style={{ padding: 16, textAlign: 'center', color: 'var(--gold)', fontWeight: 700 }}>الشركة/المدرسة</th>
 <th style={{ padding: 16, textAlign: 'center', color: 'var(--gold)', fontWeight: 700 }}>الجوال</th>
 <th style={{ padding: 16, textAlign: 'center', color: 'var(--gold)', fontWeight: 700 }}>المصدر</th>
 ...
 <div style={{ width: 40, height: 40, background: 'rgba(201,162,39,0.1)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}><IconRenderer name="ICON_Target" size={36} /></div>
 <div>
 ...
 <div><label style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginBottom: 6, display: 'block' }}>المصدر</label><select value={formData.source} onChange={e => setFormData({ ...formData, source: e.target.value })} style={{ ...inputStyle, appearance: 'auto' } as any}><option value="" style={{ background: 'var(--bg)' }}>اختر المصدر</option><option value="website" style={{ background: 'var(--bg)' }}>الموقع</option><option value="referral" style={{ background: 'var(--bg)' }}>توصية</option><option value="social" style={{ background: 'var(--bg)' }}>سوشيال ميديا</option><option value="ads" style={{ background: 'var(--bg)' }}>إعلانات</option><option value="other" style={{ background: 'var(--bg)' }}>أخرى</option></select></div>
 <div><label style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginBottom: 6, display: 'block' }}>الحالة</label><select value={formData.status} onChange={e => setFormData({ ...formData, status: e.target.value })} style={{ ...inputStyle, appearance: 'auto' } as any}><option value="new" style={{ background: 'var(--bg)' }}>جديد</option><option value="contacted" style={{ background: 'var(--bg)' }}>تم التواصل</option><option value="interested" style={{ background: 'var(--bg)' }}>مهتم</option><option value="converted" style={{ background: 'var(--bg)' }}>تم التحويل</option><option value="lost" style={{ background: 'var(--bg)' }}>خسارة</option></select></div>
 ...
 <button onClick={handleSave} disabled={saving} style={{ background: 'linear-gradient(135deg, var(--gold), var(--gold2))', color: 'var(--bg)', padding: '12px 32px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? '⏳ جاري الحفظ...' : 'Save حفظ العميل'}</button>
