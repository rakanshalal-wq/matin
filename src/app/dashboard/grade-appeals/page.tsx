...
export default function Page(){
 ...
 <div key={'إجمالي التظلمات'} style={{background:CB,border:'1px solid '+BR,borderRadius:14,padding:'18px 20px'}}><div style={{fontSize:26,fontWeight:800,color:'#C9A84C'}}>{loading?'...':items.length}</div><div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:4}}>إجمالي التظلمات</div></div><div key={'قيد المراجعة'} style={{background:CB,border:'1px solid '+BR,borderRadius:14,padding:'18px 20px'}}><div style={{fontSize:26,fontWeight:800,color:'#F59E0B'}}>{loading?'...':items.filter((r:any)=>r.status==='قيد المراجعة').length}</div><div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:4}}>قيد المراجعة</div></div><div key={'مقبولة'} style={{background:CB,border:'1px solid '+BR,borderRadius:14,padding:'18px 20px'}}><div style={{fontSize:26,fontWeight:800,color:'#10B981'}}>{loading?'...':items.filter((r:any)=>r.status==='مقبول').length}</div><div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:4}}>مقبولة</div></div><div key={'مرفوضة'} style={{background:CB,border:'1px solid '+BR,borderRadius:14,padding:'18px 20px'}}><div style={{fontSize:26,fontWeight:800,color:'#EF4444'}}>{loading?'...':items.filter((r:any)=>r.status==='مرفوض').length}</div><div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:4}}>مرفوضة</div></div>
 </div>
