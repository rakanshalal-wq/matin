  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, padding: '32px 24px', direction: 'rtl', fontFamily: 'Cairo, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}><UtensilsCrossed className="w-5 h-5 inline-block" />️ الكافتيريا</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16, marginBottom: 28 }}>
        {[
          { label: 'اجمالي الاصناف', value: totalItems, color: GOLD, icon: '<UtensilsCrossed className="w-5 h-5 inline-block" />️' },
          { label: 'متاح الان', value: availableItems, color: '#10B981', icon: '<CheckCircle className="w-5 h-5 inline-block" />' },
          { label: 'غير متاح', value: totalItems - availableItems, color: '#EF4444', icon: '<XCircle className="w-5 h-5 inline-block" />' },
          { label: 'متوسط السعر', value: avgPrice + ' ر.س', color: '#3B82F6', icon: '<Coins className="w-5 h-5 inline-block" />' },
        ].map((s, i) => (
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <input placeholder="<Search className="w-5 h-5 inline-block" /> بحث عن صنف..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{ ...inp, width: 260 }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16 }}>
        {loading ? (
          ...
          <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: 60 }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}><UtensilsCrossed className="w-5 h-5 inline-block" />️</div>
          ...
          <div key={item.id || i} style={{ background: CARD_BG, border: '1px solid ' + (item.available !== false ? BORDER : 'rgba(239,68,68,0.2)'), borderRadius: 16, padding: '20px', position: 'relative' }}>
            ...
            <div style={{ fontSize: 32, marginBottom: 12 }}><UtensilsCrossed className="w-5 h-5 inline-block" />️</div>
