'use client';
import { ClipboardList, File, Search, X } from "lucide-react";
...
export default function Page() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = editItem ? '/api/contracts?id=' + editItem.id : '/api/contracts';
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await fetch('/api/contracts?id=' + id, { method: 'DELETE', headers: getHeaders() }); fetchItems(); } catch { }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, padding: '32px 24px', direction: 'rtl', fontFamily: 'Cairo, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}><File className="w-5 h-5 inline-block" /> العقود والاتفاقيات</h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', marginTop: 6, fontSize: 14 }}>إدارة عقود الموظفين والموردين</p>
        </div>
      ...
      <div style={{ background: CARD_BG, border: '1px solid ' + BORDER, borderRadius: 16, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 60 }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}><File className="w-5 h-5 inline-block" /></div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: '#12121F', border: '1px solid ' + BORDER, borderRadius: 20, padding: 32, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? 'تعديل' : 'اضافة جديد'} — العقود والاتفاقيات</h2>
