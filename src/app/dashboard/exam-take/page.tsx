...
export default function ExamTakePage() {
 ...
 <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0C0E17', fontFamily: 'var(--font)', direction: 'rtl' as const }}>
 ...
 <h1 style={{ color: 'var(--gold)', fontSize: 24, fontWeight: 700, margin: '0 0 16px' }}>جاهز للاختبار؟</h1>
 ...
 <button onClick={enterFullscreen} style={{ padding: '14px 40px', background: 'linear-gradient(135deg, #D4A843, #E8C547)', color: '#000', border: 'none', borderRadius: 12, fontSize: 18, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)' }}>
 ...
 <a href="/dashboard/exams" style={{ padding: '12px 32px', background: 'rgba(255,255,255,0.05)', color: 'var(--gold)', border: '1px solid rgba(201,162,39,0.2)', borderRadius: 12, fontSize: 14, textDecoration: 'none', fontFamily: 'var(--font)' }}>
 ← العودة للاختبارات
 </a>
 </div>
 </div>
 ...
 // === شاشة الاختبار ===
 return (
 <div style={{ minHeight: '100vh', background: '#0C0E17', fontFamily: 'var(--font)', direction: 'rtl' as const, userSelect: 'none' as const }}>
 {/* الشريط العلوي */}
 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 24px', background: 'var(--bg-card)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
 ...
 <button onClick={finishExam} style={{ padding: '8px 20px', background: 'rgba(16,185,129,0.1)', color: '#10B981', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 8, fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)' }}> تسليم</button>
 </div>
 ...
 <span style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(201,162,39,0.15)', color: 'var(--gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 700 }}>{currentQ + 1}</span>
 ...
 <button onClick={() => setCurrentQ(Math.max(0, currentQ - 1))} disabled={currentQ === 0} style={{ padding: '10px 24px', background: currentQ === 0 ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.05)', color: currentQ === 0 ? '#4B5563' : '#D4D4D8', border: '1px solid var(--border)', borderRadius: 10, fontSize: 14, cursor: 'pointer', fontFamily: 'var(--font)' }}>
 → السابق
 </button>
 {currentQ < questions.length - 1 ? (
 <button onClick={() => setCurrentQ(currentQ + 1)} style={{ padding: '10px 24px', background: 'rgba(201,162,39,0.1)', color: 'var(--gold)', border: '1px solid rgba(201,162,39,0.2)', borderRadius: 10, fontSize: 14, cursor: 'pointer', fontFamily: 'var(--font)' }}>
 التالي ←
 </button>
 ) : (
 <button onClick={finishExam} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #10B981, #059669)', color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font)' }}> تسليم الاختبار
 </button>
 )}
 </div>
 ...
 {questions.map((_, i) => (
 <button key={i} onClick={() => setCurrentQ(i)} style={{ width: 36, height: 36, borderRadius: '50%', border: 'none', background: answers[questions[i]?.id] ? 'rgba(16,185,129,0.15)' : i === currentQ ? 'rgba(201,162,39,0.15)' : 'rgba(255,255,255,0.03)', color: answers[questions[i]?.id] ? '#10B981' : i === currentQ ? '#D4A843' : '#6B7280', fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)' }}>
 {i + 1}
 </button>
 ))}
 </div>
