  ...
  return (
    <div style={{ padding: '24px', direction: 'rtl', fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: '#C9A227', margin: 0 }}>🏫 المدارس والمؤسسات</h1>
      ...
      {showAdd && (
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(201,162,39,0.2)', borderRadius: 16, padding: 24, marginBottom: 24 }}>
          <h3 style={{ color: '#C9A227', fontSize: 18, margin: '0 0 20px', fontWeight: 700 }}>
            {editSchool ? '✏️ تعديل المؤسسة' : '➕ إضافة مؤسسة جديدة'}
          </h3>
            ...
            <div key={school.id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 20, transition: 'border-color 0.2s' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                ...
                <span style={{
                  ...
                  {school.status === 'ACTIVE' ? '✓ نشطة' : school.status === 'SUSPENDED' ? '⛔ موقوفة' : '⏳ تجريبية'}
                </span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 14 }}>
                {school.city && <span style={{ color: '#9CA3AF', fontSize: 13 }}>📍 {school.city}</span>}
                {school.phone && <span style={{ color: '#9CA3AF', fontSize: 13 }}>📱 {school.phone}</span>}
                {school.email && <span style={{ color: '#9CA3AF', fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>✉️ {school.email}</span>}
                {school.owner_name && <span style={{ color: '#9CA3AF', fontSize: 13 }}>👤 {school.owner_name}</span>}
              </div>
              {canAdd && (
