...
export default function InstituteOwnerDashboard() {
  ...
  const ACCENT = '#14B8A6'; // تيل - مناسب للمعاهد
  ...
  return (
    <div style={{ padding: '24px', maxWidth: 1400, margin: '0 auto', fontFamily: 'IBM Plex Sans Arabic,sans-serif' }}>
      ...
      <div style={{ background: `linear-gradient(135deg,rgba(20,184,166,0.08) 0%,rgba(201,168,76,0.05) 50%,rgba(16,16,38,0.8) 100%)`, border: `1px solid ${ACCENT}20`, borderRadius: 18, padding: '28px 32px', marginBottom: 24, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16, position: 'relative' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            {school?.logo_url || school?.logo
              ...
              : <div style={{ width: 64, height: 64, borderRadius: 14, background: `${ACCENT}10`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, fontWeight: 900, color: ACCENT, border: `2px solid ${ACCENT}20` }}>{school?.name_ar?.charAt(0) || school?.name?.charAt(0) || 'م'}</div>
            }
            <div>
              <div style={{ color: `${ACCENT}CC`, fontSize: 12, fontWeight: 600, marginBottom: 4 }}>{greeting()}، {user?.name?.split(' ')[0] || 'مرحباً'}</div>
              <div style={{ color: '#fff', fontSize: 22, fontWeight: 800 }}>{school?.name_ar || school?.name || 'معهدك'}</div>
              <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
                <span style={{ background: `${ACCENT}15`, color: ACCENT, fontSize: 11, padding: '3px 10px', borderRadius: 6, fontWeight: 600, border: `1px solid ${ACCENT}25` }}>معهد</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(190px,1fr))', gap: 16, marginBottom: 24 }}>
        ...
        <StatCard title="المدرسون"               value={stats.teachers ?? '—'}                          color="#3B82F6"    link="/dashboard/teachers" />
        <StatCard title="الفصول والشعب"          value={stats.classes ?? '—'}                           color="#8B5CF6"    link="/dashboard/classes" />
        <StatCard title="الاختبارات النشطة"      value={stats.active_exams ?? stats.exams ?? '—'}       color="#EF4444"    link="/dashboard/exams" />
        <StatCard title="طلبات الانضمام"         value={pendingAdmissions.length || '—'}                color="#F59E0B"    link="/dashboard/admission" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.08)', borderRadius: 14, padding: '20px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            {[
              { label: 'الطلاب',          href: '/dashboard/students',     color: '#14B8A6', count: stats.students },
              { label: 'المدرسون',         href: '/dashboard/teachers',     color: '#3B82F6', count: stats.teachers },
              { label: 'الفصول',           href: '/dashboard/classes',      color: '#8B5CF6', count: stats.classes },
              ...
              { label: 'المواد',           href: '/dashboard/subjects',     color: '#34D399', count: null },
              { label: 'المناهج',          href: '/dashboard/curriculum',   color: '#F472B6', count: null },
      ...
      {/* <IconRenderer name="ICON_Star" size={18} /> الخدمات المتخصصة للمعهد <IconRenderer name="ICON_Star" size={18} /> */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>

        {/* الخدمات الأكاديمية المتخصصة */}
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.08)', borderRadius: 14, padding: '20px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            {[
              ...
              { label: 'المنح',          href: '/dashboard/scholarships',   color: '#F472B6' },
              { label: 'الرسوم',         href: '/dashboard/student-fees',   color: '#C9A84C' },
              { label: 'المستلزمات',     href: '/dashboard/inventory',      color: '#84CC16' },
            ].map((item, i) => <SectionCard key={i} {...item} />)}
          </div>
        </div>
      ...
      {/* <IconRenderer name="ICON_Star" size={18} /> الاختبارات + طلبات الانضمام + النشاط <IconRenderer name="ICON_Star" size={18} /> */}
      ...
      {/* <IconRenderer name="ICON_Star" size={18} /> رابط المعهد العام <IconRenderer name="ICON_Star" size={18} /> */}
      {school?.code && (
        <div style={{ background: 'rgba(201,168,76,0.04)', border: '1px solid rgba(201,168,76,0.12)', borderRadius: 14, padding: '20px 24px' }}>
          <div style={{ color: 'rgba(201,168,76,0.6)', fontSize: 10, fontWeight: 800, marginBottom: 16, textTransform: 'uppercase', letterSpacing: 2 }}>صفحة المعهد العامة</div>
