...
export default function Page() {
  ...
  const fetchItems = async () => { try { const res = await fetch('/api/activity-log', { headers: getHeaders() }); const data = await res.json(); setItems(Array.isArray(data) ? data : []); } catch (e) { console.error(e); } finally { setLoading(false); } };

  const handleAdd = async () => {
    if (!formData.title) return alert('أدخل البيانات المطلوبة');
    setSaving(true);
    try {
      const res = await fetch('/api/activity-log', { method: 'POST', headers: getHeaders(), body: JSON.stringify(formData) });
  ...
  const handleDelete = async (id: any) => {
    ...
    try { await fetch(`/api/activity-log?id=${id}`, { method: 'DELETE', headers: getHeaders() }); fetchItems(); } catch (e) { console.error(e); }
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>📜 سجل العمليات</h1>
          <p style={{ color: 'rgba(255,255,255,0.6)', marginTop: 8 }}>سجل العمليات والأنشطة</p>
        </div>
