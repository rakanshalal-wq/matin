...
export default function SupportPage() {
  ...
  const typeLabels: Record<string, string> = { complaint: '😤 شكوى', suggestion: '💡 اقتراح', inquiry: '❓ استفسار', technical: '🔧 مشكلة تقنية' };
  ...
  const statusLabels: Record<string, { label: string; color: string; bg: string }> = {
    ...
    in_progress: { label: '⚙️ قيد المعالجة', color: '#F59E0B', bg: 'rgba(245,158,11,0.1)' },
  ...
  return (
    <div style={{ padding: '24px', direction: 'rtl', fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: '#C9A227', margin: 0 }}>🎧 الدعم الفني</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'الكل', value: stats.total, color: '#C9A227', icon: '📋' },
          { label: 'مفتوحة', value: stats.open, color: '#3B82F6', icon: '🔓' },
          { label: 'قيد المعالجة', value: stats.inProgress, color: '#F59E0B', icon: '⚙️' },
          { label: 'مغلقة', value: stats.closed, color: '#10B981', icon: '✅' },
        ].map((s, i) => (
      ...
      {showAdd && (
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(201,162,39,0.2)', borderRadius: 16, padding: 24, marginBottom: 24 }}>
          <h3 style={{ color: '#C9A227', fontSize: 18, margin: '0 0 20px', fontWeight: 700 }}>➕ تذكرة جديدة</h3>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 16, marginTop: 8, flexWrap: 'wrap' }}>
                      <span style={{ color: '#6B7280', fontSize: 12 }}>👤 {ticket.name || 'مجهول'}</span>
                      <span style={{ color: '#6B7280', fontSize: 12 }}>📋 {typeLabels[ticket.type] || ticket.type}</span>
                      <span style={{ color: '#6B7280', fontSize: 12 }}>📅 {ticket.created_at ? new Date(ticket.created_at).toLocaleDateString('ar-SA') : ''}</span>
                    </div>
                  </div>
