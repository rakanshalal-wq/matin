...
export default function SupervisorsPage() {
 ...
 {[
 { label: 'إجمالي المراقبين', value: items.length, icon: '<Eye size={16} />', color: '#D4A843' },
 ...
 <button onClick={() => setShowAddModal(true)} style={{ marginTop: 16, background: 'linear-gradient(135deg, #D4A843 0%, #D4B03D 100%)', color: '#06060E', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer' }}><IconRenderer name="ICON_Plus" size={18} /> إضافة أول مراقب</button>
 </div>
 ) : (
 <table style={{ width: '100%', borderCollapse: 'collapse' }}>
 <thead>
 <tr style={{ background: 'rgba(201,162,39,0.1)' }}>
 <th style={{ padding: 16, textAlign: 'right', color: '#D4A843', fontWeight: 700 }}>المراقب</th>
 ...
 <th style={{ padding: 16, textAlign: 'center', color: '#D4A843', fontWeight: 700 }}>الاختبارات المكلف بها</th>
 ...
 <button onClick={handleSave} disabled={saving} style={{ background: 'linear-gradient(135deg, #D4A843 0%, #D4B03D 100%)', color: '#06060E', padding: '12px 32px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? '⏳ جاري الحفظ...' : 'Save حفظ المراقب'}</button>
