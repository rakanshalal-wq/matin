...
export default function ForumsPage() {
 ...
 <div>
 <h1 className="page-title"><IconRenderer name="ICON_MessageSquare" size={18} /> مجموعات النقاش</h1>
 <p style={{ color: 'rgba(255,255,255,0.6)', marginTop: 8 }}>إدارة منتديات ومجموعات النقاش التعليمية</p>
 </div>
 <button onClick={() => { setEditItem(null); setForm({ title: '', category: 'general', author: '', description: '', members_count: '', posts_count: '', status: 'active' }); setShowModal(true); }} style={{ background: 'linear-gradient(135deg, var(--gold), var(--gold2))', color: 'var(--bg)', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer', fontSize: 15 }}>
 Plus إنشاء مجموعة
 </button>
 </div>
 ...
 {[
 { label: 'إجمالي المجموعات', value: stats.total, icon: "ICON_MessageSquare", color: 'var(--gold)' },
 ...
 {loading ? (
 <div style={{ padding: 60, textAlign: 'center', background: 'var(--bg-card)', borderRadius: 12, gridColumn: '1 / -1' }}><p style={{ color: 'rgba(255,255,255,0.6)' }}>⏳ جاري التحميل...</p></div>
 ) : filtered.length === 0 ? (
 <div style={{ padding: 60, textAlign: 'center', background: 'var(--bg-card)', borderRadius: 12, gridColumn: '1 / -1' }}>
 ...
 <div key={item.id} style={{ background: 'var(--bg-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 20 }}>
 ...
 {item.author && (
 <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, marginBottom: 12 }}>أنشأها: <span style={{ color: 'var(--gold)' }}>{item.author}</span></div>
 )}
 ...
 <button onClick={handleSubmit} style={{ background: 'linear-gradient(135deg, var(--gold), var(--gold2))', color: 'var(--bg)', border: 'none', borderRadius: 10, padding: '12px 24px', cursor: 'pointer', fontWeight: 700 }}>{editItem ? 'Save تحديث' : 'Plus إنشاء'}</button>
 </div>
 </div>
 </div>
 )}
