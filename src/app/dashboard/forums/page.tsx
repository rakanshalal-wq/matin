...
export default function ForumsPage() {
  ...
  const categoryIcons: any = { general: '💭', academic: '📚', activities: '🎯', parents: '👨‍👩‍👧', teachers: '👨‍🏫', students: '🎓', suggestions: '💡', support: '🛠️' };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>💭 مجموعات النقاش</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي المجموعات', value: stats.total, icon: '💭', color: '#C9A227' },
          { label: 'نشطة', value: stats.active, icon: '✅', color: '#10B981' },
          { label: 'إجمالي الأعضاء', value: stats.totalMembers, icon: '👥', color: '#3B82F6' },
          { label: 'إجمالي المنشورات', value: stats.totalPosts, icon: '📝', color: '#8B5CF6' },
        ].map((stat, i) => (
          ...
          <div key={item.id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 20, borderTop: `3px solid ${categoryColors[item.category]?.color || '#3B82F6'}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                <div style={{ width: 48, height: 48, borderRadius: 12, background: categoryColors[item.category]?.bg || 'rgba(59,130,246,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>{categoryIcons[item.category] || '💭'}</div>
                <div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#06060E', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? '✏️ تعديل مجموعة' : '➕ إنشاء مجموعة نقاش'}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} style={inputStyle}>
                  ...
                  <option value="support">دعم فني 🛠️</option>
                </select>
              </div>
              <div>
