...
export default function ElearningPage() {
  ...
  const typeIcons: any = { course: '📚', lesson: '📖', workshop: '🛠️', tutorial: '🎬', quiz: '📝', resource: '📎' };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>💻 التعليم الإلكتروني</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي المحتوى', value: stats.total, icon: '💻', color: '#C9A227' },
          { label: 'نشط', value: stats.active, icon: '✅', color: '#10B981' },
          { label: 'مسودة', value: stats.draft, icon: '📝', color: '#F59E0B' },
          { label: 'إجمالي الطلاب', value: stats.totalStudents, icon: '👨‍🎓', color: '#3B82F6' },
        ].map((stat, i) => (
                ...
                <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  <td style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(139,92,246,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{typeIcons[item.type] || '💻'}</div>
                      <div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#06060E', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? '✏️ تعديل محتوى' : '➕ إضافة محتوى تعليمي'}</h2>
