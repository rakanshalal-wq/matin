...
export default function ChatPage() {
  ...
  const typeIcons: any = { group: '👥', direct: '💬', class: '🏫', department: '🏢', announcement: '📢' };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي المحادثات', value: stats.total, icon: '💬', color: '#C9A227' },
          { label: 'نشطة', value: stats.active, icon: '✅', color: '#10B981' },
          { label: 'مجموعات', value: stats.groups, icon: '👥', color: '#3B82F6' },
          { label: 'محادثات خاصة', value: stats.direct, icon: '🔒', color: '#8B5CF6' },
        ].map((stat, i) => (
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}>👥 {item.members_count || 0}</span>
                  <button onClick={(e) => { e.stopPropagation(); handleEdit(item); }} style={{ background: 'rgba(201,162,39,0.1)', color: '#C9A227', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 11, fontWeight: 600 }}>✏️</button>
                  <button onClick={(e) => { e.stopPropagation(); handleDelete(item.id); }} style={{ background: 'rgba(239,68,68,0.1)', color: '#EF4444', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 11, fontWeight: 600 }}>🗑️</button>
                </div>
              </div>
            </div>
          </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  ...
                  <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} style={inputStyle}>
                    ...
                    <option value="direct">محادثة خاصة 💬</option>
