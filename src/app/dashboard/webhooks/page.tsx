...
import { Eye, Link, Pencil, Plus, Save, Search, Trash2, X } from "lucide-react";
...
export default function Page() {
 ...
 const fetchItems = async () => { try { const res = await fetch('/api/webhooks', { headers: getHeaders() }); const data = await res.json(); setItems(Array.isArray(data) ? data : []); } catch (e) { console.error(e); } finally { setLoading(false); } };
 ...
 const url = editItem ? `/api/webhooks?id=${editItem.id}` : '/api/webhooks';
 ...
 try { await fetch(`/api/webhooks?id=${id}`, { method: 'DELETE', headers: getHeaders() }); fetchItems(); } catch (e) { console.error(e); }
 };
 ...
 <div>
 <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}><IconRenderer name="ICON_Link" size={18} /> Webhooks</h1>
 <p style={{ color: 'rgba(255,255,255,0.6)', marginTop: 8 }}>إدارة Webhooks</p>
 </div>
 ...
 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontSize: 28 }}><IconRenderer name="ICON_Link" size={18} /></span><span style={{ fontSize: 28, fontWeight: 800, color: '#C9A227' }}>{items.length}</span></div>
 ...
 <div style={{width:44,height:44,borderRadius:10,background:"rgba(107,114,128,0.15)",display:"flex",alignItems:"center",justifyContent:"center"}}><Link size={19} color="#6B7280" /></div>
...
<table style={{ width: '100%', borderCollapse: 'collapse' }}>
 ...
 <div style={{ width: 40, height: 40, background: 'rgba(201,162,39,0.1)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}><IconRenderer name="ICON_Link" size={18} /></div>
 <div>
