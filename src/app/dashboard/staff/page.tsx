...
const branchData: Record<string, { title: string; sub: string; total: string; att: string; attSub: string; hdrSub: string }> = {
  all:  { title: 'الموارد البشرية — جميع الفروع',      sub: 'عرض شامل · 3 فروع · 86 موظف',   total: '86', att: '94%', attSub: '81 من 86', hdrSub: 'مدرسة الأمل الدولية · 3 فروع · 86 موظف' },
  main: { title: 'الموارد البشرية — الرئيسي (النزهة)', sub: 'الفرع الأم · 42 موظف',            total: '42', att: '96%', attSub: '40 من 42', hdrSub: 'الفرع الرئيسي — حي النزهة · 42 موظف' },
  b2:   { title: 'الموارد البشرية — فرع الروضة',       sub: 'الفرع الثاني · 26 موظف',          total: '26', att: '92%', attSub: '24 من 26', hdrSub: 'فرع الروضة · 26 موظف' },
  b3:   { title: 'الموارد البشرية — فرع العليا',       sub: 'الفرع الثالث · 18 موظف',          total: '18', att: '88%', attSub: '16 من 18', hdrSub: 'فرع العليا · 18 موظف' },
};

const leaveRequests = [
  { name: 'أ. محمد الغامدي',   branch: 'الرئيسي', type: 'إجازة اعتيادية 3 أيام' },
  { name: 'أ. سارة الزهراني', branch: 'الروضة',   type: 'إجازة مرضية' },
  { name: 'عبدالله الدوسري',  branch: 'الرئيسي', type: 'استئذان يوم' },
  { name: 'فيصل الشمري',      branch: 'العليا',   type: 'إجازة طارئة' },
  { name: 'ريم السلمي',       branch: 'الروضة',   type: 'إجازة اعتيادية أسبوع' },
];

const contracts = [
  { name: 'أ. خالد النمر',      role: 'معلم — الروضة',     date: 'انتهى',        bc: 'br' },
  { name: 'ريم السلمي',         role: 'فراشة — الروضة',    date: 'بعد 8 أيام',   bc: 'bo' },
  { name: 'فيصل الشمري',        role: 'أمن — العليا',       date: 'بعد 20 يوم',   bc: 'bo' },
  { name: 'أ. هند الزهراني',   role: 'معلمة — الرئيسي',   date: 'بعد 28 يوم',   bc: 'bb' },
];

const salaries = [
  { branch: 'الرئيسي — النزهة', amount: '152,000 SAR', color: '#60A5FA' },
  { branch: 'فرع الروضة',        amount: '88,000 SAR',  color: '#A78BFA' },
  { branch: 'فرع العليا',        amount: '64,000 SAR',  color: '#10B981' },
];
...
export default function StaffPage() {
  ...
  const openModal = (id: string) => {
    if (id === 'add-modal') setAddModalOpen(true);
    if (id === 'perm-modal') setPermModalOpen(true);
  };

  const closeModal = (id: string) => {
    if (id === 'add-modal') setAddModalOpen(false);
    if (id === 'perm-modal') setPermModalOpen(false);
  };
  ...
  const d = branchData[activeBranch];

  return (
    <div className="dashboard-page">

      {/* OVERLAY */}
      <div className={`ov${sidebarOpen ? ' show' : ''}`} id="ov" onClick={closeSb}></div>

      {/* ADD EMPLOYEE MODAL */}
      <div className={`modal-bg${addModalOpen ? ' show' : ''}`} id="add-modal">
        <div className="modal">
          <div className="mh">
          ...
          <div style={{padding:'16px'}}>
            ...
            <div style={{display:'flex',gap:'8px'}}>
              ...
              <button onClick={() => { showToast('✓ تم إضافة الموظف وإنشاء حسابه'); closeModal('add-modal'); }} style={{flex:2,background:'linear-gradient(135deg,var(--c),var(--c2))',border:'none',borderRadius:'9px',padding:'10px',color:'#fff',fontWeight:800,fontSize:'13px',cursor:'pointer',fontFamily:'var(--f)'}}>إضافة وإنشاء حساب ←</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* PERMISSIONS MODAL */}
      <div className={`modal-bg${permModalOpen ? ' show' : ''}`} id="perm-modal">
        <div className="modal">
          <div className="mh">
            <div className="mt">🔐 صلاحيات <span id="perm-name">{permName}</span></div>
          ...
          <div style={{padding:'16px'}}>
            ...
            <div style={{background:'var(--card)',border:'1px solid var(--b2)',borderRadius:'9px',overflow:'hidden',marginBottom:'12px'}}>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>الدخول لنظام متين</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>حساب وكلمة مرور</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>تطبيق الجوال</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>وصول من الهاتف</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
            </div>

            <div style={{fontSize:'10px',color:'var(--tm)',fontWeight:700,letterSpacing:'1px',marginBottom:'8px'}}>صلاحيات المعلمين (الأكاديمية)</div>
            <div style={{background:'var(--card)',border:'1px solid var(--b2)',borderRadius:'9px',overflow:'hidden',marginBottom:'12px'}}>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>تسجيل حضور الطلاب</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>يومياً لكل حصة</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>رفع الدرجات والواجبات</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>إدخال نتائج الطلاب</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>إنشاء اختبارات</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>اختبارات ورقية وإلكترونية</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>مراسلة أولياء الأمور</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>رسائل مباشرة في النظام</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
            ...
            <div style={{background:'var(--card)',border:'1px solid var(--b2)',borderRadius:'9px',overflow:'hidden',marginBottom:'12px'}}>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>تسجيل ركوب الطلاب (السائق)</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>حضور الباص المدرسي</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>خريطة المدرسة (الأمن)</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>للحراسة والدوريات</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
              ...
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>تقرير عمل يومي</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>للفراشين والسائقين</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
            </div>
            ...
            <div style={{background:'var(--card)',border:'1px solid var(--b2)',borderRadius:'9px',overflow:'hidden',marginBottom:'14px'}}>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>تسجيل حضوره الشخصي</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>بصمة أو من الجوال</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>طلب إجازة عبر النظام</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>اعتيادية / طارئة / مرضية</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
              <div className="perm-row"><div><div style={{fontSize:'12.5px',fontWeight:600,color:'var(--t)'}}>عرض كشف الراتب</div><div style={{fontSize:'10.5px',color:'var(--tm)'}}>قسيمة شهرية</div></div><label className="tog"><input type="checkbox" defaultChecked /><span className="tsl"></span></label></div>
            </div>
            ...
            <div style={{background:'rgba(239,68,68,.05)',border:'1px solid rgba(239,68,68,.18)',borderRadius:'9px',padding:'10px 13px',marginBottom:'14px'}}>
              ...
              <div style={{display:'flex',gap:'8px'}}>
                <button onClick={() => { showToast('✓ تم تجميد الحساب'); closeModal('perm-modal'); }} style={{flex:1,background:'rgba(239,68,68,.08)',border:'1px solid rgba(239,68,68,.25)',borderRadius:'7px',padding:'7px',color:'var(--rd)',fontSize:'11px',fontWeight:700,cursor:'pointer',fontFamily:'var(--f)'}}>🔒 تجميد الحساب</button>
                <button onClick={() => { showToast('✓ تم الحذف النهائي'); closeModal('perm-modal'); }} style={{flex:1,background:'rgba(239,68,68,.12)',border:'1px solid rgba(239,68,68,.3)',borderRadius:'7px',padding:'7px',color:'var(--rd)',fontSize:'11px',fontWeight:700,cursor:'pointer',fontFamily:'var(--f)'}}>🗑️ حذف نهائي</button>
              </div>
            </div>
            ...
            <div style={{display:'flex',gap:'8px'}}>
              ...
              <button onClick={() => { showToast('✓ تم حفظ الصلاحيات'); closeModal('perm-modal'); }} style={{flex:2,background:'linear-gradient(135deg,var(--c),var(--c2))',border:'none',borderRadius:'9px',padding:'10px',color:'#fff',fontWeight:800,fontSize:'13px',cursor:'pointer',fontFamily:'var(--f)'}}>حفظ الصلاحيات ✓</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="sb-top">
          ...
          <div className="branch-sel">
            ...
            <div className="branch-list">
              <div className={`branch-item${activeBranch === 'all' ? ' active' : ''}`} onClick={() => selectBranch('all')}>
              ...
              <div className={`branch-item${activeBranch === 'main' ? ' active' : ''}`} onClick={() => selectBranch('main')}>
              ...
              <div className={`branch-item${activeBranch === 'b2' ? ' active' : ''}`} onClick={() => selectBranch('b2')}>
              ...
              <div className={`branch-item${activeBranch === 'b3' ? ' active' : ''}`} onClick={() => selectBranch('b3')}>
      ...
      <div className="main">
        <header className="hdr">
          <div className="hl">
            ...
            <div>
              <div className="ht" id="hdr-title">{d.title}</div>
              <div className="hs">{d.hdrSub}</div>
            </div>
          </div>
          <div className="hr2">
        ...
        <div className="branch-tabs" id="branch-tabs">
          <button className={`btab${activeBranch === 'all' ? ' active' : ''}`} data-bid="all" onClick={() => switchBranch('all')} style={activeBranch === 'all' ? {color:'var(--c)',borderBottomColor:'var(--c)'} : {}}>🏫 جميع الفروع <span style={{background:'var(--cd)',color:'var(--c)',fontSize:'9px',fontWeight:700,padding:'1px 6px',borderRadius:'8px',marginRight:'4px',border:'1px solid var(--cb)'}}>86</span></button>
          <button className={`btab${activeBranch === 'main' ? ' active' : ''}`} data-bid="main" onClick={() => switchBranch('main')} style={activeBranch === 'main' ? {color:'var(--c)',borderBottomColor:'var(--c)'} : {}}>🏫 الرئيسي — النزهة <span style={{background:'rgba(96,165,250,.1)',color:'var(--bl)',fontSize:'9px',fontWeight:700,padding:'1px 6px',borderRadius:'8px',marginRight:'4px',border:'1px solid rgba(96,165,250,.2)'}}>42</span></button>
          <button className={`btab${activeBranch === 'b2' ? ' active' : ''}`} data-bid="b2" onClick={() => switchBranch('b2')} style={activeBranch === 'b2' ? {color:'var(--c)',borderBottomColor:'var(--c)'} : {}}>🏫 فرع الروضة <span style={{background:'rgba(167,139,250,.1)',color:'var(--pu)',fontSize:'9px',fontWeight:700,padding:'1px 6px',borderRadius:'8px',marginRight:'4px',border:'1px solid rgba(167,139,250,.2)'}}>26</span></button>
          <button className={`btab${activeBranch === 'b3' ? ' active' : ''}`} data-bid="b3" onClick={() => switchBranch('b3')} style={activeBranch === 'b3' ? {color:'var(--c)',borderBottomColor:'var(--c)'} : {}}>🏫 فرع العليا <span style={{background:'rgba(16,185,129,.1)',color:'var(--gr)',fontSize:'9px',fontWeight:700,padding:'1px 6px',borderRadius:'8px',marginRight:'4px',border:'1px solid rgba(16,185,129,.2)'}}>18</span></button>
        </div>
        ...
        <div className="con">
          ...
          <div className="ph">
            <div>
              <div className="pt" id="page-title">👥 {d.title}</div>
              <div className="ps" id="page-sub">{d.sub}</div>
            </div>
          ...
          <div className="sg">
            <div className="sc">
              ...
              <div className="sv" style={{color:'var(--c)'}} id="stat-total">{d.total}</div>
              <div className="sl">إجمالي الموظفين</div>
              <div className="ss" style={{color:'rgba(249,115,22,.6)'}} id="stat-total-sub">{activeBranch === 'all' ? '3 فروع' : d.hdrSub.split('·')[0].trim()}</div>
            </div>
            <div className="sc">
              ...
              <div className="sv" style={{color:'var(--gr)'}} id="stat-att">{d.att}</div>
              <div className="sl">نسبة الحضور اليوم</div>
              <div className="ss" style={{color:'rgba(16,185,129,.6)'}}>{d.attSub} حضروا</div>
            </div>
            <div className="sc">
          ...
          <div className="card">
            <div className="ch">
              <div className="ct">
                ...
                <span id="emp-count" style={{background:'var(--cd)',border:'1px solid var(--cb)',color:'var(--c)',fontSize:'10px',fontWeight:700,padding:'2px 7px',borderRadius:'20px'}}>{d.total}</span>
              </div>
            ...
            <div className="tw">
              <table>
                ...
                <tbody id="emp-table">
                  <tr>
                    <td><div style={{display:'flex',alignItems:'center',gap:'8px'}}><div style={{width:'26px',height:'26px',borderRadius:'6px',background:'rgba(249,115,22,.12)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:700,color:'var(--c)',flexShrink:0}}>م</div><span style={{fontWeight:600,color:'var(--t)'}}>أ. محمد الغامدي</span></div></td>
                    <td style={{color:'var(--tm)',fontSize:'11px'}}>معلم رياضيات</td>
                    <td><span className="badge bb">معلم</span></td>
                    <td><span style={{fontSize:'10px',color:'var(--bl)'}}>● الرئيسي</span></td>
                    <td><span className="badge bg">نشط</span></td>
                    <td style={{color:'var(--gr)',fontWeight:700,fontSize:'11px'}}>✓ حاضر</td>
                    <td><button className="btn-sm" style={{background:'var(--cd)',color:'var(--c)',border:'1px solid var(--cb)'}} onClick={() => openPerm('أ. محمد الغامدي')}>🔐 صلاحيات</button></td>
                  </tr>
                  <tr>
                    <td><div style={{display:'flex',alignItems:'center',gap:'8px'}}><div style={{width:'26px',height:'26px',borderRadius:'6px',background:'rgba(52,211,153,.12)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:700,color:'var(--gr)'}}>س</div><span style={{fontWeight:600,color:'var(--t)'}}>أ. سارة الزهراني</span></div></td>
                    <td style={{color:'var(--tm)',fontSize:'11px'}}>معلمة علوم</td>
                    <td><span className="badge bb">معلمة</span></td>
                    <td><span style={{fontSize:'10px',color:'var(--pu)'}}>● الروضة</span></td>
                    <td><span className="badge bg">نشط</span></td>
                    <td style={{color:'var(--gr)',fontWeight:700,fontSize:'11px'}}>✓ حاضر</td>
                    <td><button className="btn-sm" style={{background:'var(--cd)',color:'var(--c)',border:'1px solid var(--cb)'}} onClick={() => openPerm('أ. سارة الزهراني')}>🔐 صلاحيات</button></td>
                  </tr>
                  <tr>
                    <td><div style={{display:'flex',alignItems:'center',gap:'8px'}}><div style={{width:'26px',height:'26px',borderRadius:'6px',background:'rgba(96,165,250,.12)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:700,color:'var(--bl)'}}>ع</div><span style={{fontWeight:600,color:'var(--t)'}}>عبدالله الدوسري</span></div></td>
                    <td style={{color:'var(--tm)',fontSize:'11px'}}>سائق باص</td>
                    <td><span className="badge bo">سائق</span></td>
                    <td><span style={{fontSize:'10px',color:'var(--bl)'}}>● الرئيسي</span></td>
                    <td><span className="badge bg">نشط</span></td>
                    <td style={{color:'var(--gr)',fontWeight:700,fontSize:'11px'}}>✓ حاضر</td>
                    <td><button className="btn-sm" style={{background:'var(--cd)',color:'var(--c)',border:'1px solid var(--cb)'}} onClick={() => openPerm('عبدالله الدوسري')}>🔐 صلاحيات</button></td>
                  </tr>
                  <tr>
                    <td><div style={{display:'flex',alignItems:'center',gap:'8px'}}><div style={{width:'26px',height:'26px',borderRadius:'6px',background:'rgba(239,68,68,.12)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:700,color:'var(--rd)'}}>ف</div><span style={{fontWeight:600,color:'var(--t)'}}>فيصل الشمري</span></div></td>
                    <td style={{color:'var(--tm)',fontSize:'11px'}}>حارس أمن</td>
                    <td><span className="badge br">أمن</span></td>
                    <td><span style={{fontSize:'10px',color:'var(--gr)'}}>● العليا</span></td>
                    <td><span className="badge bo">إجازة</span></td>
                    <td style={{color:'var(--or)',fontWeight:700,fontSize:'11px'}}>✗ إجازة</td>
                    <td><button className="btn-sm" style={{background:'var(--cd)',color:'var(--c)',border:'1px solid var(--cb)'}} onClick={() => openPerm('فيصل الشمري')}>🔐 صلاحيات</button></td>
                  </tr>
                  <tr>
                    <td><div style={{display:'flex',alignItems:'center',gap:'8px'}}><div style={{width:'26px',height:'26px',borderRadius:'6px',background:'rgba(16,185,129,.12)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:700,color:'var(--gr)'}}>ر</div><span style={{fontWeight:600,color:'var(--t)'}}>ريم السلمي</span></div></td>
                    <td style={{color:'var(--tm)',fontSize:'11px'}}>فراشة</td>
                    <td><span className="badge bg">نظافة</span></td>
                    <td><span style={{fontSize:'10px',color:'var(--pu)'}}>● الروضة</span></td>
                    <td><span className="badge bg">نشط</span></td>
                    <td style={{color:'var(--gr)',fontWeight:700,fontSize:'11px'}}>✓ حاضر</td>
                    <td><button className="btn-sm" style={{background:'var(--cd)',color:'var(--c)',border:'1px solid var(--cb)'}} onClick={() => openPerm('ريم السلمي')}>🔐 صلاحيات</button></td>
                  </tr>
                  <tr>
                    <td><div style={{display:'flex',alignItems:'center',gap:'8px'}}><div style={{width:'26px',height:'26px',borderRadius:'6px',background:'rgba(167,139,250,.12)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:700,color:'var(--pu)'}}>ن</div><span style={{fontWeight:600,color:'var(--t)'}}>نورة الحربي</span></div></td>
                    <td style={{color:'var(--tm)',fontSize:'11px'}}>موظفة إدارية</td>
                    <td><span className="badge bp">إدارية</span></td>
                    <td><span style={{fontSize:'10px',color:'var(--bl)'}}>● الرئيسي</span></td>
                    <td><span className="badge bg">نشط</span></td>
                    <td style={{color:'var(--gr)',fontWeight:700,fontSize:'11px'}}>✓ حاضر</td>
                    <td><button className="btn-sm" style={{background:'var(--cd)',color:'var(--c)',border:'1px solid var(--cb)'}} onClick={() => openPerm('نورة الحربي')}>🔐 صلاحيات</button></td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="card" style={{marginBottom:0}}>
              ...
              {leaveRequests.map((l, i) => (
                <div key={i} className="item">
                  <div className="item-ic" style={{background:'rgba(249,115,22,.1)'}}>📋</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:'12px',fontWeight:600,color:'var(--t)'}}>{l.name}</div>
                    <div style={{fontSize:'10.5px',color:'var(--tm)'}}>{l.branch} · {l.type}</div>
                  </div>
                  <div style={{display:'flex',gap:'4px'}}>
                    <button className="btn-sm" style={{background:'rgba(16,185,129,.08)',color:'var(--gr)',border:'1px solid rgba(16,185,129,.2)'}} onClick={() => showToast('✓ تمت الموافقة على الإجازة')}>قبول</button>
                    <button className="btn-sm" style={{background:'rgba(239,68,68,.08)',color:'var(--rd)',border:'1px solid rgba(239,68,68,.2)'}} onClick={() => showToast('✓ تم رفض الإجازة')}>رفض</button>
                  </div>
                </div>
              ))}
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              <div className="card" style={{marginBottom:0}}>
                ...
                {contracts.map((c, i) => (
                  <div key={i} className="item">
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:'12px',fontWeight:600,color:'var(--t)'}}>{c.name}</div>
                      <div style={{fontSize:'10.5px',color:'var(--tm)'}}>{c.role} · {c.date}</div>
                    </div>
                    <button className="btn-sm" style={{background:'var(--cd)',color:'var(--c)',border:'1px solid var(--cb)'}} onClick={() => showToast('✓ تم إرسال طلب تجديد العقد')}>تجديد</button>
                  </div>
                ))}
              </div>
              ...
              <div className="card" style={{marginBottom:0}}>
                ...
                <div style={{padding:'12px 13px'}}>
                  {salaries.map((s, i) => (
                    <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:'1px solid var(--b2)',fontSize:'12px'}}>
                      <span style={{color:'var(--td)'}}>{s.branch}</span>
                      <strong style={{color:s.color}}>{s.amount}</strong>
                    </div>
                  ))}
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',fontSize:'13px',fontWeight:700}}>
                    <span style={{color:'var(--t)'}}>الإجمالي</span>
                    <span style={{color:'var(--c)'}}>304,000 SAR</span>
                  </div>
                  <button onClick={() => showToast('✓ تم صرف الرواتب')} style={{width:'100%',background:'linear-gradient(135deg,var(--c),var(--c2))',border:'none',borderRadius:'8px',padding:'9px',color:'#fff',fontSize:'12px',fontWeight:700,cursor:'pointer',fontFamily:'var(--f)'}}>💸 صرف الرواتب الآن</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="qg">
              <a className="qi" href="#" onClick={(e) => { e.preventDefault(); openModal('add-modal'); }}><div className="qic" style={{background:'rgba(249,115,22,.1)',border:'1px solid rgba(249,115,22,.2)'}}>➕</div><span className="ql">إضافة موظف</span></a>
              <a className="qi" href="#" onClick={(e) => { e.preventDefault(); openModal('perm-modal'); }}><div className="qic" style={{background:'rgba(239,68,68,.1)',border:'1px solid rgba(239,68,68,.2)'}}>🔐</div><span className="ql">الصلاحيات</span></a>
      ...
      {/* TOAST */}
      {toastMsg && (
        <div style={{position:'fixed',bottom:'24px',left:'50%',transform:'translateX(-50%)',background:'var(--c)',color:'#fff',padding:'10px 20px',borderRadius:'10px',fontSize:'13px',fontWeight:700,zIndex:999,fontFamily:'var(--f)'}}>
          {toastMsg}
        </div>
      )}
