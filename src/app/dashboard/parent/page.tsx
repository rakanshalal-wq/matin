...
export default function ParentPage() {
  ...
  const [busNotifVisible, setBusNotifVisible] = useState(true);
  const [activeChild, setActiveChild] = useState<'فهد' | 'نورة'>('فهد');
  const [mobileTime, setMobileTime] = useState('10:22 ص');
  ...
  useEffect(() => {
    const updateTime = () => {
      setMobileTime(
        new Date().toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' }) + ' ص'
      );
    };
  ...
  const toggleSb = () => setSidebarOpen(v => !v);
  const closeSb = () => setSidebarOpen(false);

  const isF = activeChild === 'فهد';
  const childName = isF ? 'فهد محمد' : 'نورة محمد';
  const childFullName = isF ? 'فهد محمد الزهراني' : 'نورة محمد الزهراني';
  const childSub = isF ? 'الصف الرابع — الفصل ب · مدرسة الأمل الدولية' : 'الصف الأول — الفصل أ · مدرسة الأمل الدولية';
  const childLoc = isF ? 'داخل المدرسة — الفصل الدراسي' : 'في الباص المدرسي — طريق المدرسة';
  const childLocColor = isF ? 'var(--gr)' : 'var(--or)';
  const childAv = isF ? '👦' : '👧';
  const mobileLbl = isF ? 'فهد 👦' : 'نورة 👧';
  ...
  return (
    <div className="dashboard-page">
      ...
      {/* Toast */}
      {toastMsg && (
        <div style={{position:'fixed',bottom:'24px',left:'50%',transform:'translateX(-50%)',background:'var(--c)',color:'#fff',padding:'10px 20px',borderRadius:'10px',fontSize:'13px',fontWeight:700,zIndex:999,fontFamily:'var(--f)',boxShadow:'0 4px 14px rgba(52,211,153,.3)'}}>
          {toastMsg}
        </div>
      )}
      ...
      {/* EXCUSE MODAL */}
      <div className={`modal-bg${excuseModal ? ' show' : ''}`} id="excuse-modal">
        <div className="modal">
          <div className="mh">
            <div className="mt">📋 تقديم عذر غياب</div>
            <button className="mx" onClick={() => setExcuseModal(false)}>×</button>
          </div>
          <div style={{padding:'16px'}}>
            ...
            <div style={{display:'flex',gap:'8px'}}>
              <button onClick={() => setExcuseModal(false)} style={{flex:1,background:'rgba(255,255,255,.05)',border:'1px solid var(--b1)',borderRadius:'9px',padding:'10px',color:'var(--td)',fontSize:'13px',cursor:'pointer',fontFamily:'var(--f)'}}>إلغاء</button>
              <button onClick={() => { toast('تم إرسال العذر للمدرسة ✓'); setExcuseModal(false); }} style={{flex:2,background:'linear-gradient(135deg,var(--c),var(--c2))',border:'none',borderRadius:'9px',padding:'10px',color:'#fff',fontWeight:800,fontSize:'13px',cursor:'pointer',fontFamily:'var(--f)'}}>إرسال ←</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* MESSAGE MODAL */}
      <div className={`modal-bg${msgModal ? ' show' : ''}`} id="msg-modal">
        <div className="modal">
          <div className="mh">
            <div className="mt">💬 رسالة للمعلم</div>
            <button className="mx" onClick={() => setMsgModal(false)}>×</button>
          </div>
          <div style={{padding:'16px'}}>
            ...
            <div style={{display:'flex',gap:'8px'}}>
              <button onClick={() => setMsgModal(false)} style={{flex:1,background:'rgba(255,255,255,.05)',border:'1px solid var(--b1)',borderRadius:'9px',padding:'10px',color:'var(--td)',fontSize:'13px',cursor:'pointer',fontFamily:'var(--f)'}}>إلغاء</button>
              <button onClick={() => { toast('تم إرسال الرسالة ✓'); setMsgModal(false); }} style={{flex:2,background:'linear-gradient(135deg,var(--c),var(--c2))',border:'none',borderRadius:'9px',padding:'10px',color:'#fff',fontWeight:800,fontSize:'13px',cursor:'pointer',fontFamily:'var(--f)'}}>إرسال ←</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* PAY MODAL */}
      <div className={`modal-bg${payModal ? ' show' : ''}`} id="pay-modal">
        <div className="modal">
          <div className="mh">
            <div className="mt">💳 دفع الرسوم الدراسية</div>
            <button className="mx" onClick={() => setPayModal(false)}>×</button>
          </div>
          <div style={{padding:'16px'}}>
            ...
            <div style={{display:'flex',gap:'8px',marginTop:'10px'}}>
              <button onClick={() => setPayModal(false)} style={{flex:1,background:'rgba(255,255,255,.05)',border:'1px solid var(--b1)',borderRadius:'9px',padding:'10px',color:'var(--td)',fontSize:'13px',cursor:'pointer',fontFamily:'var(--f)'}}>إلغاء</button>
              <button onClick={() => { toast('تم الدفع بنجاح ✓'); setPayModal(false); }} style={{flex:2,background:'linear-gradient(135deg,var(--c),var(--c2))',border:'none',borderRadius:'9px',padding:'10px',color:'#fff',fontWeight:800,fontSize:'13px',cursor:'pointer',fontFamily:'var(--f)'}}>ادفع الآن ←</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <aside className={`sb${sidebarOpen ? ' open' : ''}`} id="sb">
        <div className="sb-top">
          ...
          <div className="par-card">
            ...
            <div className={`child-row${activeChild === 'فهد' ? ' active' : ''}`} onClick={() => setActiveChild('فهد')}>
            ...
            <div className={`child-row${activeChild === 'نورة' ? ' active' : ''}`} onClick={() => setActiveChild('نورة')}>
              <div className="child-av">👧</div>
        ...
        <nav className="nav">
          ...
          <a className="ni" href="#" onClick={() => setExcuseModal(true)}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/></svg><span style={{color:'var(--c)',fontWeight:600}}>تقديم عذر غياب</span></a>
      ...
      <div className="main">
        <header className="hdr">
          <div className="hl">
            ...
            <div>
              <div className="ht" id="hdr-title">متابعة {childName}</div>
              <div className="hs">مدرسة الأمل الدولية — الفصل الثاني 1445/1446</div>
            </div>
          </div>
          <div className="hr2">
        ...
        <div className="con">
          ...
          {busNotifVisible && (
            <div id="bus-notif" style={{display:'flex',alignItems:'center',gap:'10px',background:'rgba(52,211,153,.07)',border:'1px solid rgba(52,211,153,.22)',borderRadius:'11px',padding:'10px 14px',marginBottom:'12px',fontSize:'12.5px',flexWrap:'wrap'}}>
              ...
              <button onClick={() => setBusNotifVisible(false)} style={{background:'none',border:'none',color:'var(--tm)',cursor:'pointer',fontSize:'18px'}}>×</button>
            </div>
          )}

          {/* CHILD BANNER */}
          <div className="child-banner" id="child-banner">
            <div className="cb-av" id="cb-av">{childAv}</div>
            <div className="cb-info">
              <div className="cb-name" id="cb-name">{childFullName}</div>
              <div className="cb-sub" id="cb-sub">{childSub}</div>
              <div className="cb-loc" id="cb-loc" style={{color:childLocColor}}>
                <div style={{width:'8px',height:'8px',borderRadius:'50%',background:childLocColor,boxShadow:`0 0 5px ${childLocColor}`}}></div>
                {childLoc}
              </div>
            </div>
            ...
            <div className="map-wrap" style={{marginBottom:0}}>
              ...
              <div className="map-body" style={{height:'165px'}}>
                ...
                <svg style={{position:'absolute',inset:0,width:'100%',height:'100%'}} viewBox="0 0 500 165" preserveAspectRatio="none">
                  ...
                  <path d="M 340 70 Q 380 68 420 66 Q 455 64 475 62" stroke="#60A5FA" strokeWidth="2.5" fill="none" strokeDasharray="6,4" opacity=".6"/>
                </svg>
                {/* Stops */}
            ...
            <div className="map-wrap" style={{marginBottom:0}}>
              ...
              <div className="map-body" style={{height:'165px'}}>
                ...
                <div className="pin" style={{right:'45%',top:'52%'}} id="mobile-pin">
                  <div className="pin-lbl" id="mobile-lbl">{mobileLbl}</div>
                  <div className="pin-ring" style={{width:'30px',height:'30px'}}>
                    <div className="pin-ic" style={{width:'16px',height:'16px',fontSize:'10px'}}>{childAv}</div>
                  </div>
                </div>
                {/* Location zones */}
                ...
                <div style={{position:'absolute',bottom:'6px',left:'10px',fontSize:'9.5px',color:'var(--tm)'}} id="mobile-time">{mobileTime}</div>
              </div>
              <div style={{padding:'8px 13px',borderTop:'1px solid var(--b2)',display:'flex',alignItems:'center',gap:'8px',background:'rgba(255,255,255,.01)'}}>
                <div style={{fontSize:'11px',color:'var(--td)'}}>مرتبط بجوال {isF ? 'فهد' : 'نورة'} · تحديث كل دقيقة</div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              ...
              <div className="card" style={{marginBottom:0}}>
                <div className="ch">
                  ...
                  <button className="cl" onClick={() => setMsgModal(true)} style={{color:'var(--bl)'}}>رد</button>
                </div>
                <div className="msg">
                ...
                <div style={{padding:'8px 13px',borderTop:'1px solid var(--b2)'}}>
                  <button onClick={() => setMsgModal(true)} style={{width:'100%',background:'var(--cd)',border:'1px solid var(--cb)',borderRadius:'7px',padding:'7px',color:'var(--c)',fontSize:'11px',fontWeight:700,cursor:'pointer',fontFamily:'var(--f)'}}>💬 راسل معلم</button>
                </div>
              </div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              ...
              <div className="card" style={{marginBottom:0}}>
                ...
                <div style={{padding:'12px 13px'}}>
                  ...
                  <button onClick={() => setPayModal(true)} style={{width:'100%',background:'linear-gradient(135deg,var(--c),var(--c2))',border:'none',borderRadius:'8px',padding:'9px',color:'#fff',fontSize:'12px',fontWeight:700,cursor:'pointer',fontFamily:'var(--f)'}}>💳 دفع المتبقي الآن</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="qg">
              ...
              <a className="qi" href="#" onClick={() => setExcuseModal(true)}><div className="qic" style={{background:'rgba(251,146,60,.1)',border:'1px solid rgba(251,146,60,.2)'}}>📋</div><span className="ql">تقديم عذر</span></a>
              <a className="qi" href="#" onClick={() => setMsgModal(true)}><div className="qic" style={{background:'rgba(167,139,250,.1)',border:'1px solid rgba(167,139,250,.2)'}}>💬</div><span className="ql">راسل المعلم</span></a>
              ...
              <a className="qi" href="#" onClick={() => setPayModal(true)}><div className="qic" style={{background:'rgba(239,68,68,.1)',border:'1px solid rgba(239,68,68,.2)'}}>💳</div><span className="ql">دفع الرسوم</span></a>
