...
const STATUS_CFG: Record<string, any> = {
  ...
  overdue: { label: 'متأخرة', color: '#EF4444', bg: 'rgba(239,68,68,0.1)', icon: '<Circle className="w-5 h-5 inline-block" />' },
  partial: { label: 'جزئية', color: '#3B82F6', bg: 'rgba(59,130,246,0.1)', icon: '<Zap className="w-5 h-5 inline-block" />' },
  paid:    { label: 'مدفوعة', color: '#10B981', bg: 'rgba(16,185,129,0.1)', icon: '<CheckCircle className="w-5 h-5 inline-block" />' },
};

const METHODS = [
  { key: 'moyasar', label: 'بطاقة بنكية', icon: '<CreditCard className="w-5 h-5 inline-block" />', desc: 'Visa / Mastercard / Mada' },
  { key: 'tabby', label: 'تابي (4 أقساط)', icon: '<Circle className="w-5 h-5 inline-block" />', desc: 'اشتري الآن وادفع لاحقاً' },
  { key: 'tamara', label: 'تمارا', icon: '<Circle className="w-5 h-5 inline-block" />', desc: 'قسّم على 3 أشهر' },
  { key: 'stc_pay', label: 'STC Pay', icon: '<Smartphone className="w-5 h-5 inline-block" />', desc: 'محفظتك الرقمية' },
  { key: 'bank_transfer', label: 'تحويل بنكي', icon: '<Landmark className="w-5 h-5 inline-block" />', desc: 'تحويل مباشر' },
  { key: 'cash', label: 'نقداً', icon: '<Banknote className="w-5 h-5 inline-block" />', desc: 'الدفع في المدرسة' },
];
...
export default function ParentPaymentsPage() {
  ...
  useEffect(() => {
    ...
    if (s === 'success') setMsg({ text: '<CheckCircle className="w-5 h-5 inline-block" /> تم الدفع بنجاح!', type: 'success' });
    else if (s === 'failed') setMsg({ text: '<XCircle className="w-5 h-5 inline-block" /> فشل الدفع، حاول مجدداً', type: 'error' });
  }, []);
  ...
  if (loading) return <div style={{ padding: 24, direction: 'rtl', background: '#06060E', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8B5CF6', fontSize: 18 }}><CreditCard className="w-5 h-5 inline-block" /> جاري التحميل...</div>;

  return (
    <div style={{ padding: 24, direction: 'rtl', fontFamily: 'IBM Plex Sans Arabic,Arial,sans-serif', background: '#06060E', minHeight: '100vh' }}>
      <h1 style={{ color: '#8B5CF6', fontSize: 24, fontWeight: 800, margin: '0 0 4px' }}><CreditCard className="w-5 h-5 inline-block" /> الفواتير والدفع</h1>
      ...
      {data?.summary && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 12, marginBottom: 24 }}>
          {[{ l: 'المستحق', v: `${Number(data.summary.total_due || 0).toLocaleString()} ر`, c: '#EF4444', i: '<ClipboardList className="w-5 h-5 inline-block" />' }, { l: 'فواتير معلّقة', v: data.summary.pending_count || 0, c: '#F59E0B', i: '⏳' }, { l: 'المدفوع', v: `${Number(data.summary.total_paid || 0).toLocaleString()} ر`, c: '#10B981', i: '<CheckCircle className="w-5 h-5 inline-block" />' }].map(st => (
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, background: 'rgba(255,255,255,0.04)', borderRadius: 12, padding: 6 }}>
        {[{ k: 'invoices', l: '<ClipboardList className="w-5 h-5 inline-block" /> الفواتير' }, { k: 'history', l: '<Clock className="w-5 h-5 inline-block" /> سجل المدفوعات' }].map(t => (
        ...
        <div>
          ...
          {filtered.length === 0 ? (
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 40, textAlign: 'center' }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}><PartyPopper className="w-5 h-5 inline-block" /></div>
              ...
              <div key={inv.id} onClick={() => canPay && openPayment(inv)} style={{ background: cfg.bg, border: `1px solid ${cfg.color}30`, borderRadius: 14, padding: 18, marginBottom: 12, cursor: canPay ? 'pointer' : 'default' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                  ...
                  <div style={{ textAlign: 'center', minWidth: 120 }}>
                    ...
                    {canPay && <div style={{ marginTop: 10 }}><button style={{ padding: '8px 20px', borderRadius: 8, cursor: 'pointer', background: 'linear-gradient(135deg,#8B5CF6,#6D28D9)', border: 'none', color: 'white', fontSize: 13, fontWeight: 700, width: '100%' }}><CreditCard className="w-5 h-5 inline-block" /> ادفع الآن</button></div>}
                  </div>
                </div>
              </div>
            );
      ...
      {activeTab === 'history' && (
        <div>
          {history.length === 0 ? <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 40, textAlign: 'center', color: 'rgba(255,255,255,0.5)' }}><Mailbox className="w-5 h-5 inline-block" /> لا توجد مدفوعات سابقة</div>
          : history.map((p: any) => (
            <div key={p.id} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 18, marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
              <div>
                ...
                <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, marginTop: 4 }}>{p.student_name && `<User className="w-5 h-5 inline-block" />‍<GraduationCap className="w-5 h-5 inline-block" /> ${p.student_name} — `}<Calendar className="w-5 h-5 inline-block" /> {new Date(p.created_at).toLocaleDateString('ar-SA')}</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ color: '#10B981', fontSize: 22, fontWeight: 800 }}>{Number(p.amount).toLocaleString()} ريال</div>
                <span style={{ display: 'inline-block', padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700, background: p.status === 'completed' ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)', color: p.status === 'completed' ? '#10B981' : '#EF4444' }}>{p.status === 'completed' ? '<CheckCircle className="w-5 h-5 inline-block" /> مكتمل' : '<XCircle className="w-5 h-5 inline-block" /> فشل'}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#06060E', border: '1px solid rgba(139,92,246,0.3)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto', direction: 'rtl' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ color: '#8B5CF6', fontSize: 18, fontWeight: 800, margin: 0 }}><CreditCard className="w-5 h-5 inline-block" /> إتمام الدفع</h3>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={processPayment} disabled={!paymentMethod || paying} style={{ flex: 2, padding: 13, borderRadius: 10, cursor: 'pointer', background: paymentMethod ? 'linear-gradient(135deg,#8B5CF6,#6D28D9)' : 'rgba(255,255,255,0.1)', border: 'none', color: paymentMethod ? 'white' : 'rgba(255,255,255,0.3)', fontSize: 14, fontWeight: 700, opacity: paying ? 0.7 : 1 }}>
                {paying ? '⏳ جاري المعالجة...' : `<CreditCard className="w-5 h-5 inline-block" /> ادفع ${Number(selectedInvoice.total).toLocaleString()} ريال`}
              </button>
            </div>
          </div>
        </div>
      ...
      {showModal && selectedInvoice && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#0F0F1A', border: '1px solid rgba(201,162,39,0.2)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 440, direction: 'rtl' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ color: '#C9A227', fontSize: 18, fontWeight: 700, margin: 0 }}><CreditCard className="w-5 h-5 inline-block" /> تفاصيل الفاتورة</h2>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: 16, marginBottom: 20 }}>
              ...
              <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13 }}>الحالة: {selectedInvoice.status === 'paid' ? '<CheckCircle className="w-5 h-5 inline-block" /> مدفوعة' : selectedInvoice.status === 'pending' ? '⏳ معلقة' : '<Circle className="w-5 h-5 inline-block" /> متأخرة'}</div>
            ...
            {selectedInvoice.status !== 'paid' && (
              ...
              }} disabled={saving} style={{ width: '100%', background: saving ? 'rgba(16,185,129,0.3)' : 'rgba(16,185,129,0.15)', color: '#10B981', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 10, padding: '12px 0', cursor: saving ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: 14, marginBottom: 10 }}>{saving ? 'جاري...' : '<CheckCircle className="w-5 h-5 inline-block" /> تأكيد الدفع'}</button>
            )}
