...
export default function CircularsPage() {
  ...
  const typeIcons: any = { general: "ICON_File", academic: "ICON_BookOpen", administrative: "ICON_Building", financial: "ICON_Coins", event: "ICON_PartyPopper", holiday: 'Palmtree<IconRenderer name="ICON_Star" size={18} />', policy: "ICON_ScrollText", safety: 'Shield<IconRenderer name="ICON_Star" size={18} />' };
          ...
          <div key={item.id} onClick={() => setViewItem(item)} style={{ background: item.priority === 'urgent' ? 'rgba(239,68,68,0.04)' : 'rgba(255,255,255,0.03)', border: `1px solid ${item.priority === 'urgent' ? 'rgba(239,68,68,0.15)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 14, padding: '18px 22px', cursor: 'pointer', borderRight: `4px solid ${typeColors[item.type]?.color || '#3B82F6'}` }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div style={{ display: 'flex', gap: 14, flex: 1 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    {item.sender && <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}><IconRenderer name="ICON_Star" size={18} /> {item.sender}</span>}
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                <button onClick={(e) => { e.stopPropagation(); handleEdit(item); }} style={{ background: 'rgba(201,162,39,0.1)', color: '#C9A227', border: 'none', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}><IconRenderer name="ICON_Star" size={18} /></button>
                <button onClick={(e) => { e.stopPropagation(); handleDelete(item.id); }} style={{ background: 'rgba(239,68,68,0.1)', color: '#EF4444', border: 'none', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}><IconRenderer name="ICON_Star" size={18} /></button>
              </div>
            </div>
          </div>
        ))}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} style={inputStyle}>
                  ...
                  <option value="holiday">إجازة Palmtree<IconRenderer name="ICON_Star" size={18} /></option>
                  <option value="policy">سياسة ScrollText</option>
                  <option value="safety">أمان Shield<IconRenderer name="ICON_Star" size={18} /></option>
                </select>
              </div>
              <div>
