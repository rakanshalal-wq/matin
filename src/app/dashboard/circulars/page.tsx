...
export default function CircularsPage() {
  ...
  const typeIcons: any = { general: "ICON_File", academic: "ICON_BookOpen", administrative: "ICON_Building", financial: "ICON_Coins", event: "ICON_PartyPopper", holiday: 'Palmtree️', policy: "ICON_ScrollText", safety: 'Shield️' };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}>Megaphone التعاميم</h1>
      ...
      <div style={{ marginBottom: 20 }}>
        <input placeholder="Search بحث بالعنوان أو المرسل أو المحتوى..." value={search} onChange={e => setSearch(e.target.value)} style={{ ...inputStyle, maxWidth: 400 }} />
      </div>
              ...
              <div style={{ display: 'flex', gap: 14, flex: 1 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                    ...
                    {item.priority === 'urgent' && <span style={{ background: 'rgba(239,68,68,0.1)', color: '#EF4444', padding: '2px 8px', borderRadius: 6, fontSize: 10, fontWeight: 700 }}>Circle عاجل</span>}
                  ...
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    {item.sender && <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}>PenTool️ {item.sender}</span>}
                    <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}>Users {audienceLabels[item.target_audience] || item.target_audience}</span>
                    {item.publish_date && <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}>Calendar {new Date(item.publish_date).toLocaleDateString('ar-SA')}</span>}
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                <button onClick={(e) => { e.stopPropagation(); handleEdit(item); }} style={{ background: 'rgba(201,162,39,0.1)', color: '#C9A227', border: 'none', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>Pencil️</button>
                <button onClick={(e) => { e.stopPropagation(); handleDelete(item.id); }} style={{ background: 'rgba(239,68,68,0.1)', color: '#EF4444', border: 'none', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>Trash2️</button>
              </div>
            </div>
          </div>
        ))}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#06060E', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 650, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 20, marginBottom: 16, padding: '12px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: 10 }}>
              ...
              {viewItem.publish_date && <div><span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13 }}>Calendar </span><span style={{ color: 'rgba(255,255,255,0.7)' }}>{new Date(viewItem.publish_date).toLocaleDateString('ar-SA')}</span></div>}
            </div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#06060E', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? 'Pencil️ تعديل تعميم' : 'Plus تعميم جديد'}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} style={inputStyle}>
                  ...
                  <option value="holiday">إجازة Palmtree️</option>
                  <option value="policy">سياسة ScrollText</option>
                  <option value="safety">أمان Shield️</option>
                </select>
              </div>
              <div>
