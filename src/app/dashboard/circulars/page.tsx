...
export default function CircularsPage() {
  ...
  const typeIcons: any = { general: '<File className="w-5 h-5 inline-block" />', academic: '<BookOpen className="w-5 h-5 inline-block" />', administrative: '<Building className="w-5 h-5 inline-block" />', financial: '<Coins className="w-5 h-5 inline-block" />', event: '<PartyPopper className="w-5 h-5 inline-block" />', holiday: '<Palmtree className="w-5 h-5 inline-block" />️', policy: '<ScrollText className="w-5 h-5 inline-block" />', safety: '<Shield className="w-5 h-5 inline-block" />️' };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'white', margin: 0 }}><Megaphone className="w-5 h-5 inline-block" /> التعاميم</h1>
        ...
        <button onClick={() => { setEditItem(null); setForm({ title: '', type: 'general', content: '', sender: '', target_audience: 'all', priority: 'normal', publish_date: '', status: 'draft' }); setShowModal(true); }} style={{ background: 'linear-gradient(135deg, #C9A227 0%, #D4B03D 100%)', color: '#06060E', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer', fontSize: 15 }}>
          <Plus className="w-5 h-5 inline-block" /> تعميم جديد
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي التعاميم', value: stats.total, icon: '<Megaphone className="w-5 h-5 inline-block" />', color: '#C9A227' },
          { label: 'منشور', value: stats.published, icon: '<CheckCircle className="w-5 h-5 inline-block" />', color: '#10B981' },
          { label: 'مسودة', value: stats.draft, icon: '<FileText className="w-5 h-5 inline-block" />', color: '#6B7280' },
          { label: 'عاجل', value: stats.urgent, icon: '<Siren className="w-5 h-5 inline-block" />', color: '#EF4444' },
        ].map((stat, i) => (
      ...
      <div style={{ marginBottom: 20 }}>
        <input placeholder="<Search className="w-5 h-5 inline-block" /> بحث بالعنوان أو المرسل أو المحتوى..." value={search} onChange={e => setSearch(e.target.value)} style={{ ...inputStyle, maxWidth: 400 }} />
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center', background: 'rgba(255,255,255,0.03)', borderRadius: 12 }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}><Megaphone className="w-5 h-5 inline-block" /></div>
          ...
          <div key={item.id} onClick={() => setViewItem(item)} style={{ background: item.priority === 'urgent' ? 'rgba(239,68,68,0.04)' : 'rgba(255,255,255,0.03)', border: `1px solid ${item.priority === 'urgent' ? 'rgba(239,68,68,0.15)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 14, padding: '18px 22px', cursor: 'pointer', borderRight: `4px solid ${typeColors[item.type]?.color || '#3B82F6'}` }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div style={{ display: 'flex', gap: 14, flex: 1 }}>
                <div style={{ width: 48, height: 48, borderRadius: 12, background: typeColors[item.type]?.bg || 'rgba(59,130,246,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>{typeIcons[item.type] || '<File className="w-5 h-5 inline-block" />'}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                    <span style={{ color: 'white', fontWeight: 700, fontSize: 16 }}>{item.title}</span>
                    {item.priority === 'urgent' && <span style={{ background: 'rgba(239,68,68,0.1)', color: '#EF4444', padding: '2px 8px', borderRadius: 6, fontSize: 10, fontWeight: 700 }}><Circle className="w-5 h-5 inline-block" /> عاجل</span>}
                  ...
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    {item.sender && <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}><PenTool className="w-5 h-5 inline-block" />️ {item.sender}</span>}
                    <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}><Users className="w-5 h-5 inline-block" /> {audienceLabels[item.target_audience] || item.target_audience}</span>
                    {item.publish_date && <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}><Calendar className="w-5 h-5 inline-block" /> {new Date(item.publish_date).toLocaleDateString('ar-SA')}</span>}
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                <button onClick={(e) => { e.stopPropagation(); handleEdit(item); }} style={{ background: 'rgba(201,162,39,0.1)', color: '#C9A227', border: 'none', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}><Pencil className="w-5 h-5 inline-block" />️</button>
                <button onClick={(e) => { e.stopPropagation(); handleDelete(item.id); }} style={{ background: 'rgba(239,68,68,0.1)', color: '#EF4444', border: 'none', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}><Trash2 className="w-5 h-5 inline-block" />️</button>
              </div>
            </div>
          </div>
        ))}
          ...
          <div style={{ background: '#06060E', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 650, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 28 }}>{typeIcons[viewItem.type] || '<File className="w-5 h-5 inline-block" />'}</span>
                <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{viewItem.title}</h2>
              </div>
              <button onClick={() => setViewItem(null)} style={{ background: 'rgba(239,68,68,0.1)', color: '#EF4444', border: 'none', borderRadius: 8, width: 32, height: 32, cursor: 'pointer', fontSize: 16 }}><X className="w-5 h-5 inline-block" /></button>
            </div>
            ...
            <div style={{ display: 'flex', gap: 20, marginBottom: 16, padding: '12px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: 10 }}>
              ...
              {viewItem.publish_date && <div><span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13 }}><Calendar className="w-5 h-5 inline-block" /> </span><span style={{ color: 'rgba(255,255,255,0.7)' }}>{new Date(viewItem.publish_date).toLocaleDateString('ar-SA')}</span></div>}
            </div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#06060E', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '90%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <h2 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: 0 }}>{editItem ? '<Pencil className="w-5 h-5 inline-block" />️ تعديل تعميم' : '<Plus className="w-5 h-5 inline-block" /> تعميم جديد'}</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} style={inputStyle}>
                  <option value="general">عام <File className="w-5 h-5 inline-block" /></option>
                  <option value="academic">أكاديمي <BookOpen className="w-5 h-5 inline-block" /></option>
                  <option value="administrative">إداري <Building className="w-5 h-5 inline-block" /></option>
                  <option value="financial">مالي <Coins className="w-5 h-5 inline-block" /></option>
                  <option value="event">فعالية <PartyPopper className="w-5 h-5 inline-block" /></option>
                  <option value="holiday">إجازة <Palmtree className="w-5 h-5 inline-block" />️</option>
                  <option value="policy">سياسة <ScrollText className="w-5 h-5 inline-block" /></option>
                  <option value="safety">أمان <Shield className="w-5 h-5 inline-block" />️</option>
                </select>
              </div>
              <div>
              ...
              <div>
                ...
                <select value={form.target_audience} onChange={e => setForm({ ...form, target_audience: e.target.value })} style={inputStyle}>
                  <option value="all">الجميع <Users className="w-5 h-5 inline-block" /></option>
                  <option value="students">الطلاب <GraduationCap className="w-5 h-5 inline-block" /></option>
                  <option value="teachers">المعلمين <User className="w-5 h-5 inline-block" />‍<School className="w-5 h-5 inline-block" /></option>
                  <option value="parents">أولياء الأمور <User className="w-5 h-5 inline-block" />‍<User className="w-5 h-5 inline-block" />‍<User className="w-5 h-5 inline-block" /></option>
                  <option value="employees">الموظفين <Shirt className="w-5 h-5 inline-block" /></option>
                </select>
              </div>
              <div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24, justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleSubmit} style={{ background: 'linear-gradient(135deg, #C9A227 0%, #D4B03D 100%)', color: '#06060E', border: 'none', borderRadius: 10, padding: '12px 24px', cursor: 'pointer', fontWeight: 700 }}>{editItem ? '<Save className="w-5 h-5 inline-block" /> تحديث' : '<Megaphone className="w-5 h-5 inline-block" /> نشر'}</button>
            </div>
          </div>
        </div>
      )}
