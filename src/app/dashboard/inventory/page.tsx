...
export default function InventoryPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, padding: '32px 24px', direction: 'rtl', fontFamily: 'Cairo, sans-serif' }}>
      ...
      {lowStock.length > 0 && <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 12, padding: '14px 18px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}><span style={{ fontSize: 20 }}><IconRenderer name="ICON_Star" size={18} /></span><span style={{ color: '#F59E0B', fontSize: 14 }}><strong>{lowStock.length} أصناف</strong> وصلت للحد الأدنى: {lowStock.map((r: any) => r.name).join('، ')}</span></div>}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 16, marginBottom: 28 }}>
        {[{ l: 'اجمالي الاصناف', v: items.length, c: GOLD, i: "ICON_Package" }, { l: 'مخزون كافي', v: items.length - lowStock.length, c: '#10B981', i: "ICON_CheckCircle" }, { l: 'مخزون منخفض', v: lowStock.length, c: '#F59E0B', i: 'AlertTriangle<IconRenderer name="ICON_Star" size={18} />' }, { l: 'اجمالي القيمة', v: items.reduce((s: number, r: any) => s + (Number(r.price || 0) * Number(r.quantity || 0)), 0).toFixed(0) + ' ر.س', c: '#3B82F6', i: "ICON_Coins" }].map((s, i) => (
      ...
      <div style={{ background: CB, border: '1px solid ' + BR, borderRadius: 16, overflow: 'hidden' }}>
        {loading ? <div style={{ textAlign: 'center', padding: 60, color: 'rgba(255,255,255,0.4)' }}>جاري التحميل...</div> : filtered.length === 0 ? <div style={{ textAlign: 'center', padding: 60 }}><div style={{ fontSize: 48, marginBottom: 16 }}><IconRenderer name="ICON_Package" size={36} /></div><p style={{ color: 'rgba(255,255,255,0.4)' }}>لا توجد اصناف</p><button onClick={() => setShowModal(true)} style={{ background: GOLD, border: 'none', borderRadius: 10, padding: '10px 24px', color: '#0B0B16', fontWeight: 700, cursor: 'pointer', marginTop: 16 }}>+ اضافة</button></div> :
