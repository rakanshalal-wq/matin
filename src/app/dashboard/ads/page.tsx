...
export default function AdsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setMsg(editItem ? '<CheckCircle className="w-5 h-5 inline-block" /> تم تعديل الإعلان' : '<CheckCircle className="w-5 h-5 inline-block" /> تم إضافة الإعلان بنجاح');
        setMsgType('success');
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setMsg('<CheckCircle className="w-5 h-5 inline-block" /> تم حذف الإعلان');
      setMsgType('success');
      setTimeout(() => setMsg(''), 3000);
    } catch {}
  };

  const positionLabels: Record<string, string> = { top: '⬆️ أعلى الصفحة', bottom: '⬇️ أسفل الصفحة', sidebar: '◀️ الشريط الجانبي', all: '<Globe className="w-5 h-5 inline-block" /> في كل مكان' };
  const targetLabels: Record<string, string> = { all: '<Users className="w-5 h-5 inline-block" /> الجميع', schools: '<School className="w-5 h-5 inline-block" /> المدارس', universities: '<GraduationCap className="w-5 h-5 inline-block" /> الجامعات', institutes: '<BookOpen className="w-5 h-5 inline-block" /> المعاهد' };
  ...
  return (
    <div style={{ padding: '24px', direction: 'rtl', fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: '#C9A227', margin: 0 }}><Megaphone className="w-5 h-5 inline-block" /> الإعلانات</h1>
        ...
        <button onClick={() => setShowAdd(!showAdd)} style={{ padding: '10px 24px', background: showAdd ? '#374151' : 'linear-gradient(135deg, #C9A227, #E8C547)', color: showAdd ? '#fff' : '#000', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>
          {showAdd ? '<X className="w-5 h-5 inline-block" /> إلغاء' : '+ إضافة إعلان'}
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'الكل', value: ads.length, color: '#C9A227', icon: '<Megaphone className="w-5 h-5 inline-block" />' },
          { label: 'نشطة', value: ads.filter(a => a.is_active).length, color: '#10B981', icon: '<CheckCircle className="w-5 h-5 inline-block" />' },
          { label: 'إجمالي المشاهدات', value: ads.reduce((s, a) => s + (a.views || 0), 0), color: '#3B82F6', icon: '<Eye className="w-5 h-5 inline-block" />️' },
          { label: 'إجمالي النقرات', value: ads.reduce((s, a) => s + (a.clicks || 0), 0), color: '#8B5CF6', icon: '<Mouse className="w-5 h-5 inline-block" />️' },
        ].map((s, i) => (
      ...
      {showAdd && (
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(201,162,39,0.2)', borderRadius: 16, padding: 24, marginBottom: 24 }}>
          <h3 style={{ color: '#C9A227', fontSize: 18, margin: '0 0 20px', fontWeight: 700 }}>{editItem ? '<Pencil className="w-5 h-5 inline-block" />️ تعديل الإعلان' : '<Plus className="w-5 h-5 inline-block" /> إضافة إعلان جديد'}</h3>
          ...
          <button onClick={handleSave} disabled={saving} style={{ marginTop: 20, padding: '12px 32px', background: 'linear-gradient(135deg, #C9A227, #E8C547)', color: '#000', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'IBM Plex Sans Arabic, sans-serif', opacity: saving ? 0.5 : 1 }}>
            {saving ? '⏳ جاري الحفظ...' : editItem ? '<Save className="w-5 h-5 inline-block" /> حفظ التعديلات' : '<Megaphone className="w-5 h-5 inline-block" /> نشر الإعلان'}
          </button>
        </div>
      )}
              ...
              <div style={{ padding: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                  ...
                  <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600, background: ad.is_active ? 'rgba(16,185,129,0.1)' : 'rgba(107,114,128,0.1)', color: ad.is_active ? '#10B981' : '#6B7280', flexShrink: 0, marginRight: 8 }}>
                    {ad.is_active ? '<Check className="w-5 h-5 inline-block" /> نشط' : '⏸ موقوف'}
                  </span>
                </div>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
                  ...
                  <span style={{ color: '#6B7280', fontSize: 12 }}><Eye className="w-5 h-5 inline-block" />️ {(ad.views || 0).toLocaleString()}</span>
                  <span style={{ color: '#6B7280', fontSize: 12 }}><Mouse className="w-5 h-5 inline-block" />️ {(ad.clicks || 0).toLocaleString()}</span>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={() => handleEdit(ad)} style={{ padding: '7px 14px', background: 'rgba(201,162,39,0.1)', color: '#C9A227', border: '1px solid rgba(201,162,39,0.2)', borderRadius: 8, fontSize: 12, cursor: 'pointer', fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}><Pencil className="w-5 h-5 inline-block" />️</button>
