...
export default function AdsPage() {
  ...
  const positionLabels: Record<string, string> = { top: '⬆️ أعلى الصفحة', bottom: '⬇️ أسفل الصفحة', sidebar: '◀️ الشريط الجانبي', all: '🌐 في كل مكان' };
  const targetLabels: Record<string, string> = { all: '👥 الجميع', schools: '🏫 المدارس', universities: '🎓 الجامعات', institutes: '📚 المعاهد' };
  ...
  return (
    <div style={{ padding: '24px', direction: 'rtl', fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: '#C9A227', margin: 0 }}>📢 الإعلانات</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'الكل', value: ads.length, color: '#C9A227', icon: '📢' },
          { label: 'نشطة', value: ads.filter(a => a.is_active).length, color: '#10B981', icon: '✅' },
          { label: 'إجمالي المشاهدات', value: ads.reduce((s, a) => s + (a.views || 0), 0), color: '#3B82F6', icon: '👁️' },
          { label: 'إجمالي النقرات', value: ads.reduce((s, a) => s + (a.clicks || 0), 0), color: '#8B5CF6', icon: '🖱️' },
        ].map((s, i) => (
            ...
            <div key={ad.id} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${ad.is_active ? 'rgba(16,185,129,0.2)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 14, overflow: 'hidden' }}>
              ...
              <div style={{ padding: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
                  ...
                  <span style={{ color: '#6B7280', fontSize: 12 }}>👁️ {(ad.views || 0).toLocaleString()}</span>
                  <span style={{ color: '#6B7280', fontSize: 12 }}>🖱️ {(ad.clicks || 0).toLocaleString()}</span>
                </div>
