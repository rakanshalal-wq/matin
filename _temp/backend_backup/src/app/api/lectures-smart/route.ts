...
export async function POST(request: Request) {
  ...
  if (action === 'confirm_attendance') {
    ...
    const lectureResult = await pool.query(
      `SELECT * FROM lectures WHERE id = $1 AND school_id = $2`,
  ...
  if (action === 'send_reminders') {
    ...
    const lectures3h = await pool.query(
      `SELECT l.*, s.name as school_name FROM lectures l
       JOIN schools s ON s.id = l.school_id::text
      ...
      await pool.query(
        `INSERT INTO notifications (user_id, title, message, type, school_id, owner_id, created_at)
         SELECT u.id, 
                ...
                $2, $3, NOW()
         FROM users u
         WHERE u.school_id = $4 AND u.role IN ('teacher', 'student') AND u.status = 'active'`,
        [lecture.title, lecture.school_id, lecture.owner_id, parseInt(lecture.school_id)]
      );
      
      await pool.query(
        `UPDATE lectures SET reminder_sent_3h = true WHERE id = $1`,
        [lecture.id]
      );
      reminders_sent++;
    }
    ...
    for (const lecture of lectures1h.rows) {
      ...
      await pool.query(
        `UPDATE lectures SET reminder_sent_1h = true WHERE id = $1`,
        [lecture.id]
      );
      reminders_sent++;
    }
    ...
    for (const lecture of lectures15m.rows) {
      ...
      await pool.query(
        `UPDATE lectures SET reminder_sent_15m = true WHERE id = $1`,
        [lecture.id]
      );
      reminders_sent++;
    }
  ...
  if (action === 'generate_post_questions') {
    ...
    // تحديث حالة المحاضرة
    await pool.query(
      `UPDATE lectures SET post_lecture_questions_sent = true, ai_questions = $1 WHERE id = $2`,
    ...
    await pool.query(
      `INSERT INTO notifications (user_id, title, message, type, school_id, owner_id, created_at)
       SELECT u.id,
              ...
              $2, $3, NOW()
       FROM users u
       WHERE u.school_id = $4 AND u.role = 'student' AND u.status = 'active'`,
  ...
  if (action === 'answer_post_question') {
    ...
    const questionResult = await pool.query(
      `SELECT * FROM lecture_post_questions WHERE id = $1`,
      [question_id]
    );
  ...
  if (action === 'post_questions' && lecture_id) {
    const questions = await pool.query(
      `SELECT lpq.*, 
       ...
       LEFT JOIN lecture_post_answers lpa ON lpa.question_id = lpq.id AND lpa.student_id = $1
