...
export default function LecturesPage() {
 ...
 const [liveLoading, setLiveLoading] = useState(false);
  ...
  const { data: lecturesData, isLoading: lecturesLoading } = useQuery({
    queryKey: ['lectures-lectures'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/lectures', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: liveSessionsData, isLoading: liveSessionsLoading } = useQuery({
    queryKey: ['lectures-live'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/live-sessions', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
 ...
 { label: 'فيديو مسجل', value: lectures.filter((l: any) => l.type === 'video').length, icon: "ICON_Video", color: '#8B5CF6' },
 { label: 'بث مباشر', value: liveSessions.filter((l: any) => l.status === 'live').length, icon: "ICON_Circle", color: '#EF4444' },
 ...
 const typeConf = LECTURE_TYPES.find((t: any) => t.value === lec.type) || LECTURE_TYPES[0];
 return (
 ...
 {LECTURE_TYPES.map((t: any) => <option key={t.value} value={t.value}><IconRenderer name={t.icon} /> {t.label}</option>)}
 </select>
 </div>
