...
import { DashboardHomeSkeleton } from '@/components/DashboardSkeleton';
...
export default function DashboardHome() {
 ...
 // ⚡ React Query - dashboard stats (cache 2 دقيقة)
 ...
 // ⚡ React Query - school info (cache 5 دقائق)
 const { data: schoolData } = useQuery({
   queryKey: ['dashboard-school'],
   queryFn: async () => {
     const r = await fetch('/api/schools', { headers: getHeaders() });
     const d = await r.json();
     const schools = Array.isArray(d) ? d : [];
 ...
 // ⚡ React Query - upcoming exams (cache 3 دقائق)
 const { data: upcomingExams = [] } = useQuery({
   queryKey: ['dashboard-exams'],
   queryFn: async () => {
     const r = await fetch('/api/exams?limit=5', { headers: getHeaders() });
     const d = await r.json();
 ...
 // ⚡ React Query - pending admissions (cache دقيقتان)
 const { data: pendingAdmissions = [] } = useQuery({
   queryKey: ['dashboard-admissions'],
   queryFn: async () => {
     const r = await fetch('/api/admission?status=pending&limit=5', { headers: getHeaders() });
     const d = await r.json();
     return Array.isArray(d) ? d.slice(0, 5) : [];
   },
   enabled: !!user,
 ...
 if (res.ok) {
   queryClient.invalidateQueries({ queryKey: ['dashboard-admissions'] });
 ...
 // ⚡ React Query يتولى جلب البيانات تلقائياً عند تعيين user
 setLoading(false);
 };
