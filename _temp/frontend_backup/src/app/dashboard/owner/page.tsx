...
export default function OwnerDashboard() {
 ...
 // ⚡ React Query hooks - كل API يُجلب بشكل مستقل مع cache ذكي
 const { data: statsData } = useQuery({
   queryKey: ['dash-owner-stats'],
   queryFn: async () => { const r = await fetch('/api/dashboard-stats', { headers: getH() }); return r.json(); },
   enabled: !!user, staleTime: 2 * 60 * 1000,
 });
 const stats = statsData || {};

 const { data: schoolsData } = useQuery({
   queryKey: ['dash-owner-schools'],
   queryFn: async () => { const r = await fetch('/api/schools?limit=50', { headers: getH() }); const d = await r.json(); return Array.isArray(d) ? d : d?.schools || []; },
   enabled: !!user, staleTime: 3 * 60 * 1000,
 });
 const schools = schoolsData || [];

 const { data: joinRequestsData } = useQuery({
   queryKey: ['dash-owner-join-requests'],
   queryFn: async () => { const r = await fetch('/api/join-requests?status=pending&limit=20', { headers: getH() }); const d = await r.json(); return Array.isArray(d) ? d : d?.requests || []; },
   enabled: !!user, staleTime: 2 * 60 * 1000,
 });
 const joinRequests = joinRequestsData || [];

 const { data: plansData } = useQuery({
   queryKey: ['dash-owner-plans'],
   queryFn: async () => { const r = await fetch('/api/plans', { headers: getH() }); const d = await r.json(); return Array.isArray(d) ? d : d?.plans || []; },
   enabled: !!user, staleTime: 5 * 60 * 1000,
 });
 const plans = plansData || [];

 const { data: couponsData } = useQuery({
   queryKey: ['dash-owner-coupons'],
   queryFn: async () => { const r = await fetch('/api/coupons', { headers: getH() }); const d = await r.json(); return Array.isArray(d) ? d : d?.coupons || []; },
   enabled: !!user, staleTime: 5 * 60 * 1000,
 });
 const coupons = couponsData || [];

 const { data: taxesData } = useQuery({
   queryKey: ['dash-owner-taxes'],
   queryFn: async () => { const r = await fetch('/api/taxes', { headers: getH() }); const d = await r.json(); return Array.isArray(d) ? d : d?.taxes || []; },
   enabled: !!user, staleTime: 5 * 60 * 1000,
 });
 const taxes = taxesData || [];

 const { data: supportData } = useQuery({
   queryKey: ['dash-owner-support'],
   queryFn: async () => { const r = await fetch('/api/support?limit=20', { headers: getH() }); const d = await r.json(); return Array.isArray(d) ? d : d?.tickets || []; },
   enabled: !!user, staleTime: 2 * 60 * 1000,
 });
 const support = supportData || [];

 const { data: auditLogData } = useQuery({
   queryKey: ['dash-owner-audit'],
   queryFn: async () => { const r = await fetch('/api/activity-log?limit=20', { headers: getH() }); const d = await r.json(); return Array.isArray(d) ? d : d?.logs || []; },
   enabled: !!user, staleTime: 3 * 60 * 1000,
 });
 const auditLog = auditLogData || [];

 const { data: adsData } = useQuery({
   queryKey: ['dash-owner-ads'],
   queryFn: async () => { const r = await fetch('/api/advertisements', { headers: getH() }); const d = await r.json(); return Array.isArray(d) ? d : d?.ads || []; },
   enabled: !!user, staleTime: 5 * 60 * 1000,
 });
 const ads = adsData || [];

 const { data: commissionsData } = useQuery({
   queryKey: ['dash-owner-commissions'],
   queryFn: async () => { const r = await fetch('/api/commissions?limit=10', { headers: getH() }); const d = await r.json(); return Array.isArray(d) ? d : d?.commissions || []; },
   enabled: !!user, staleTime: 5 * 60 * 1000,
 });
 const commissions = commissionsData || [];

 // ⚡ loadAll يُحدّث الـ cache فقط بدلاً من إعادة الجلب
 const loadAll = useCallback(() => {
   queryClient.invalidateQueries({ queryKey: ['dash-owner-stats'] });
   queryClient.invalidateQueries({ queryKey: ['dash-owner-schools'] });
   queryClient.invalidateQueries({ queryKey: ['dash-owner-join-requests'] });
   queryClient.invalidateQueries({ queryKey: ['dash-owner-plans'] });
   queryClient.invalidateQueries({ queryKey: ['dash-owner-coupons'] });
   queryClient.invalidateQueries({ queryKey: ['dash-owner-support'] });
   queryClient.invalidateQueries({ queryKey: ['dash-owner-ads'] });
 }, [queryClient]);

 // ⚡ تحقق من المستخدم فقط - React Query يتولى جلب البيانات
 useEffect(() => {
