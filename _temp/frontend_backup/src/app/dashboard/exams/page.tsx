...
export default function SmartExamsPage() {
 ...
 const [results, setResults] = useState<any[]>([]);
  ...
  const { data: examsData, isLoading: examsLoading } = useQuery({
    queryKey: ['exams-exams'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/exams', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: exams_ai_generateData, isLoading: exams_ai_generateLoading } = useQuery({
    queryKey: ['exams-exams-ai-generate'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/exams/ai-generate', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
 ...
 { label: 'جاري الآن', value: exams.filter((e: any) => e.status === 'ACTIVE').length, icon: '⏱', color: '#EF4444' },
 { label: 'منشور', value: exams.filter((e: any) => e.status === 'PUBLISHED').length, icon: "ICON_Megaphone", color: '#3B82F6' },
 { label: 'منتهي', value: exams.filter((e: any) => e.status === 'COMPLETED').length, icon: "ICON_CheckCircle", color: '#10B981' },
 ].map((s, i) => (
 ...
 {QUESTION_TYPES.map((t: any) => <option key={t.value} value={t.value}><IconRenderer name={t.icon} /> {t.label}</option>)}
 </select>
 ...
 <select onChange={e => { const ex = exams.find((x: any) => x.id === +e.target.value); setSelectedExam(ex); if (ex) fetchResults(ex.id); }}
 style={{ padding: '9px 14px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: '#1a2d4a', color: '#fff', fontSize: 13 }}>
 <option value="">-- اختر --</option>
 {exams.map((e: any) => <option key={e.id} value={e.id}>{e.title_ar || e.title}</option>)}
 </select>
 </div>
 ...
 { label: 'الناجحون', value: results.filter((r: any) => r.passed).length, icon: "ICON_CheckCircle", color: '#10B981' },
 { label: 'الراسبون', value: results.filter((r: any) => !r.passed).length, icon: "ICON_XCircle", color: '#EF4444' },
 ].map((s, i) => (
 <div key={i} style={{ background: `${s.color}10`, border: `1px solid ${s.color}25`, borderRadius: 14, padding: '16px 20px' }}>
 <div style={{ color: '#9CA3AF', fontSize: 12 }}>{s.label}</div>
 <div style={{ color: '#fff', fontSize: 28, fontWeight: 800 }}>{s.value}</div>
 ...
 {['الطالب', 'الدرجة', 'النسبة', 'الوقت', 'الحالة', 'التاريخ'].map((h: any) => (
 ...
 {[{label:'عنوان الاختبار *',key:'title_ar',placeholder:'مثال: اختبار الفصل الأول'},{label:'المادة',key:'subject',placeholder:'مثال: الرياضيات'},{label:'الصف',key:'grade',placeholder:'مثال: الصف الثالث'},{label:'التعليمات',key:'instructions',placeholder:'تعليمات للطلاب'}].map((f: any) => (
 ...
 {[{label:'المدة (دقيقة)',key:'duration',type:'number'},{label:'الدرجة الكلية',key:'total_marks',type:'number'},{label:'درجة النجاح',key:'pass_marks',type:'number'}].map((f: any) => (
 <div key={f.key}>
