...
export default function TeacherDashboard() {
 ...
 const [hwErrMsg, setHwErrMsg] = useState('');
  ...
  const { data: homeworkData, isLoading: homeworkLoading } = useQuery({
    ...
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/homework', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: attendanceData, isLoading: attendanceLoading } = useQuery({
    ...
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/attendance', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
 ...
 try {
 const records = students.map((s: any) => ({ student_id: s.id, class_id: selectedClass, date: attendanceDate, status: attendance[s.id] || 'PRESENT' }));
 ...
 {[{ v: 'PRESENT', label: 'حاضر', color: '#10B981' }, { v: 'ABSENT', label: 'غائب', color: '#EF4444' }, { v: 'LATE', label: 'متأخر', color: '#F59E0B' }].map((opt: any) => (
