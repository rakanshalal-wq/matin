...
export default function DriverDashboard() {
 ...
 const [msgErr, setMsgErr] = useState(''); const [msgOk, setMsgOk] = useState(''); const [msgLoading, setMsgLoading] = useState(false);
  ...
  const { data: transportData, isLoading: transportLoading } = useQuery({
    queryKey: ['driver-transport'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/transport', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: student_trackingData, isLoading: student_trackingLoading } = useQuery({
    queryKey: ['driver-student-tracking'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/student-tracking', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: transport_locationData, isLoading: transport_locationLoading } = useQuery({
    queryKey: ['driver-transport-location'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/transport/location', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: transport_boardData, isLoading: transport_boardLoading } = useQuery({
    queryKey: ['driver-transport-board'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/transport/board', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: messagesData, isLoading: messagesLoading } = useQuery({
    ...
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/messages', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
 ...
 setTrips(tripList);
 setActiveTrip(tripList.find((t: any) => t.status === 'active' || t.status === 'in_progress') || null);
 ...
 setStudents(prev => prev.map((s: any) => s.id === boardStudent.id ? { ...s, boarded: true } : s));
 ...
 const boardedCount = students.filter((s: any) => s.boarded).length;
 ...
 {['الطالب','الحالة','الإجراء'].map((h: any) => <th key={h} style={{ padding:'12px 16px',color:'rgba(238,238,245,0.5)',fontSize:12,fontWeight:600,textAlign:'right',borderBottom:`1px solid ${BORDER}` }}>{h}</th>)}
 </tr>
 </thead>
 <tbody>
 {students.map((s,i) => (
 ...
 <thead>
 <tr style={{ background:'rgba(255,255,255,0.02)' }}>
 {['المسار','المركبة','الحالة','وقت البدء'].map((h: any) => <th key={h} style={{ padding:'12px 16px',color:'rgba(238,238,245,0.5)',fontSize:12,fontWeight:600,textAlign:'right',borderBottom:`1px solid ${BORDER}` }}>{h}</th>)}
 </tr>
 </thead>
 <tbody>
 ...
 {students.filter((s: any) =>!s.boarded).map((s,i) => (
 ...
 {students.filter((s: any) =>!s.boarded).length===0 && <p style={{ color:'rgba(238,238,245,0.3)',fontSize:14,textAlign:'center',padding:'20px 0' }}>جميع الطلاب ركبوا الحافلة</p>}
 </div>
 </div>
 ) : (
 <div>
