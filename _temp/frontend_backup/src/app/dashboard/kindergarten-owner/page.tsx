...
export default function KindergartenOwnerDashboard() {
 ...
 const [time, setTime] = useState(new Date());
  ...
  const { data: auth_verifyData, isLoading: auth_verifyLoading } = useQuery({
    queryKey: ['kindergarten-owner-auth-verify'],
    queryFn: async () => {
  ...
  const { data: dashboard_statsData, isLoading: dashboard_statsLoading } = useQuery({
    queryKey: ['kindergarten-owner-dashboard-stats'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
  ...
  const { data: schoolsData, isLoading: schoolsLoading } = useQuery({
    queryKey: ['kindergarten-owner-schools'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
  ...
  const { data: examsData, isLoading: examsLoading } = useQuery({
    queryKey: ['kindergarten-owner-exams'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
  ...
  const { data: admissionData, isLoading: admissionLoading } = useQuery({
    queryKey: ['kindergarten-owner-admission'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
  ...
  const { data: activity_logData, isLoading: activity_logLoading } = useQuery({
    queryKey: ['kindergarten-owner-activity-log'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
  ...
  const { data: vaccinationsData, isLoading: vaccinationsLoading } = useQuery({
    queryKey: ['kindergarten-owner-vaccinations'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/vaccinations', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
