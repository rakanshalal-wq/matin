...
export default function CollegesPage() {
 ...
 const [errMsg, setErrMsg] = useState('');
  ...
  const { data: collegesData, isLoading: collegesLoading } = useQuery({
    queryKey: ['colleges-colleges'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/colleges', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
 ...
 active: colleges.filter((c: any) => c.is_active).length,
 ...
 {schools.filter((s: any) => s.type === 'university' || s.type === 'institute').map((s: any) => (
