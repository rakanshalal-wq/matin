...
export default function WeeklySchedulePage() {
 ...
 const [subjectColorMap, setSubjectColorMap] = useState<Record<string, string>>({});
  ...
  const { data: classesData, isLoading: classesLoading } = useQuery({
    queryKey: ['weekly-schedule-classes'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/classes', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: teachersData, isLoading: teachersLoading } = useQuery({
    queryKey: ['weekly-schedule-teachers'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/teachers', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: subjectsData, isLoading: subjectsLoading } = useQuery({
    queryKey: ['weekly-schedule-subjects'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/subjects', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: academicData, isLoading: academicLoading } = useQuery({
    queryKey: ['weekly-schedule-academic'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/academic-structure?type=periods', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: weeklyData, isLoading: weeklyLoading } = useQuery({
    queryKey: ['weekly-schedule-weekly'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/weekly-schedule', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: academicData1, isLoading: academicLoading1 } = useQuery({
    queryKey: ['weekly-schedule-academic'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/academic-structure', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
