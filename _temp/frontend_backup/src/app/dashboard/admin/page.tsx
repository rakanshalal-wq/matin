...
export default function AdminDashboard() {
 ...
 const [cLoading, setCLoading] = useState(false);
  ...
  const { data: studentsData, isLoading: studentsLoading } = useQuery({
    ...
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/students', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
 ...
 {(['overview','students','teachers','classes'] as const).map((tab: any) =>{
 ...
 {['الاسم','البريد الإلكتروني','الجنس','المرحلة','الإجراءات'].map((h: any) =>(
