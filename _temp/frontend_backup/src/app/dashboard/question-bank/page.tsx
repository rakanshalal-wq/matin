...
const diffConfig: any = {
 ...
 medium: { label: 'متوسط', color: '#F59E0B', bg: 'rgba(245,158,11,0.15)', icon: "ICON_Circle" },
 hard: { label: 'صعب', color: '#EF4444', bg: 'rgba(239,68,68,0.15)', icon: "ICON_Circle" },
};
...
export default function QuestionBankPage() {
 ...
 const [importResult, setImportResult] = useState<any>(null);
  ...
  // ⚡ React Query hooks
  const { data: question_bankData, isLoading: question_bankLoading } = useQuery({
    queryKey: ['question-bank-question-bank'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/question-bank', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: question_bank_importData, isLoading: question_bank_importLoading } = useQuery({
    queryKey: ['question-bank-question-bank-import'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/question-bank/import', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
 ...
 lesson: form.lesson,
 options: form.question_type === 'mcq' ? form.options.filter((o: any) => o.trim()) : [],
 ...
 setMsg('CheckCircle تم إضافة السؤال بنجاح');
 fetchAll();
 ...
 setImportProgress('XCircle يجب أن يكون الملف بصيغة Excel (.xlsx أو .xls)');
 return;
 }
 setImportFile(file);
 setImportProgress('');
 ...
 if (!importFile) { setImportProgress('XCircle يرجى اختيار ملف Excel'); return; }
 if (!importSubject) { setImportProgress('XCircle يرجى تحديد المادة الدراسية'); return; }
 if (!importGrade) { setImportProgress('XCircle يرجى تحديد الصف الدراسي'); return; }
 setImporting(true);
 ...
 const data = await res.json();
 if (!res.ok) {
 setImportProgress('XCircle ' + (data.error || 'فشل الاستيراد'));
 return;
 }
 setImportProgress('');
 ...
 const lessons = [...new Set(questions.map((q: any) => q.lesson).filter(Boolean))];
 ...
 { label: 'سهل', value: stats.easy || questions.filter((q: any) => q.difficulty === 'easy').length, color: '#10B981' },
 { label: 'متوسط', value: stats.medium || questions.filter((q: any) => q.difficulty === 'medium').length, color: '#F59E0B' },
 { label: 'صعب', value: stats.hard || questions.filter((q: any) => q.difficulty === 'hard').length, color: '#EF4444' },
 ...
 {lessons.map((l: any) => <option key={l} value={l}>{l}</option>)}
 </select>
 </div>
 ...
 {canEdit && <button onClick={() => handleDelete(q.id)} style={{ background: 'none', border: 'none', color: '#EF4444', cursor: 'pointer', fontSize: '16px', padding: '4px', flexShrink: 0 }}> </button>}
 </div>
 </div>
 );
 })
 ...
 {form.options.filter((o: any) => o.trim()).map((opt, i) => (
 ...
 <button style={s.btn('#10B981')} onClick={handleAdd} disabled={saving}>{saving ? 'جاري الحفظ...' : 'CheckCircle حفظ السؤال'}</button>
 </div>
 </div>
 </div>
 )}
 ...
 style={{ width: '100%', padding: '8px 10px', borderRadius: '8px', background: '#0f172a', border: '1px solid #334155', color: '#f1f5f9', fontSize: '13px' }}>
 {importStage === 'الابتدائية' && ['الأول الابتدائي','الثاني الابتدائي','الثالث الابتدائي','الرابع الابتدائي','الخامس الابتدائي','السادس الابتدائي'].map((g: any) => <option key={g} value={g}>{g}</option>)}
 {importStage === 'المتوسطة' && ['الأول المتوسط','الثاني المتوسط','الثالث المتوسط'].map((g: any) => <option key={g} value={g}>{g}</option>)}
 {importStage === 'الثانوية' && ['أول ثانوي (مشترك)','ثاني ثانوي','ثالث ثانوي'].map((g: any) => <option key={g} value={g}>{g}</option>)}
 </select>
 </div>
 </div>
 ...
 style={{ width: '100%', padding: '8px 10px', borderRadius: '8px', background: '#0f172a', border: '1px solid #334155', color: '#f1f5f9', fontSize: '13px' }}>
 {['لغتي الخالدة','الرياضيات','العلوم','الدراسات الإسلامية','القرآن الكريم','اللغة الإنجليزية','التربية الوطنية','التاريخ','الجغرافيا','الفيزياء','الكيمياء','الأحياء','علم البيانات','الهندسة','مبادئ المحاسبة','مبادئ الإدارة','الاقتصاد','التفسير وعلوم القرآن','الحديث وعلومه','الفقه وأصوله','أخرى'].map((sub: any) => (
