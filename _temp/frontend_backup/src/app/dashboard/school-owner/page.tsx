...
export default function SchoolOwnerDashboard() {
 ...
 const [schoolOk, setSchoolOk] = useState('');
  ...
  const { data: dashboardData, isLoading: dashboardLoading } = useQuery({
    queryKey: ['school-owner-dashboard'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
  ...
  const { data: admissionData, isLoading: admissionLoading } = useQuery({
    queryKey: ['school-owner-admission'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/admission?status=pending&limit=5', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: activityData, isLoading: activityLoading } = useQuery({
    queryKey: ['school-owner-activity'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
  ...
  const { data: admissionData1, isLoading: admissionLoading1 } = useQuery({
    queryKey: ['school-owner-admission'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/admission', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
