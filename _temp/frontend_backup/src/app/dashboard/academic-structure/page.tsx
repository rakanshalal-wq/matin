...
export default function AcademicStructurePage() {
 ...
 const [form, setForm] = useState<any>({});
  ...
  // ⚡ React Query hooks
  const { data: academic_structureData, isLoading: academic_structureLoading } = useQuery({
    queryKey: ['academic-structure-academic-structure'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
      const r = await fetch('/api/academic-structure?type=all', { headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
      if (!r.ok) return null;
      return r.json();
    },
    staleTime: 2 * 60 * 1000,
  ...
  const { data: academic_structure_1Data, isLoading: academic_structure_1Loading } = useQuery({
    queryKey: ['academic-structure-academic-structure'],
    queryFn: async () => {
      const token = typeof window !== 'undefined' ? localStorage.getItem('matin_token') : null;
