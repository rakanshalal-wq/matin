    ...
    if (school_id && user.role === "super_admin") {
      const result = await pool.query(
        `SELECT p.*, ss.school_id, ss.status as sub_status, ss.end_date
         FROM plans p
         LEFT JOIN subscriptions ss ON ss.plan_id = p.id AND ss.school_id = $1 AND ss.status = "active"
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (body.action === "assign_to_school") {
      ...
      // جلب مالك المدرسة
      const schoolRes = await pool.query("SELECT owner_id FROM schools WHERE id = $1", [school_id]);
      ...
      // تحديث باقة المدرسة
      await pool.query(
        "UPDATE schools SET plan_id = $1, plan = $2 WHERE id = $3",
