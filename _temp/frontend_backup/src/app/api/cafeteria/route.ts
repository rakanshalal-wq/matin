...
export async function GET(request: Request) {
  try {
    ...
    const [countResult, dataResult] = await Promise.all([
      pool.query('SELECT COUNT(*) FROM cafeteria'),
      pool.query(`SELECT * FROM cafeteria ORDER BY created_at DESC LIMIT $1 OFFSET $2`, [limit, offset])
    ]);
...
export async function POST(request: Request) {
  try {
    ...
    if (!item_name) return NextResponse.json({ error: 'اسم الصنف مطلوب' }, { status: 400 });
