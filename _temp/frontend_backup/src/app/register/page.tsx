...
export default function RegisterPage() {
  ...
  const [step, setStep] = useState(1); // 1=معلومات المؤسسة, 2=بيانات الحساب, 3=تأكيد OTP
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // الانتقال لخطوة OTP
      setRegisteredEmail(form.contact_email.trim().toLowerCase());
      setStep(3);
    } catch {
  ...
  const handleOtpChange = (index: number, value: string) => {
    ...
    const newOtp = [...otp];
    newOtp[index] = value.slice(-1); // حرف واحد فقط
    setOtp(newOtp);
  ...
  const handleVerifyOtp = async () => {
    const code = otp.join('');
    if (code.length < 4) { setError('أدخل رمز التحقق المكون من 4 أرقام'); return; }
    setLoading(true);
    setError('');
    try {
      const res = await fetch('/api/auth/verify-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'verify', email: registeredEmail, code }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error || 'رمز التحقق غير صحيح'); return; }
      setSuccess('تم تأكيد بريدك الإلكتروني بنجاح! جاري توجيهك...');
  ...
  const handleResend = async () => {
    ...
    try {
      const res = await fetch('/api/auth/verify-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'resend', email: registeredEmail }),
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleSkipVerification = () => {
    // السماح بالدخول مباشرة بدون تأكيد البريد
      ...
      <div style={{ width: '100%', maxWidth: 480, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ background: '#0B0B16', border: '1px solid rgba(255, 255, 255, 0.06)', borderRadius: 24, padding: '40px 32px', boxShadow: '0 40px 100px rgba(0,0,0,0.5)' }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <h2 style={{ color: '#EEEEF5', fontSize: 24, fontWeight: 800, marginBottom: 8 }}>
              {step === 3 ? 'تأكيد البريد الإلكتروني' : 'ابدأ رحلتك مع متين'}
            </h2>
            <p style={{ color: 'rgba(238,238,245,0.4)', fontSize: 14, margin: 0 }}>
              {step === 3 ? `أدخل الرمز المرسل إلى ${registeredEmail}` : 'انضم لأكثر من 500 مؤسسة تعليمية في المملكة'}
            </p>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginBottom: 32 }}>
            {[1, 2, 3].map((s) => (
              <React.Fragment key={s}>
                ...
                {s < 3 && <div style={{ flex: 1, height: 1, background: step > s ? 'var(--gold)' : 'rgba(255,255,255,0.08)', maxWidth: 60, transition: 'all 0.3s' }} />}
              </React.Fragment>
            ))}
          </div>
          ...
          {resendMsg && <div style={{ background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 12, padding: '12px 16px', marginBottom: 20, color: '#93C5FD', fontSize: 14, fontWeight: 600 }}>{resendMsg}</div>}

          {/* الخطوة 1: معلومات المؤسسة */}
          {step === 1 && (
            <div>
          ...
          {step === 3 && (
            <div>
              ...
              <div style={{ textAlign: 'center', marginBottom: 28 }}>
                <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'rgba(201,168,76,0.1)', border: '2px solid rgba(201,168,76,0.3)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 32 }}>
                  📧
                </div>
              </div>

              {/* خانات OTP */}
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 28, direction: 'ltr' }}>
                {otp.map((digit, index) => (
                  <input
                    key={index}
                    ref={otpRefs[index]}
                    ...
                    style={{
                      ...
                      fontSize: 28,
                      fontWeight: 800,
                      background: digit ? 'rgba(201,168,76,0.1)' : 'rgba(255,255,255,0.04)',
                      border: digit ? '2px solid rgba(201,168,76,0.6)' : '1px solid rgba(255,255,255,0.1)',
                      borderRadius: 14,
                      color: '#C9A84C',
                      outline: 'none',
                      transition: 'all 0.2s',
              ...
              <button
                ...
                style={{ ...btnStyle, marginTop: 0, opacity: (loading || otp.join('').length < 4) ? 0.5 : 1, cursor: (loading || otp.join('').length < 4) ? 'not-allowed' : 'pointer' }}
              >
              ...
              <div style={{ textAlign: 'center', marginTop: 20 }}>
                <p style={{ color: 'rgba(238,238,245,0.4)', fontSize: 13, marginBottom: 8 }}>لم يصلك الرمز؟</p>
                <button
                  onClick={handleResend}
                  disabled={resendLoading}
                  style={{ background: 'none', border: 'none', color: '#C9A84C', fontSize: 14, fontWeight: 700, cursor: resendLoading ? 'not-allowed' : 'pointer', opacity: resendLoading ? 0.5 : 1, fontFamily: 'var(--font)' }}
                >
              ...
              <div style={{ textAlign: 'center', marginTop: 16 }}>
                <button
                  ...
                  style={{ background: 'none', border: 'none', color: 'rgba(238,238,245,0.3)', fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)', textDecoration: 'underline' }}
                >
                  تخطي التحقق والدخول مباشرة
                </button>
              </div>
            </div>
          )}
