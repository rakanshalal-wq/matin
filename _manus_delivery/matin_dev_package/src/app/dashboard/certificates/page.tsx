...
export default function CertificatesPage() {
 ...
 case 'excellence': return { text: 'تميز', color: 'var(--gold)', bg: 'rgba(201,162,39,0.1)' };
 ...
 <div>
 <h1 className="page-title"><IconRenderer name="ICON_Trophy" size={18} /> الشهادات</h1>
 ...
 {[
 { label: 'إجمالي الشهادات', value: items.length, icon: "ICON_Trophy", color: 'var(--gold)' },
 ...
 <button onClick={() => setShowAddModal(true)} style={{ marginTop: 16, background: 'linear-gradient(135deg, var(--gold), var(--gold2))', color: 'var(--bg)', padding: '12px 24px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer' }}><IconRenderer name="ICON_Plus" size={18} /> إصدار أول شهادة</button>
 </div>
 ) : (
 ...
 <div><label style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginBottom: 6, display: 'block' }}>نوع الشهادة</label><select value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value })} style={{ ...inputStyle, appearance: 'auto' } as any}><option value="completion" style={{ background: 'var(--bg)' }}>إتمام دراسة</option><option value="excellence" style={{ background: 'var(--bg)' }}>تميز</option><option value="participation" style={{ background: 'var(--bg)' }}>مشاركة</option><option value="behavior" style={{ background: 'var(--bg)' }}>سلوك</option><option value="transfer" style={{ background: 'var(--bg)' }}>نقل</option></select></div>
 ...
 <button onClick={handleSave} disabled={saving} style={{ background: 'linear-gradient(135deg, var(--gold), var(--gold2))', color: 'var(--bg)', padding: '12px 32px', borderRadius: 10, border: 'none', fontWeight: 700, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? '⏳ جاري الحفظ...' : 'Save إصدار الشهادة'}</button>
