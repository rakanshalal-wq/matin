...
import { BookOpen, Users, Calendar, ClipboardCheck, BarChart3, Clock, Award, FileText, Plus, ChevronLeft } from 'lucide-react';
...
export default function TrainerDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview'|'courses'|'trainees'|'attendance'>('overview');
  ...
  if (loading) return <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100vh',background:DARK,color:'#fff',fontFamily:"'IBM Plex Sans Arabic',sans-serif"}}><div style={{textAlign:'center'}}><BookOpen size={48} color={G} /><div style={{marginTop:16,color:DIM}}>جارٍ التحميل...</div></div></div>;

  const statCards = [
    { icon: BookOpen, label: 'البرامج التدريبية', value: stats.classes_count || classes.length || 0, color: '#3B82F6' },
    { icon: Users, label: 'المتدربون', value: stats.students_count || 0, color: '#10B981' },
    { icon: ClipboardCheck, label: 'الحضور اليوم', value: stats.today_attendance || '-', color: '#F59E0B' },
    { icon: Award, label: 'معدل الإنجاز', value: stats.completion_rate || '0%', color: '#A78BFA' },
  ];

  const tabs = [
    { id: 'overview', label: 'نظرة عامة', Ic: BarChart3 },
    { id: 'courses', label: 'البرامج', Ic: BookOpen },
    { id: 'trainees', label: 'المتدربون', Ic: Users },
    ...
    <div style={{fontFamily:"'IBM Plex Sans Arabic',sans-serif",direction:'rtl',background:DARK,color:'#EEEEF5',minHeight:'100vh'}}>
      <div style={{maxWidth:1200,margin:'0 auto',padding:'24px 16px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
          <div>
            <h1 style={{fontSize:22,fontWeight:800,margin:0}}>مرحباً، {user?.name || 'المدرب'}</h1>
            <p style={{fontSize:13,color:MUT,margin:'4px 0 0'}}>لوحة تحكم المدرب</p>
          </div>
        ...
        {activeTab === 'overview' && (
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(340px,1fr))',gap:16}}>
            <div style={{background:CARD,border:'1px solid '+BD,borderRadius:16,padding:20}}>
              <h3 style={{fontSize:16,fontWeight:700,marginBottom:14,display:'flex',alignItems:'center',gap:8}}><BookOpen size={18} color={G} /> البرامج التدريبية</h3>
              {classes.length === 0 ? <p style={{color:MUT,fontSize:13}}>لا توجد برامج حالياً</p> :
                classes.slice(0,5).map((c:any,i:number) => (
                  <div key={i} style={{padding:'10px 0',borderBottom:i<classes.length-1?'1px solid '+BD:'none',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div><div style={{fontSize:14,fontWeight:600}}>{c.name}</div><div style={{fontSize:11,color:MUT}}>{c.subject || c.description || ''}</div></div>
                    <div style={{fontSize:12,color:G}}>{c.students_count || 0} متدرب</div>
                  </div>
                ))
              }
            </div>
            <div style={{background:CARD,border:'1px solid '+BD,borderRadius:16,padding:20}}>
              <h3 style={{fontSize:16,fontWeight:700,marginBottom:14,display:'flex',alignItems:'center',gap:8}}><Calendar size={18} color={'#3B82F6'} /> الجدول القادم</h3>
              <p style={{color:MUT,fontSize:13}}>لا توجد جلسات قادمة</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'courses' && (
          <div style={{background:CARD,border:'1px solid '+BD,borderRadius:16,padding:20}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              <h3 style={{fontSize:16,fontWeight:700,margin:0}}>البرامج التدريبية</h3>
            </div>
            {classes.length === 0 ? <p style={{color:MUT,fontSize:13,textAlign:'center',padding:40}}>لا توجد برامج تدريبية</p> :
                ...
                {classes.map((c:any,i:number) => (
                  <div key={i} style={{background:'rgba(255,255,255,0.02)',border:'1px solid '+BD,borderRadius:12,padding:16,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div style={{display:'flex',alignItems:'center',gap:12}}>
                      ...
                      <div><div style={{fontSize:14,fontWeight:600}}>{c.name}</div><div style={{fontSize:11,color:MUT}}>{c.subject || ''} {c.schedule ? '| '+c.schedule : ''}</div></div>
                    </div>
                    <div style={{fontSize:13,color:G,fontWeight:600}}>{c.students_count || 0} متدرب</div>
                  </div>
                ))}
              </div>
            }
        ...
        {activeTab === 'trainees' && (
          <div style={{background:CARD,border:'1px solid '+BD,borderRadius:16,padding:20}}>
            <h3 style={{fontSize:16,fontWeight:700,marginBottom:14}}>المتدربون</h3>
            <select value={selectedClass} onChange={e => loadTrainees(e.target.value)} style={{width:'100%',background:'rgba(255,255,255,0.05)',border:'1px solid '+BD,color:'#EEEEF5',padding:'10px 14px',borderRadius:10,fontSize:13,fontFamily:'inherit',marginBottom:16}}>
              <option value="">اختر البرنامج</option>
              {classes.map((c:any) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            {students.length === 0 ? <p style={{color:MUT,fontSize:13,textAlign:'center',padding:20}}>اختر برنامجاً لعرض المتدربين</p> :
              <div style={{display:'grid',gap:8}}>
                {students.map((s:any,i:number) => (
                  <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 14px',background:'rgba(255,255,255,0.02)',borderRadius:10,border:'1px solid '+BD}}>
                    <div style={{width:36,height:36,borderRadius:'50%',background:G+'20',display:'flex',alignItems:'center',justifyContent:'center'}}><Users size={16} color={G} /></div>
                    <div><div style={{fontSize:13,fontWeight:600}}>{s.name || s.student_name}</div><div style={{fontSize:11,color:MUT}}>{s.email || s.phone || ''}</div></div>
                  </div>
                ))}
              </div>
            }
        ...
        {activeTab === 'attendance' && (
          <div style={{background:CARD,border:'1px solid '+BD,borderRadius:16,padding:20}}>
            <h3 style={{fontSize:16,fontWeight:700,marginBottom:14,display:'flex',alignItems:'center',gap:8}}><ClipboardCheck size={18} color={G} /> تسجيل الحضور</h3>
            <div style={{display:'flex',gap:12,marginBottom:16,flexWrap:'wrap'}}>
              <select value={selectedClass} onChange={e => loadTrainees(e.target.value)} style={{flex:1,minWidth:200,background:'rgba(255,255,255,0.05)',border:'1px solid '+BD,color:'#EEEEF5',padding:'10px 14px',borderRadius:10,fontSize:13,fontFamily:'inherit'}}>
                <option value="">اختر البرنامج</option>
                {classes.map((c:any) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            ...
            {students.length > 0 && (
              <>
                ...
                <button onClick={saveAttendance} disabled={savingAtt} style={{width:'100%',background:'linear-gradient(135deg,'+G+',#B8943A)',border:'none',borderRadius:12,padding:12,color:'#fff',fontSize:14,fontWeight:700,cursor:'pointer',fontFamily:'inherit',opacity:savingAtt?0.6:1}}>{savingAtt?'جارٍ الحفظ...':'حفظ الحضور'}</button>
