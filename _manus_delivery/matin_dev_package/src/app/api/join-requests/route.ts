...
export async function GET(request: Request) {
  try {
    ...
    let params: any[] = [];
    if (user.role === 'owner') {
      schoolFilter = 'AND u.school_id IN (SELECT id FROM schools WHERE owner_id = $1)';
      params = [String(user.id)];
    } else if (user.role === 'admin') {
      schoolFilter = 'AND u.school_id = $1';
...
export async function PUT(request: Request) {
  try {
    ...
    if (action === 'approve' && class_id) {
      await pool.query('UPDATE students SET class_id = $1 WHERE user_id = $2', [class_id, String(userId)]);
    }
