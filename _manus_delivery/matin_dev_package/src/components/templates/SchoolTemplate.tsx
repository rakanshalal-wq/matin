...
const SchoolTemplate: React.FC<SchoolTemplateProps> = ({ data, globalAds, globalProducts, globalLibrary }) => {
  const primaryColor = data.primary_color || '#D4A843';
  const secondaryColor = data.secondary_color || '#1B263B';
    ...
    <div style={{ background: '#0A0F1C', color: '#fff', minHeight: '100vh', direction: 'rtl', fontFamily: fontFamily }}>
      ...
      <nav style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.05)', position: 'fixed', top: 0, width: '100%', zIndex: 100, background: 'rgba(10,15,28,0.8)', backdropFilter: 'blur(12px)' }}>
        ...
        <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
          <a href="#about" style={{ color: '#fff', textDecoration: 'none', fontSize: '14px', fontWeight: 600 }}>عن المدرسة</a>
      ...
      {data.show_global_ads && globalAds && globalAds.length > 0 && (
        <div style={{ marginTop: '80px', background: 'linear-gradient(90deg, #D4A843, #FFD700)', color: '#000', padding: '12px', textAlign: 'center', fontWeight: 900, fontSize: '14px', position: 'relative', zIndex: 90 }}>
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 460px', gap: '60px', alignItems: 'center' }}>
          <div>
            ...
            <h1 style={{ fontSize: '52px', fontWeight: 900, lineHeight: 1.1, marginBottom: '20px' }}>
              نصنع قادة المستقبل <br /><span style={{ color: primaryColor }}>بأحدث المعايير</span>
            </h1>
            <p style={{ fontSize: '18px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: '32px', maxWidth: '520px' }}>
              {data.description || 'نحن ملتزمون بتقديم بيئة تعليمية محفزة تدمج بين التكنولوجيا والقيم لبناء جيل متميز.'}
            </p>
            <div style={{ display: 'flex', gap: '16px' }}>
              <button style={{ background: primaryColor, color: '#000', border: 'none', borderRadius: '12px', padding: '14px 32px', fontSize: '16px', fontWeight: 800, cursor: 'pointer' }}>سجل الآن</button>
              <button style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '14px 28px', color: '#fff', fontSize: '16px', fontWeight: 700, cursor: 'pointer' }}>جولة افتراضية</button>
            </div>
          </div>
          <div style={{ position: 'relative' }}>
            <div style={{ width: '100%', height: '400px', borderRadius: '30px', background: `linear-gradient(45deg, ${primaryColor}20, ${secondaryColor}20)`, border: '1px solid rgba(255,255,255,0.1)', overflow: 'hidden' }}>
              {data.cover_image ? <img src={data.cover_image} alt="School" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '80px' }}>🏫</div>}
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="store" style={{ padding: '80px 24px', background: 'rgba(255,255,255,0.02)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
            <div>
              <h2 style={{ fontSize: '32px', fontWeight: 900 }}>متجر متين التعليمي</h2>
              <p style={{ color: 'rgba(255,255,255,0.4)', marginTop: '8px' }}>منتجات مختارة بعناية لتعزيز تجربتكم التعليمية</p>
            </div>
          </div>
